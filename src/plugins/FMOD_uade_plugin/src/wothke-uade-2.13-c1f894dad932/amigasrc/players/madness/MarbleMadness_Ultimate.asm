	******************************************************
	**** Marble Madness Ultimate Race - Custom Module ****
	****         all adaptions by Wanted Team	  ****
	******************************************************

; XXX slightly patched version (my assembler doesn't like the B**.L addressing
; modes. also I renamed some labels to reflect the actual functions for better readability)	
; arrgh, seems this version no longer works like the original..
	
	incdir	"dh2:include/"
	include 'misc/Eagleplayer.i'

	SECTION Player,Code_C

MM_Start
	PLAYERHEADER PlayerTagArray

	dc.b	"$VER: Marble Madness 'Ultimate Race Music' - Custom Module,",10
	dc.b	'adapted by Don Adan/Wanted Team',0
	even

PlayerTagArray
	dc.l	DTP_CustomPlayer,1
	dc.l	DTP_PlayerName,PlayerName
	dc.l	DTP_Interrupt,Interrupt
	dc.l	DTP_InitPlayer,InitPlayer
	dc.l	DTP_EndPlayer,EndPlayer
	dc.l	DTP_InitSound,InitSound
	dc.l	DTP_EndSound,EndSound
	dc.l	DTP_SubSongRange,SubSongRange			; XXX only addition/change compared to SkyFox
	dc.l	EP_Get_ModuleInfo,Get_ModuleInfo
	dc.l	TAG_DONE
PlayerName
	dc.b	"CustomPlay",0
Text
	dc.b	10
	dc.b	"             MARBLE MADNESS",10
	dc.b	10
	dc.b	"            AMIGA VERSION BY",10
	dc.b	"               LARRY REED",10
	dc.b	10
	dc.b	"     (C) 1984-86 BY ATARI GAMES CORP.",10
	dc.b	"           AND ELECTRONIC ARTS",0
	even

Timer
	dc.w	0

***************************************************************************
***************************** DTP_SubSongRange ****************************
***************************************************************************

SubSongRange
	moveq	#0,D0
	moveq	#2,D1
	rts

***************************************************************************
***************************** EP_Get_ModuleInfo ***************************
***************************************************************************

Get_ModuleInfo
	lea	InfoBuffer(PC),A0
	rts

InfoBuffer
	dc.l	MI_SpecialInfo,Text
	dc.l	MI_LoadSize,MM_End-MM_Start
	dc.l	0

***************************************************************************
***************************** DTP_InitPlayer ******************************
***************************************************************************

InitPlayer
	move.l	4.W,SysBase
	
	bsr.w	StartMusic
	
	moveq	#0,D0
	rts

***************************************************************************
***************************** DTP_EndPlayer *******************************
***************************************************************************

EndPlayer
	bsr.w	StopMusic
	moveq	#0,D0
	rts

***************************************************************************
***************************** DTP_Intterrupt ******************************
***************************************************************************

Interrupt
	movem.l	D1-A6,-(SP)

	bsr.w	Play

	movem.l	(SP)+,D1-A6
	moveq	#0,D0
	rts

***************************************************************************
***************************** DTP_InitSound *******************************
***************************************************************************

; XXX this function expects A5 to already point to EagleBase (same as in Skyfox) 
; but EagleBase doesn't seem to be used elsewhere, i.e. contrary to other modules 
; there is no need to store a reference to EagleBase
InitSound
	move.w	Timer(PC),D0
	bne.b	Done
	move.w	dtg_Timer(A5),D0
	mulu.w	#5,D0
	divu.w	#6,D0			; 60 Hz
	lea	Timer(PC),A0
	move.w	D0,(A0)
Done	move.w	D0,dtg_Timer(A5)

; below corresponds to PlayMusic in Skyfox
	move.w	dtg_SndNum(A5),D1
	moveq	#0,D0
	move.b	SongNumber(PC,D1.W),D0
	clr.l	-(SP)
	move.l	D0,-(SP)
	bsr.w	InitMusic
	addq.l	#8,SP
	rts

; XXX seems there are 3 tracks and the below maps idx to some kind of offset
SongNumber
	dc.b	25
	dc.b	30
	dc.b	32
	dc.b	0

***************************************************************************
***************************** DTP_EndSound ********************************
***************************************************************************

EndSound
	rts

***************************************************************************
***************************************************************************
***************************************************************************

; Music from "Marble Madness" (c) 1986 by Electronic Arts

;	SECTION	MarbleMadness01EE28,CODE
whatever_1	LINK.W	A6,#-2
	MOVEM.L	D7,-(SP)
	MOVE.W	10(A6),D7
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9A2,A0
	CMPI.L	#$FFFFFFFF,(A0)
	BNE.S	whatever_1B
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

whatever_1B	MOVE.L	D7,D0
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9A2,A0
	MOVE.L	(A0),-(SP)
	JSR	StopSfx
	ADDQ.L	#4,SP
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9A2,A0
	MOVEQ	#-1,D1
	MOVE.L	D1,(A0)
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC01EE8A	LINK.W	A6,#-$1A
	MOVEM.L	D2/D3/D6/D7/A2-A5,-(SP)
	MOVE.B	15(A6),D6
	EXT.W	D6
	MOVEQ	#-1,D7
lbC01EE9A	ADDQ.W	#1,D7
	SUBI.W	#12,D6
	BGE.S	lbC01EE9A
	MOVE.L	D7,D0
	ADDI.W	#12,D6
	MOVE.W	10(A6),D7
	MOVE.L	D7,D1
	MOVE.L	D7,D2
	EXT.L	D2
	ASL.L	#4,D2
	MOVEA.L	D2,A0
	ADDA.L	#lbL01F91A,A0
	MOVEA.L	A0,A5
	MOVEA.L	(A5),A0
	MOVEA.L	10(A0),A4
	MOVE.W	D0,-$12(A6)
	ANDI.L	#$FFFF,D0
	MOVEQ	#0,D2
	MOVE.B	12(A5),D2
	CMP.L	D2,D0
	BLS.S	lbC01EEE0
	ANDI.W	#$FF,D2
	MOVE.W	D2,-$12(A6)
lbC01EEE0	MOVEQ	#0,D0
	MOVE.W	-$12(A6),D0
	MOVEQ	#0,D1
	MOVE.B	13(A5),D1
	CMP.L	D1,D0
	SCS	D2
	NEG.B	D2
	EXT.W	D2
	EXT.L	D2
	MOVE.W	D2,10(A6)
	TST.W	D2
	BEQ.S	lbC01EF06
	ANDI.W	#$FF,D1
	MOVE.W	D1,-$12(A6)
lbC01EF06	MOVEQ	#0,D0
	MOVE.W	-$12(A6),D0
	MOVEQ	#8,D1
	SUB.L	D0,D1
	MOVEQ	#0,D2
	MOVE.W	8(A5),D2
	ASL.L	D1,D2
	MOVE.L	D2,(A4)
	ASL.L	#1,D2
	MOVEA.L	4(A5),A0
	ADDA.L	D2,A0
	MOVE.L	A0,8(A4)
	MOVE.L	D6,D1
	MOVE.L	D6,D2
	EXT.L	D2
	ASL.L	#1,D2
	MOVEA.L	D2,A1
	ADDA.L	#lbL01F902,A1
	MOVE.W	(A1),D2
	MOVE.W	10(A6),D3
	ASL.W	D3,D2
	MOVE.W	D2,4(A4)
	MOVE.L	D7,D3
	EXT.L	D3
	MOVEA.L	D3,A0
	ADDA.L	#lbL01F99A,A0
	MOVE.B	(A0),D3
	ANDI.W	#$FF,D3
	MOVE.W	D3,6(A4)
	MOVEA.L	(A5),A0
	TST.L	(A0)
	BEQ.S	lbC01EF8E
	MOVEA.L	(A0),A1
	MOVEA.L	10(A1),A3
	MOVEQ	#8,D2
	SUB.L	D0,D2
	MOVEQ	#0,D0
	MOVE.W	10(A5),D0
	ASL.L	D2,D0
	MOVE.L	D0,(A3)
	MOVE.L	(A4),D2
	SUB.L	D0,D2
	ASL.L	#1,D2
	MOVEA.L	8(A4),A0
	ADDA.L	D2,A0
	MOVE.L	A0,8(A3)
	MOVE.W	4(A4),4(A3)
	MOVE.W	6(A4),6(A3)
lbC01EF8E	TST.B	15(A5)
	BNE.S	lbC01EF9E
	MOVE.B	14(A5),D0
	CMP.B	15(A6),D0
	BEQ.S	lbC01F00C
lbC01EF9E	MOVE.L	D7,D0
	EXT.L	D0
	MOVE.L	D0,-(SP)
;XXX	BSR	whatever_1
	BSR	whatever_1
	ADDQ.L	#4,SP
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9A2,A0
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A1
	ADDA.L	#lbL01FE82,A1
	MOVE.L	D7,D1
	EXT.L	D1
	ASL.L	#2,D1
	MOVEA.L	D1,A2
	ADDA.L	#lbL01FEA2,A2
	MOVE.L	A0,$24(SP)
	MOVEA.L	$10(A6),A0
	MOVE.B	6(A0),D1
	EXT.W	D1
	EXT.L	D1
	MOVE.W	4(A0),D2
	EXT.L	D2
	MOVE.L	D2,-(SP)
	MOVE.L	D1,-(SP)
	MOVE.L	#$FFFF,-(SP)
	MOVE.L	(A2),-(SP)
	MOVE.L	(A1),-(SP)
	MOVE.L	(A5),-(SP)
	JSR	StartSfx
	LEA	$18(SP),SP
	MOVEA.L	$24(SP),A0
	MOVE.L	D0,(A0)
lbC01F00C	MOVE.B	15(A6),14(A5)
	MOVE.L	D7,D0
	EXT.L	D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9C2,A0
	MOVE.B	(A0),D0
	ANDI.B	#$40,D0
	MOVE.B	D0,15(A5)
	MOVEM.L	(SP)+,D2/D3/D6/D7/A2-A5
	UNLK	A6
	RTS

foo1	LINK.W	A6,#-2
	CLR.W	-2(A6)
lbC01F038	MOVE.W	-2(A6),D0
	CMPI.W	#8,D0
	BGE.S	lbC01F052
	EXT.L	D0
	MOVE.L	D0,-(SP)
;XXX	BSR	whatever_1
	BSR	whatever_1
	ADDQ.L	#4,SP
	ADDQ.W	#1,-2(A6)
	BRA.S	lbC01F038

lbC01F052	UNLK	A6
	RTS

lbC01F056	LINK.W	A6,#0
	MOVEM.L	A2,-(SP)
	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9FA,A0
	MOVEQ	#0,D0
	MOVE.W	D0,(A0)
	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FA0A,A0
	MOVE.W	(A0),D0
	SUBQ.W	#1,D0
	MOVE.W	D0,(A0)
	TST.W	D0
	BLE.S	lbC01F094
	MOVEM.L	(SP)+,A2
	UNLK	A6
	RTS

lbC01F094	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9EA,A0
	MOVE.W	(A0),D1
	ADDQ.W	#1,D1
	MOVE.W	D1,(A0)
	MOVE.W	10(A6),D1
	EXT.L	D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01FA0A,A0
	MOVEA.L	D0,A1
	ADDA.L	#lbL01F9EA,A1
	MOVE.W	(A1),D0
	MULS.W	#6,D0
	MOVEA.L	12(A6),A2
	MOVEA.L	(A2),A1
	ADDA.L	D0,A1
	MOVE.W	(A1),(A0)
	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FA0A,A0
	MOVE.W	(A0),D0
	TST.W	D0
	BNE.S	lbC01F15A
	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9EA,A0
	MOVEQ	#0,D0
	MOVE.W	D0,(A0)
	MOVE.W	10(A6),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FA0A,A0
	MOVEA.L	(A2),A1
	MOVE.W	(A1),(A0)
	TST.W	10(A6)
	BNE.S	lbC01F15A
	SUBQ.W	#1,lbW01FA1E
	TST.W	lbW01FA1E
	BGT.S	lbC01F15A
	MOVE.B	#1,lbB01F8FE
;XXX	BSR	foo1
	BSR	foo1	
	MOVE.W	#1,musicDone
	
; Skyfox makes additional call to SongEnd here	
	
	MOVE.B	lbB01FA24,D0
	EXT.W	D0
	EXT.L	D0
	TST.L	D0
	BLE.S	lbC01F15A
	MOVEQ	#1,D1
	ASL.L	D0,D1
	MOVE.L	D1,-(SP)
	MOVE.L	lbL01FA20,-(SP)
	JSR	_Signal
	ADDQ.L	#8,SP
lbC01F15A	MOVEM.L	(SP)+,A2
	UNLK	A6
	RTS

lbC01F162	LINK.W	A6,#-$14
	MOVEM.L	D2/D6/D7/A2,-(SP)
	MOVE.W	10(A6),D7
	EXT.L	D7
lbC01F170	TST.W	musicDone
;XXX	BNE	lbC01F250
	BNE	lbC01F250
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9EA,A0
	MOVE.W	(A0),D2
	MULS.W	#6,D2
	MOVEA.L	12(A6),A1
	MOVEA.L	(A1),A0
	ADDA.L	D2,A0
	MOVEA.L	D1,A2
	ADDA.L	#lbL01F9FA,A2
	MOVE.W	(A2),D1
	MOVE.W	D1,$1C(SP)
	ADDQ.W	#1,D1
	MOVE.W	D1,(A2)
	MOVE.W	$1C(SP),D1
	EXT.L	D1
	MOVEA.L	2(A0),A2
	ADDA.L	D1,A2
	MOVE.B	(A2),D6
	MOVE.L	D6,D1
	BTST	#7,D1
	BEQ.S	lbC01F1E0
	MOVE.L	D6,D1
	ANDI.B	#15,D1
	TST.B	D1
	BNE.S	lbC01F1D4
	MOVE.L	A1,-(SP)
	MOVE.L	D0,-(SP)
;XXX	BSR	lbC01F056
	BSR	lbC01F056
	ADDQ.L	#8,SP
	BRA.S	lbC01F170

lbC01F1D4	MOVEA.L	D7,A0
	ADDA.L	#lbL01F9C2,A0
	MOVE.B	D6,(A0)
	BRA.S	lbC01F170

lbC01F1E0	MOVE.L	D7,D0
	MOVE.L	D7,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL01F9CA,A0
	MOVEA.L	D0,A1
	ADDA.L	#lbL01F9C2,A1
	MOVE.B	(A1),D1
	EXT.W	D1
	EXT.L	D1
	ANDI.L	#15,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A1
	ADDA.L	#lbL01FA26,A1
	MOVE.L	(A1),D1
	ADD.L	D1,(A0)
	CMPI.B	#$7F,D6
	BEQ.S	lbC01F250
	MOVE.B	lbB01FA25,D0
	ADD.B	D6,D0
	MOVE.L	D0,D6
	BLT.S	lbC01F248
	CMPI.B	#$6C,D6
	BGE.S	lbC01F248
	TST.W	lbW01F8FC
	BEQ.S	lbC01F248
	MOVE.L	D6,D0
	EXT.W	D0
	EXT.L	D0
	MOVE.L	12(A6),-(SP)
	MOVE.L	D0,-(SP)
	MOVE.L	D7,-(SP)
;XXX	BSR	lbC01EE8A
	BSR	lbC01EE8A
	LEA	12(SP),SP
	NOP
lbC01F248	MOVEM.L	(SP)+,D2/D6/D7/A2
	UNLK	A6
	RTS

lbC01F250	MOVE.L	D7,-(SP)
;XXX	BSR	whatever_1
	BSR	whatever_1
	ADDQ.L	#4,SP
	MOVEM.L	(SP)+,D2/D6/D7/A2
	UNLK	A6
	RTS

Play
lbC01F260	ADDQ.L	#1,jiffyClock
	MOVE.W	lbW01F900_allocSig,D0
	EXT.L	D0
	MOVEQ	#1,D1
	ASL.L	D0,D1
	MOVE.L	D1,-(SP)
	MOVE.L	lbL01FADE,-(SP)
	JSR	_Signal
	ADDQ.L	#8,SP
	MOVEQ	#0,D0
	RTS

lbC01F286	LINK.W	A6,#-14
	MOVEM.L	D6/D7/A4/A5,-(SP)
	MOVE.L	lbL01FA68,D6
	LEA	lbL01F9CA,A5
	MOVEA.L	lbL01FA1A,A4
	MOVEQ	#0,D7
	SUBQ.L	#8,A4
lbC01F2A4	ADDQ.L	#8,A4
	TST.L	(A4)
	BEQ.S	lbC01F2E4
	TST.L	(A5)
	BGT.S	lbC01F2BE
	MOVE.L	D7,D0
	EXT.L	D0
	MOVE.L	A4,-(SP)
	MOVE.L	D0,-(SP)
;XXX	BSR	lbC01F162
	BSR	lbC01F162
	ADDQ.L	#8,SP
	BRA.S	lbC01F2E4

lbC01F2BE	MOVE.L	(A5),D0
	CMP.L	D6,D0
	BGT.S	lbC01F2E4
	MOVE.L	D7,D0
	EXT.L	D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9C2,A0
	MOVE.B	(A0),D0
	BTST	#6,D0
	BEQ.S	lbC01F2E4
	MOVE.L	D7,D0
	EXT.L	D0
	MOVE.L	D0,-(SP)
;XXX	BSR	whatever_1
	BSR	whatever_1
	ADDQ.L	#4,SP
lbC01F2E4	MOVEA.L	A5,A0
	ADDQ.L	#4,A5
	SUB.L	D6,(A0)
	ADDQ.W	#1,D7
	CMPI.W	#8,D7
	BLT.S	lbC01F2A4
	MOVEM.L	(SP)+,D6/D7/A4/A5
	UNLK	A6
	RTS

runTask2_lbC01F2FA	LINK.W	A6,#-8
	MOVEQ	#-1,D0
	MOVE.L	D0,-(SP)
	JSR	_AllocSignal
	ADDQ.L	#4,SP
	MOVE.L	jiffyClock,-8(A6)
	MOVE.W	D0,lbW01F900_allocSig
	
	
lbC01F318	MOVE.W	lbW01F900_allocSig,D0
	EXT.L	D0
	MOVEQ	#1,D1
	ASL.L	D0,D1
	MOVE.L	D1,-(SP)
	JSR	_Wait
	
	ADDQ.L	#4,SP
	MOVE.B	#1,lbB01F8FF
	TST.B	lbB01F8FE
	BNE.S	lbC01F38A
	MOVE.L	-8(A6),D0
	MOVE.L	jiffyClock,D1
	MOVE.L	D1,-8(A6)
	SUB.L	D0,D1
	MOVE.L	D1,-4(A6)
	CMPI.L	#5,D1
	BLE.S	lbC01F362
	MOVEQ	#5,D0
	MOVE.L	D0,-4(A6)
	BRA.S	lbC01F378

lbC01F362	MOVE.W	maxDelay,D0
	EXT.L	D0
	MOVE.L	-4(A6),D1
	CMP.L	D0,D1
	BLE.S	lbC01F378
	MOVE.W	D1,maxDelay
	
	
lbC01F378	MOVE.L	-4(A6),D0
	SUBQ.L	#1,-4(A6)
	TST.L	D0
	BEQ.S	lbC01F38A
;XXX	BSR	lbC01F286
	BSR	lbC01F286
	BRA.S	lbC01F378

lbC01F38A	CLR.B	lbB01F8FF
	BRA.S	lbC01F318

StopMus	MOVE.B	#1,lbB01F8FE
; XXX this blocking wait is absent in SkyFox (code slightly different)
lbC01F39A	TST.B	lbB01F8FF
	BNE.S	lbC01F39A
	BSR	foo1
	RTS

ReStartMus	BSR.S	StopMus
	CLR.B	lbB01F8FE
	RTS

SetTempo	LINK.W	A6,#-4
	MOVE.B	lbB01F8FE,D0
	EXT.W	D0
	EXT.L	D0
	MOVE.L	D0,-4(A6)
	BSR.S	StopMus
	MOVEQ	#9,D0
	MOVE.W	10(A6),D1
	ANDI.L	#$FFFF,D1
	ASL.L	D0,D1
	MOVE.L	D1,D0
	MOVE.L	#$96,D1
	JSR	SetTempo_sub1
	MOVE.L	D0,lbL01FA68
	TST.L	-4(A6)
	BNE.S	lbC01F3F0
	BSR.S	ReStartMus
lbC01F3F0	UNLK	A6
	RTS

Initmus	LINK.W	A6,#-10
	MOVEQ	#1,D0
	MOVE.B	D0,lbB01F8FE
	EXT.W	D0
	MOVE.W	D0,musicDone
	CLR.B	lbB01F8FF
	MOVEQ	#$70,D0
	MOVE.L	D0,-(SP)
	PEA	lbL01FCE2
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#$70,D0
	MOVE.L	D0,-(SP)
	PEA	lbL01FDB2
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVE.L	#$200,-(SP)
	PEA	lbL01FAE2
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#$5C,D0
	MOVE.L	D0,-(SP)
	PEA	lbL01FA82
	JSR	_Zero
	ADDQ.L	#8,SP
;	MOVEQ	#$16,D0
;	MOVE.L	D0,-(SP)
;	PEA	lbL01FA6C
;	JSR	_Zero
;	ADDQ.L	#8,SP
	MOVE.L	#lbL01FAE2,lbL01FABC
	LEA	lbW01FCE0,A0
	MOVE.L	A0,lbL01FAB8
	MOVE.L	A0,lbL01FAC0
	MOVE.B	#1,lbB01FA8A
	MOVE.W	lbW01F8F8,D0
	MOVE.B	D0,lbB01FA8B
	
	MOVE.L	#lbL01FA82,lbL01FADE
	MOVE.L	#lbL01FCE2,-6(A6)
	CLR.W	-2(A6)
lbC01F4B0	MOVE.W	-2(A6),D0
	CMPI.W	#8,D0
	BGE.S	lbC01F516
	EXT.L	D0
	MOVE.L	D0,0(SP)
	ASL.L	#4,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F91A,A0
	MOVEA.L	-6(A6),A1
	MOVE.L	A1,(A0)
	MOVE.L	0(SP),D0
	MOVEQ	#12,D1
	JSR	_CXM33
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FD52,A0
	MOVE.L	A0,10(A1)
	MOVE.L	0(SP),D0
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F9A2,A0
	MOVEQ	#-1,D0
	MOVE.L	D0,(A0)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL01F9C2,A0
	MOVE.B	#$C1,(A0)
	ADDI.L	#14,-6(A6)
	ADDQ.W	#1,-2(A6)
	BRA.S	lbC01F4B0

lbC01F516	MOVE.L	#lbL01FDB2,-6(A6)
	CLR.W	-2(A6)
lbC01F522	MOVE.W	-2(A6),D0
	CMPI.W	#8,D0
	BGE.S	lbC01F554
	MOVEA.L	-6(A6),A0
	MOVE.W	#$FFFE,4(A0)
	MULS.W	#12,D0
	MOVEA.L	D0,A1
	ADDA.L	#lbL01FE22,A1
	MOVE.L	A1,10(A0)
	ADDI.L	#14,-6(A6)
	ADDQ.W	#1,-2(A6)
	BRA.S	lbC01F522

lbC01F554	MOVE.L	#$4B00,-(SP)
	BSR	SetTempo
	ADDQ.L	#4,SP
	CLR.L	-(SP)

	PEA	runTask2_lbC01F2FA(PC)
	PEA	lbL01FA82
	JSR	_AddTask
	
	LEA	12(SP),SP
;	MOVEQ	#$16,D0
;	MOVE.L	D0,-(SP)
;	PEA	lbL01FA6C
;	JSR	_Zero
;	ADDQ.L	#8,SP
;	LEA	lbC01F260(PC),A0
;	MOVE.L	A0,lbL01FA7E
;	MOVE.B	#2,lbB01FA74
;	PEA	lbL01FA6C
;	MOVEQ	#5,D0
;	MOVE.L	D0,-(SP)
;	JSR	_AddIntServer
;	ADDQ.L	#8,SP
	UNLK	A6
	RTS

ResetMus	BSR	StopMus
;	PEA	lbL01FA6C
;	MOVEQ	#5,D0
;	MOVE.L	D0,-(SP)
;	JSR	_RemIntServer
;	ADDQ.L	#8,SP
	PEA	lbL01FA82
	JSR	_RemTask
	ADDQ.L	#4,SP
	RTS

StartMus	LINK.W	A6,#-4
	MOVEM.L	A2,-(SP)
	BSR	StopMus
	MOVE.L	8(A6),lbL01FA1A
	MOVE.W	14(A6),lbW01FA1E
	MOVE.B	$13(A6),lbB01FA24
	CLR.L	-(SP)
	JSR	_FindTask
	
	ADDQ.L	#4,SP
	MOVEQ	#$20,D1
	MOVE.L	D1,-(SP)
	PEA	lbL01F9CA
	MOVE.L	D0,lbL01FA20
	JSR	_Zero
	
	ADDQ.L	#8,SP
	MOVEQ	#$10,D0
	MOVE.L	D0,-(SP)
	PEA	lbL01F9FA
	JSR	_Zero
	
	ADDQ.L	#8,SP
	MOVEQ	#$10,D0
	MOVE.L	D0,-(SP)
	PEA	lbL01F9EA
	JSR	_Zero
	
	ADDQ.L	#8,SP
	CLR.L	-4(A6)
lbC01F644	MOVE.L	-4(A6),D0
	CMPI.L	#8,D0
	BGE.S	lbC01F694
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FA0A,A0
	MOVE.L	-4(A6),D0
	ASL.L	#3,D0
	MOVEA.L	lbL01FA1A,A1
	ADDA.L	D0,A1
	MOVEA.L	(A1),A2

	cmp.l	#0,A2
	beq.b	SkipZero

	MOVE.W	(A2),(A0)
SkipZero
	MOVE.L	-4(A6),D0
	ASL.L	#4,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F91A,A0
	MOVE.B	#$80,14(A0)
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F91A,A0
	MOVE.B	#1,15(A0)
	ADDQ.L	#1,-4(A6)
	BRA.S	lbC01F644

lbC01F694	MOVEQ	#0,D0
	MOVE.W	D0,musicDone
	MOVE.W	$16(A6),D0
	MOVE.W	D0,lbW01F8FC
	TST.W	D0
	BNE.S	lbC01F6C0
	CLR.L	-4(A6)
lbC01F6AE	BSR	lbC01F286
	ADDQ.L	#1,-4(A6)
	MOVE.W	musicDone,D0
	TST.W	D0
	BEQ.S	lbC01F6AE
lbC01F6C0	BSR	ReStartMus
	MOVE.L	-4(A6),D0
	MOVEM.L	(SP)+,A2
	UNLK	A6
	RTS

lbC01F6D0	LINK.W	A6,#0
	MOVEM.L	D2,-(SP)
	MOVEQ	#0,D0
	MOVE.W	14(A6),D0
	MOVE.B	$13(A6),D1
	EXT.W	D1
	EXT.L	D1
	MOVEQ	#1,D2
	MOVE.L	D2,-(SP)
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	MOVE.L	8(A6),-(SP)
	BSR	StartMus
	LEA	$10(SP),SP
	MOVEM.L	(SP)+,D2
	UNLK	A6
	RTS

;	LINK.W	A6,#0
;	MOVEQ	#0,D0
;	MOVE.W	14(A6),D0
;	MOVEQ	#0,D1
;	MOVE.L	D1,-(SP)
;	MOVE.L	D1,-(SP)
;	MOVE.L	D0,-(SP)
;	MOVE.L	8(A6),-(SP)
;	BSR	StartMus
;	LEA	$10(SP),SP
;	UNLK	A6
;	RTS

;	LINK.W	A6,#0
;	MOVE.B	11(A6),D0
;	ADDQ.B	#5,D0
;	MOVE.B	D0,lbB01FA25
;	UNLK	A6
;	RTS

;	LEA	lbL01FA26,A0
;	MOVE.L	A0,D0
;	RTS

lbC01F742	LINK.W	A6,#-4
	MOVEQ	#1,D0
	MOVE.L	D0,-4(A6)
lbC01F74C	MOVE.L	-4(A6),D0
	MOVEQ	#1,D1
	ASL.L	D0,D1
	MOVEA.L	8(A6),A0
	MOVE.L	(A0),D0
	AND.L	D1,D0
	TST.L	D0
	BNE.S	lbC01F77A
	MOVE.L	4(A0),D0
	AND.L	D1,D0
	TST.L	D0
	BNE.S	lbC01F77A
	CMPI.L	#9,-4(A6)
	BGE.S	lbC01F77A
	ADDQ.L	#1,-4(A6)
	BRA.S	lbC01F74C

lbC01F77A	MOVE.L	-4(A6),D0
	UNLK	A6
	RTS

lbC01F782	LINK.W	A6,#-$14
	MOVEM.L	D2/D3/A5,-(SP)
	MOVEQ	#0,D0
	MOVE.B	11(A6),D0
	MOVE.L	D0,12(SP)
	ASL.L	#4,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F91A,A0
	MOVEA.L	A0,A5
	MOVE.L	12(SP),D0
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FE82,A0
	MOVE.L	$14(A6),(A0)
	MOVE.L	12(SP),D0
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01FEA2,A0
	MOVE.L	$18(A6),(A0)
	MOVEA.L	$10(A6),A0
	MOVE.L	8(A0),D0
	MOVE.L	D0,-8(A6)
	TST.L	D0
	BNE.S	lbC01F7E2
	MOVE.L	A0,-(SP)
	BSR	lbC01F742
	ADDQ.L	#4,SP
	MOVE.L	D0,-12(A6)
	BRA.S	lbC01F818

lbC01F7E2	MOVEQ	#1,D0
	MOVE.L	D0,-12(A6)
lbC01F7E8	MOVE.L	-12(A6),D0
	MOVEQ	#1,D1
	ASL.L	D0,D1
	CMP.L	-8(A6),D1
	BCC.S	lbC01F7FC
	ADDQ.L	#1,-12(A6)
	BRA.S	lbC01F7E8

lbC01F7FC	MOVE.L	-12(A6),D0
	MOVEQ	#1,D1
	ASL.L	D0,D1
	CMP.L	-8(A6),D1
	BEQ.S	lbC01F818
	MOVE.L	$10(A6),-(SP)
	BSR	lbC01F742
	ADDQ.L	#4,SP
	MOVE.L	D0,-12(A6)
lbC01F818	MOVEQ	#0,D0
	MOVEA.L	$10(A6),A0
	MOVE.B	14(A0),D0
	MOVE.L	D0,-$10(A6)
	CMPI.L	#9,D0
	BLS.S	lbC01F834
	MOVEQ	#9,D0
	MOVE.L	D0,-$10(A6)
lbC01F834	TST.L	-$10(A6)
	BNE.S	lbC01F840
	MOVEQ	#1,D0
	MOVE.L	D0,-$10(A6)
lbC01F840	MOVEQ	#10,D0
	SUB.L	-$10(A6),D0
	MOVE.L	D0,-8(A6)
	CMP.L	-12(A6),D0
	BCC.S	lbC01F854
	MOVE.L	D0,-12(A6)
lbC01F854	MOVEQ	#9,D0
	MOVE.L	-12(A6),D1
	SUB.L	D1,D0
	MOVE.B	D0,12(A5)
	MOVE.L	-8(A6),D0
	SUB.L	D1,D0
	MOVE.B	D0,13(A5)
	MOVEA.L	$10(A6),A0
	MOVE.L	4(A0),D0
	LSR.L	D1,D0
	MOVE.W	D0,10(A5)
	MOVE.L	(A0),D2
	LSR.L	D1,D2
	MOVEQ	#0,D3
	MOVE.W	10(A5),D3
	ADD.L	D3,D2
	MOVE.W	D2,8(A5)
	SUBQ.L	#1,D1
	MOVEQ	#1,D0
	ASL.L	D1,D0
	MOVEQ	#0,D1
	MOVE.W	8(A5),D1
	JSR	lbC023EF8
	ASL.L	#1,D0
	MOVEA.L	12(A6),A0
	SUBA.L	D0,A0
	MOVE.L	A0,4(A5)
	MOVEA.L	(A5),A0
	MOVE.W	#1,4(A0)
	TST.W	10(A5)
	BEQ.S	lbC01F8CE
	MOVEQ	#0,D0
	MOVE.B	11(A6),D0
	MOVEQ	#14,D1
	JSR	_CXM33
	MOVEA.L	D0,A1
	ADDA.L	#lbL01FDB2,A1
	MOVE.L	A1,(A0)
	BRA.S	lbC01F8D2

lbC01F8CE	MOVEA.L	(A5),A0
	CLR.L	(A0)
lbC01F8D2	MOVEM.L	(SP)+,D2/D3/A5
	UNLK	A6
	RTS

lbC01F8DA	LINK.W	A6,#0
	MOVEQ	#0,D0
	MOVE.B	11(A6),D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL01F99A,A0
	MOVE.B	15(A6),(A0)
	UNLK	A6
	RTS


;	SECTION	MarbleMadness01F8F4,DATA
jiffyClock	dc.l	0
; XXX extra .w as compared to SkyFox
lbW01F8F8	dc.w	6
musicDone	dc.w	1
lbW01F8FC	dc.w	1

lbB01F8FE	dc.b	1
lbB01F8FF	dc.b	0
lbW01F900_allocSig	dc.w	0
lbL01F902	dc.l	$1AC0194
	dc.l	$17D0168
	dc.l	$1530140
	dc.l	$12E011D
	dc.l	$10D00FE
	dc.l	$F000E2
lbL01F91A	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01F99A	dc.l	$1010101
	dc.l	0
lbL01F9A2	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01F9C2	dc.l	0
	dc.l	0
lbL01F9CA	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01F9EA	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01F9FA	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01FA0A	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL01FA1A	dc.l	0
lbW01FA1E	dc.w	0
lbL01FA20	dc.l	0
lbB01FA24	dc.b	0
lbB01FA25	dc.b	5
lbL01FA26	dc.l	$10000
	dc.l	$600000
	dc.l	$300000
	dc.l	$180000
	dc.l	$C0000
	dc.l	$60000
	dc.l	$30000
	dc.l	$20000
	dc.l	$480000
	dc.l	$240000
	dc.l	$120000
	dc.l	$90000
	dc.l	$200000
	dc.l	$100000
	dc.l	$80000
	dc.l	$40000
maxDelay	dc.w	0

;	SECTION	MarbleMadness01FA68,BSS
lbL01FA68	ds.l	1
;lbL01FA6C	ds.l	2
;lbB01FA74	ds.b	10
;lbL01FA7E	ds.l	1
lbL01FA82	ds.l	2
lbB01FA8A	ds.b	1
lbB01FA8B	ds.b	$2D
lbL01FAB8	ds.l	1
lbL01FABC	ds.l	1
lbL01FAC0	ds.l	7
	ds.w	1
lbL01FADE	ds.l	1
lbL01FAE2	ds.l	$7F
	ds.w	1
lbW01FCE0	ds.w	1
lbL01FCE2	ds.l	$1C
lbL01FD52	ds.l	$18
lbL01FDB2	ds.l	$1C
lbL01FE22	ds.l	$18
lbL01FE82	ds.l	8
lbL01FEA2	ds.l	8
	ds.w	1


;	SECTION	MarbleMadness01FEC4,CODE
MakePort	LINK.W	A6,#0
	MOVEQ	#0,D0
	MOVEA.L	8(A6),A0
	MOVE.B	D0,14(A0)
	MOVE.B	D0,9(A0)
	CLR.L	10(A0)
	MOVE.B	#4,8(A0)
	MOVEQ	#-1,D0
	MOVE.L	D0,-(SP)
	JSR	_AllocSignal
	ADDQ.L	#4,SP
	MOVEA.L	8(A6),A0
	MOVE.B	D0,15(A0)
	CLR.L	-(SP)
	JSR	_FindTask
	ADDQ.L	#4,SP
	MOVEA.L	8(A6),A0
	MOVE.L	D0,$10(A0)
	ADDA.W	#$14,A0
	MOVE.L	A0,-(SP)
	JSR	_NewList
	ADDQ.L	#4,SP
	UNLK	A6
	RTS

UnmakePort	LINK.W	A6,#0
	MOVEQ	#0,D0
	MOVEA.L	8(A6),A0
	MOVE.B	15(A0),D0
	MOVE.L	D0,-(SP)
	JSR	_FreeSignal
	ADDQ.L	#4,SP
	UNLK	A6
	RTS

OpenAD_etc	LINK.W	A6,#-$12
	MOVEQ	#$44,D0
	MOVE.L	D0,-(SP)
	PEA	lbL02120C
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#0,D0
	MOVE.L	D0,-(SP)
	PEA	lbL02120C
	MOVE.L	D0,-(SP)
	PEA	audiodevice_MSG
	JSR	_OpenDevice
	
	LEA	$10(SP),SP
	MOVE.L	lbL020FA8,lbL02121A
	MOVEQ	#14,D0
	MOVE.L	D0,-(SP)
	PEA	lbL0214C8
	JSR	_Zero
	
	ADDQ.L	#8,SP
	PEA	lbL0214C8
	JSR	_NewList
	
	ADDQ.L	#4,SP
	MOVE.L	#lbL021250,-10(A6)
	MOVE.B	#1,-5(A6)
	CLR.L	-4(A6)
	
AD_loop	MOVE.L	-4(A6),D0
	CMPI.L	#4,D0
	BCC	lbC0200CC
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	MOVE.L	A0,0(SP)
	ADDA.L	#lbL021138,A0
	MOVE.L	-4(A6),D0
	MOVEQ	#$24,D1
	JSR	_CXM33
	
	MOVEA.L	D0,A1
	ADDA.L	#lbL021018,A1
	MOVE.L	A1,(A0)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL021138,A0
	MOVEA.L	(A0),A1
	MOVE.L	#lbW021158,$16(A1)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL021138,A0
	MOVE.L	(A0),-(SP)
	PEA	lbL0214C8
	JSR	_AddHead
	
	ADDQ.L	#8,SP
	MOVE.L	-4(A6),D0
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	MOVE.L	A0,0(SP)
	ADDA.L	#lbL021148,A0
	MOVE.L	-4(A6),D0
	MOVEQ	#$24,D1
	JSR	_CXM33
	
	MOVEA.L	D0,A1
	ADDA.L	#lbL0210A8,A1
	MOVE.L	A1,(A0)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL021148,A0
	MOVEA.L	(A0),A1
	MOVE.L	#lbW021158,$16(A1)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL021148,A0
	MOVE.L	(A0),-(SP)
	PEA	lbL0214C8
	JSR	_AddHead
	
	ADDQ.L	#8,SP
	MOVE.L	-4(A6),D0
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021470,A0
	MOVEA.L	-10(A6),A1
	MOVE.L	A1,(A0)
	MOVEQ	#$44,D0
	MOVE.L	D0,-(SP)
	MOVE.L	A1,-(SP)
	PEA	lbL02120C
	JSR	_Copy
	
	LEA	12(SP),SP
	MOVEA.L	-10(A6),A0
	MOVE.W	#$20,$1C(A0)
	MOVE.B	#$7F,9(A0)
	MOVE.B	#$41,$1E(A0)
	LEA	-5(A6),A1
	MOVE.L	A1,$22(A0)
	MOVEQ	#1,D0
	MOVE.L	D0,$26(A0)
	MOVE.L	A0,-(SP)
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
	ADDI.L	#$44,-10(A6)
	MOVEQ	#0,D0
	MOVE.B	-5(A6),D0
	ASL.L	#1,D0
	MOVE.B	D0,-5(A6)
	ADDQ.L	#1,-4(A6)
	BRA	AD_loop

; XXX compared to Skyfox below code is additional...
	
lbC0200CC	MOVE.L	#lbL021250,-10(A6)
	MOVE.L	#lbL021360,-14(A6)
	MOVEQ	#4,D0
	MOVE.L	D0,-4(A6)
	
added_loop	MOVE.L	-4(A6),D0
	CMPI.L	#8,D0
	BCC.S	lbC020128
	ASL.L	#2,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021470,A0
	MOVEA.L	-14(A6),A1
	MOVE.L	A1,(A0)
	MOVEQ	#$44,D0
	MOVE.L	D0,-(SP)
	MOVE.L	A1,-(SP)
	MOVE.L	-10(A6),-(SP)
	JSR	_Copy
	
	LEA	12(SP),SP
	ADDI.L	#$44,-10(A6)
	ADDI.L	#$44,-14(A6)
	ADDQ.L	#1,-4(A6)
	BRA.S	added_loop

lbC020128	UNLK	A6
	RTS

; XXX almost identical to SkyFox impl
lbC02012C	LINK.W	A6,#-2
	MOVEM.L	D7,-(SP)
	MOVE.B	11(A6),D7
	MOVE.L	D7,D0
	EXT.W	D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FEC,A0
	MOVE.W	8(A6),D0
	MOVE.W	(A0),D1
	CMP.W	D1,D0
	BEQ.S	lbC020154
	MOVEQ	#-1,D7
lbC020154	MOVE.L	D7,D0
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

; XXX identical to SkyFox impl
lbC02015E	LINK.W	A6,#-$52
	MOVEM.L	D7,-(SP)
	MOVE.B	11(A6),D0
	EXT.W	D0
	EXT.L	D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021010,A0
	CLR.B	(A0)
	MOVEA.L	D0,A0
	ADDA.L	#lbL02100C,A0
	TST.B	(A0)
	BEQ.S	lbC02018C
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC02018C	MOVE.L	#lbL021250,-$4E(A6)
	CLR.L	-6(A6)
	MOVE.B	11(A6),D0
	EXT.W	D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021004,A0
	MOVE.W	(A0),D7
	
; XXX identical to SkyFox impl
loop_lbC0201AC	MOVE.L	D7,D0
	BTST	#0,D0
	BEQ.S	lbC0201F0
	MOVEQ	#$44,D0
	MOVE.L	D0,-(SP)
	PEA	-$4A(A6)
	MOVE.L	-$4E(A6),-(SP)
	JSR	_Copy
	
	LEA	12(SP),SP
	MOVE.W	#8,-$2E(A6)
	MOVE.B	#1,-$2C(A6)
	PEA	-$4A(A6)
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
	MOVEA.L	-6(A6),A0
	ADDA.L	#lbL020FD8,A0
	MOVE.B	#$FF,(A0)
	
;XXX almost identical to SkyFox
lbC0201F0	ADDI.L	#$44,-$4E(A6)
	ADDQ.L	#1,-6(A6)
	MOVE.L	D7,D0
	ASR.W	#1,D7
	BNE.S	loop_lbC0201AC
	
	
	MOVE.B	11(A6),D0
	EXT.W	D0
	EXT.L	D0
	MOVE.L	D0,4(SP)
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021004,A0
	CLR.W	(A0)
	MOVEA.L	4(SP),A0
	ADDA.L	#lbL02100C,A0
	MOVE.B	#1,(A0)
	MOVE.L	4(SP),D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FEC,A0
	MOVE.W	(A0),D0
	ADDQ.W	#1,D0
	MOVE.W	D0,(A0)
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC020244	LINK.W	A6,#-6
	MOVEM.L	D6/D7,-(SP)
	MOVE.L	8(A6),D7
	MOVEQ	#0,D6
lbC020252	MOVE.L	D7,D0
	BTST	#0,D0
	BEQ.S	lbC020266
	MOVEA.L	D6,A0
	ADDA.L	#lbL020FD8,A0
	MOVE.B	15(A6),(A0)
lbC020266	ADDQ.L	#1,D6
	MOVE.L	D7,D0
	ASR.W	#1,D7
	BNE.S	lbC020252
	MOVEM.L	(SP)+,D6/D7
	UNLK	A6
	RTS

;XXX almost identical to SkyFox
lbC020276	LINK.W	A6,#-12
	MOVEM.L	D2/D5-D7,-(SP)
lbC02027E	MOVEQ	#0,D5
lbC020280	MOVE.W	#1,-12(A6)
	MOVEQ	#0,D6
	MOVE.L	D5,D0
	MOVE.L	D5,D1
	ASL.L	#1,D1
	MOVEA.L	$10(A6),A0
	ADDA.L	D1,A0
	MOVE.W	(A0),D7
lbC020296	MOVE.L	D7,D0
	BTST	#0,D0
	BEQ.S	lbC0202B4
	MOVEA.L	D6,A0
	ADDA.L	#lbL020FD8,A0
	MOVE.B	(A0),D0
	CMPI.B	#$FF,D0
	BEQ.S	lbC0202B4
	CLR.W	-12(A6)
	BRA.S	lbC0202BC

;XXX almost identical to SkyFox
lbC0202B4	ADDQ.L	#1,D6
	MOVE.L	D7,D0
	ASR.W	#1,D7
	BNE.S	lbC020296
lbC0202BC	TST.W	-12(A6)
	BEQ.S	lbC0202E8
	MOVE.L	D5,D0
	MOVE.L	D5,D1
	ASL.L	#1,D1
	MOVEA.L	$10(A6),A0
	ADDA.L	D1,A0
	MOVE.W	(A0),D1
	EXT.L	D1
	MOVEQ	#4,D2
	MOVE.L	D2,-(SP)
	MOVE.L	D1,-(SP)
	BSR	lbC020244
	ADDQ.L	#8,SP
	MOVE.L	D5,D0
	MOVEM.L	(SP)+,D2/D5-D7
	UNLK	A6
	RTS

;XXX identical to SkyFox
lbC0202E8	ADDQ.L	#1,D5
	MOVE.L	D5,D0
	MOVE.B	11(A6),D1
	EXT.W	D1
	EXT.L	D1
	CMP.L	D1,D0
	BCS.S	lbC020280
	MOVEQ	#0,D5
lbC0202FA	MOVE.L	D5,D0
	MOVE.L	D5,D1
	ASL.L	#1,D1
	MOVEA.L	$10(A6),A0
	ADDA.L	D1,A0
	MOVE.W	(A0),D7
	MOVEQ	#0,D6
lbC02030A	CMPI.L	#4,D6
	BCC.S	lbC02035C
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021004,A0
	MOVE.W	(A0),D1
	CMP.W	D7,D1
	BNE.S	lbC020358
	MOVEA.L	D0,A0
	ADDA.L	#lbL021014,A0
	MOVE.B	(A0),D1
	CMP.B	15(A6),D1
	BGE.S	lbC020358
	MOVE.L	D0,-(SP)
	BSR	lbC02015E
	ADDQ.L	#4,SP
	MOVE.L	D7,D0
	EXT.L	D0
	MOVEQ	#4,D1
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	BSR	lbC020244
	ADDQ.L	#8,SP
	MOVE.L	D5,D0
	MOVEM.L	(SP)+,D2/D5-D7
	UNLK	A6
	RTS

lbC020358	ADDQ.L	#1,D6
	BRA.S	lbC02030A

lbC02035C	ADDQ.L	#1,D5
	MOVE.L	D5,D0
	MOVE.B	11(A6),D1
	EXT.W	D1
	EXT.L	D1
	CMP.L	D1,D0
	BCS.S	lbC0202FA
	MOVEQ	#0,D5
lbC02036E	MOVE.L	D5,D0
	MOVE.L	D5,D1
	ASL.L	#1,D1
	MOVEA.L	$10(A6),A0
	ADDA.L	D1,A0
	MOVE.W	(A0),D7
	MOVEQ	#0,D6
lbC02037E	CMPI.L	#4,D6
	BCC.S	lbC0203BC
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021004,A0
	MOVE.W	(A0),D1
	AND.W	D7,D1
	TST.W	D1
	BEQ.S	lbC0203B8
	MOVEA.L	D0,A0
	ADDA.L	#lbL021014,A0
	MOVE.B	(A0),D1
	CMP.B	15(A6),D1
	BGE.S	lbC0203B8
	MOVE.L	D0,-(SP)
	BSR	lbC02015E
	ADDQ.L	#4,SP
	BRA	lbC02027E

lbC0203B8	ADDQ.L	#1,D6
	BRA.S	lbC02037E

lbC0203BC	ADDQ.L	#1,D5
	MOVE.L	D5,D0
	MOVE.B	11(A6),D1
	EXT.W	D1
	EXT.L	D1
	CMP.L	D1,D0
	BCS.S	lbC02036E
	MOVEQ	#-1,D0
	MOVEM.L	(SP)+,D2/D5-D7
	UNLK	A6
	RTS

lbC0203D6	LINK.W	A6,#-12
	MOVE.W	14(A6),D0
	EXT.L	D0
	CMPI.L	#$FFFFFFFE,D0
	BEQ.S	lbC020400
	CMPI.L	#$FFFFFFFF,D0
	BNE.S	lbC020410
	MOVEQ	#2,D0
	MOVE.L	D0,-4(A6)
	MOVE.L	#lbW020FE0,-12(A6)
	BRA.S	lbC02041E

lbC020400	MOVEQ	#2,D0
	MOVE.L	D0,-4(A6)
	MOVE.L	#lbW020FDC,-12(A6)
	BRA.S	lbC02041E

lbC020410	MOVEQ	#4,D0
	MOVE.L	D0,-4(A6)
	MOVE.L	#lbW020FE4,-12(A6)
lbC02041E	MOVE.W	10(A6),D0
	EXT.L	D0
	MOVE.L	-12(A6),-(SP)
	MOVE.L	D0,-(SP)
	MOVE.L	-4(A6),-(SP)
	BSR	lbC020276
	LEA	12(SP),SP
	EXT.L	D0
	MOVE.L	D0,-8(A6)
	ADDQ.L	#1,D0
	BNE.S	lbC020446
	MOVEQ	#0,D0
	UNLK	A6
	RTS

lbC020446	MOVE.L	-8(A6),D0
	ASL.L	#1,D0
	MOVEA.L	-12(A6),A0
	ADDA.L	D0,A0
	MOVE.W	(A0),D0
	UNLK	A6
	RTS

lbC020458	LINK.W	A6,#-10
	MOVEM.L	D7,-(SP)
	MOVEQ	#0,D7
lbC020462	MOVE.L	D7,D0
	EXT.W	D0
	EXT.L	D0
	MOVEA.L	D0,A0
	MOVE.L	A0,4(SP)
	ADDA.L	#lbL02100C,A0
	TST.B	(A0)
	BEQ.S	lbC0204D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL02100C,A0
	CLR.B	(A0)
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FEC,A0
	MOVE.W	(A0),D0
	ADDQ.W	#1,D0
	MOVE.W	D0,(A0)
	MOVE.B	#$53,-4(A6)
	MOVE.L	4(SP),D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FEC,A0
	MOVE.L	4(SP),D0
	ASL.L	#1,D0
	MOVEA.L	D0,A1
	ADDA.L	#lbL020FEC,A1
	MOVE.W	(A1),D0
	ANDI.W	#$7FFF,D0
	MOVE.W	D0,(A0)
	MOVE.W	D0,-6(A6)
	MOVE.B	D7,-3(A6)
	MOVE.L	-6(A6),D0
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC0204D0	ADDQ.B	#1,D7
	CMPI.B	#4,D7
	BLT.S	lbC020462
	MOVEQ	#-1,D0
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC0204E2	LINK.W	A6,#-14
	MOVEM.L	D4-D7,-(SP)
	MOVE.B	11(A6),D0
	EXT.W	D0
	EXT.L	D0
	MOVEA.L	D0,A0
	MOVE.L	A0,$10(SP)
	ADDA.L	#lbL021164,A0
	TST.B	(A0)
	BEQ.S	lbC020572
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FF4,A0
	MOVE.W	(A0),D5
	MOVE.L	$10(SP),D0
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021004,A0
	MOVE.W	(A0),D7
	MOVEQ	#0,D6
lbC020520	MOVE.L	D7,D0
	BTST	#0,D0
	BEQ.S	lbC02056A
	MOVE.L	D5,D0
	MULU.W	#$CD,D0
	LSR.W	#8,D0
	MOVE.L	D0,D4
	CMPI.L	#1,D6
	BEQ.S	lbC020542
	CMPI.L	#2,D6
	BNE.S	lbC020554
lbC020542	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FBC,A0
	MOVE.W	D4,(A0)
	BRA.S	lbC02056A

lbC020554	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FBC,A0
	MOVE.W	#$100,D1
	SUB.W	D4,D1
	MOVE.W	D1,(A0)
lbC02056A	ADDQ.L	#1,D6
	MOVE.L	D7,D0
	ASR.W	#1,D7
	BNE.S	lbC020520
lbC020572	MOVEM.L	(SP)+,D4-D7
	UNLK	A6
	RTS

lbC02057A	LINK.W	A6,#-$14
	MOVEM.L	D7/A3-A5,-(SP)
	MOVE.L	8(A6),D7
lbC020586	MOVEA.L	D7,A0
	ADDA.L	#lbL020FD4,A0
	TST.B	(A0)
	BEQ.S	lbC02059A
	MOVEM.L	(SP)+,D7/A3-A5
	UNLK	A6
	RTS

lbC02059A	MOVE.L	D7,D0
	MULS.W	#14,D0
	MOVEA.L	D0,A0
	ADDA.L	#SfxStack,A0
	MOVEA.L	A0,A4
	MOVE.W	4(A4),D0
	TST.W	D0
	BMI.S	lbC0205B8
	SUBQ.W	#1,D0
	MOVE.W	D0,4(A4)
lbC0205B8	MOVE.W	4(A4),D0
	ADDQ.W	#1,D0
	BNE.S	lbC0205E6
	TST.L	(A4)
	BEQ.S	lbC0205D2
	MOVEA.L	(A4),A0
	MOVEA.L	A4,A1
	MOVEQ	#13,D0
lbC0205CA	MOVE.B	(A0)+,(A1)+
	DBRA	D0,lbC0205CA
	BRA.S	lbC020586

lbC0205D2	MOVEA.L	D7,A0
	ADDA.L	#lbL020FD4,A0
	MOVE.B	#1,(A0)
	MOVEM.L	(SP)+,D7/A3-A5
	UNLK	A6
	RTS

lbC0205E6	TST.L	6(A4)
	BEQ.S	lbC0205EE
	BRA.S	lbC020586

lbC0205EE	MOVEA.L	10(A4),A5
	MOVEA.L	12(A6),A3
	TST.L	$10(A6)
	BEQ.S	lbC020622
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FB4,A0
	MOVE.W	6(A5),(A0)
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FAC,A0
	MOVE.W	4(A5),(A0)
	MOVE.B	#$10,$1E(A3)
	BRA.S	lbC020626

lbC020622	CLR.B	$1E(A3)
lbC020626	MOVE.L	8(A5),$22(A3)
	MOVE.L	(A5),D0
	ASL.L	#1,D0
	MOVE.L	D0,$26(A3)
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FCC,A0
	MOVE.L	D0,-(SP)
	MOVE.L	A0,$14(SP)
	JSR	lbC021708
	ADDQ.L	#4,SP
	MOVEA.L	$10(SP),A0
	MOVE.W	D0,(A0)
	MOVE.W	D0,$2A(A3)
	MOVE.L	D7,D0
	MOVE.L	D7,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FC4,A0
	MOVE.L	D0,-(SP)
	MOVE.L	A0,$14(SP)
	JSR	lbC021850
	ADDQ.L	#4,SP
	MOVEA.L	$10(SP),A0
	MOVE.W	D0,(A0)
	MOVE.W	D0,$2C(A3)
	MOVE.W	4(A4),D0
	ADDQ.W	#2,D0
	BNE.S	lbC02068E
	CLR.W	$2E(A3)
	BRA.S	lbC02069C

lbC02068E	MOVE.W	4(A4),D0
	ADDQ.W	#1,D0
	MOVE.W	D0,4(A4)
	MOVE.W	D0,$2E(A3)
lbC02069C	CLR.W	4(A4)
	MOVE.W	#3,$1C(A3)
	MOVE.L	A3,-(SP)
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
	MOVEM.L	(SP)+,D7/A3-A5
	UNLK	A6
	RTS

lbC0206B8	LINK.W	A6,#-6
	MOVEM.L	D7,-(SP)
	MOVEA.L	8(A6),A0
	MOVE.L	$14(A0),-(SP)
	BSR	lbC02012C
	ADDQ.L	#4,SP
	MOVE.L	D0,D7
	TST.B	D7
	BPL.S	lbC0206DC
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC0206DC	MOVE.L	D7,D0
	EXT.W	D0
	EXT.L	D0
	MOVE.L	D0,4(SP)
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FF4,A0
	MOVEA.L	8(A6),A1
	MOVE.W	$18(A1),(A0)
	MOVE.L	4(SP),-(SP)
	BSR	lbC0204E2
	ADDQ.L	#4,SP
	MOVEM.L	(SP)+,D7
	UNLK	A6
	RTS

lbC02070A	LINK.W	A6,#-$32
	PEA	-$32(A6)
	JSR	_NewList
	
	ADDQ.L	#4,SP
	MOVEQ	#$24,D0
	MOVE.L	D0,-(SP)
	PEA	-$24(A6)
	JSR	_Zero
	
	ADDQ.L	#8,SP
	TST.L	12(A6)
	BNE.S	lbC020738
	MOVE.L	#lbW021158,12(A6)
lbC020738	MOVE.L	12(A6),-14(A6)
	PEA	-$24(A6)
	PEA	-$32(A6)
	JSR	_AddHead
	
	ADDQ.L	#8,SP
	PEA	-$32(A6)
	JSR	_TickAudEG
	ADDQ.L	#4,SP
	MOVEA.L	8(A6),A0
	MOVE.L	-14(A6),$16(A0)
	MOVE.L	-$16(A6),14(A0)
	UNLK	A6
	RTS

lbC02076E	LINK.W	A6,#-14
	MOVEM.L	D5-D7/A2,-(SP)
	MOVEQ	#-1,D0
	MOVEA.L	8(A6),A0
	MOVE.L	D0,$14(A0)
	BSR	lbC020458
	MOVEA.L	8(A6),A0
	MOVE.L	D0,$14(A0)
	ADDQ.L	#1,D0
	BNE.S	lbC020798
	MOVEM.L	(SP)+,D5-D7/A2
	UNLK	A6
	RTS

lbC020798	MOVEA.L	8(A6),A0
	MOVE.L	$14(A0),-(SP)
	BSR	lbC02012C
	ADDQ.L	#4,SP
	MOVE.L	D0,D5
	EXT.W	D5
	EXT.L	D5
	MOVEA.L	D5,A0
	ADDA.L	#lbL021014,A0
	MOVEA.L	8(A6),A1
	MOVE.B	$1B(A1),(A0)
	MOVE.L	D5,D0
	MOVE.L	D5,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FF4,A0
	MOVE.W	$18(A1),(A0)
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FFC,A0
	MOVE.W	$1E(A1),(A0)
	MOVEA.L	D1,A0
	ADDA.L	#lbL021004,A0
	MOVE.W	$1C(A1),D1
	MOVE.W	D1,(A0)
	MOVE.L	D1,D7
	MOVE.L	D7,D1
	EXT.L	D1
	MOVE.L	D0,-(SP)
	MOVE.L	D1,-(SP)
	BSR	lbC020244
	ADDQ.L	#8,SP
	MOVEA.L	D5,A0
	ADDA.L	#lbL021164,A0
	MOVEA.L	8(A6),A1
	MOVE.W	$18(A1),D0
	MOVE.L	A0,$10(SP)
	TST.W	D0
	BMI.S	lbC02081A
	CMPI.W	#$140,D0
	BGE.S	lbC02081A
	MOVEQ	#1,D0
	BRA.S	lbC02081C

lbC02081A	MOVEQ	#0,D0
lbC02081C	MOVEA.L	$10(SP),A0
	MOVE.B	D0,(A0)
	TST.B	D0
	BEQ.S	lbC02082E
	MOVE.L	D5,-(SP)
	BSR	lbC0204E2
	ADDQ.L	#4,SP
lbC02082E	MOVEQ	#0,D6
lbC020830	MOVE.L	D7,D0
	BTST	#0,D0
	BEQ	lbC020900
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021138,A0
	MOVEA.L	8(A6),A1
	MOVE.L	$24(A1),-(SP)
	MOVE.L	(A0),-(SP)
	BSR	lbC02070A
	ADDQ.L	#8,SP
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021148,A0
	MOVEA.L	8(A6),A1
	MOVE.L	$28(A1),-(SP)
	MOVE.L	(A0),-(SP)
	BSR	lbC02070A
	ADDQ.L	#8,SP
	MOVEA.L	D5,A0
	ADDA.L	#lbL021164,A0
	TST.B	(A0)
	BNE.S	lbC020894
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#1,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FBC,A0
	MOVE.W	#$100,(A0)
lbC020894	MOVEA.L	D6,A0
	ADDA.L	#lbL020FD4,A0
	CLR.B	(A0)
	MOVE.L	D6,D0
	MULS.W	#14,D0
	MOVEA.L	D0,A0
	ADDA.L	#SfxStack,A0
	MOVEA.L	8(A6),A2
	MOVEA.L	$20(A2),A1
	MOVE.L	A0,$10(SP)
	MOVEA.L	A1,A0
	MOVEA.L	$10(SP),A1
	MOVEQ	#13,D0
lbC0208C0	MOVE.B	(A0)+,(A1)+
	DBRA	D0,lbC0208C0
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021470,A0
	MOVEQ	#1,D1
	MOVE.L	D1,-(SP)
	MOVE.L	(A0),-(SP)
	MOVE.L	D0,-(SP)
	BSR	lbC02057A
	LEA	12(SP),SP
	MOVE.L	D6,D0
	MOVE.L	D6,D1
	ASL.L	#2,D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL021480,A0
	CLR.L	-(SP)
	MOVE.L	(A0),-(SP)
	MOVE.L	D0,-(SP)
	BSR	lbC02057A
	LEA	12(SP),SP
lbC020900	ADDQ.L	#1,D6
	MOVE.L	D7,D0
	ASR.W	#1,D7
	BNE	lbC020830
	MOVEA.L	D5,A0
	ADDA.L	#lbL021010,A0
	MOVE.B	#1,(A0)
	MOVEM.L	(SP)+,D5-D7/A2
	UNLK	A6
	RTS

lbC02091E	LINK.W	A6,#-2
	MOVEA.L	8(A6),A0
	MOVE.L	$14(A0),-(SP)
	BSR	lbC02012C
	ADDQ.L	#4,SP
	MOVE.B	D0,-1(A6)
	TST.B	D0
	BPL.S	lbC02093C
	UNLK	A6
	RTS

lbC02093C	MOVE.B	-1(A6),D0
	EXT.W	D0
	EXT.L	D0
	MOVE.L	D0,-(SP)
	BSR	lbC02015E
	ADDQ.L	#4,SP
	UNLK	A6
	RTS

; XXX below almost identical to SkyFox
lbC020950	LINK.W	A6,#-4
	MOVE.W	#1,lbW0211E4
	CLR.L	-4(A6)
lbC020960	MOVE.L	-4(A6),D0
	CMPI.L	#4,D0
	BGE.S	lbC020986
	MOVEA.L	D0,A0
	ADDA.L	#lbL021010,A0
	TST.B	(A0)
	BEQ.S	lbC020980
	MOVE.L	D0,-(SP)
	BSR	lbC02015E
	ADDQ.L	#4,SP
lbC020980	ADDQ.L	#1,-4(A6)
	BRA.S	lbC020960

lbC020986	UNLK	A6
	RTS

lbC02098A	LINK.W	A6,#0
	MOVEA.L	8(A6),A0
	MOVE.B	$1A(A0),D0
	EXT.W	D0
	EXT.L	D0
	SUBI.L	#1,D0
	BLT.S	lbC020A12
	CMPI.L	#6,D0
	BGE.S	lbC020A12
	ASL.L	#1,D0
	JMP	lbC0209B0(PC,D0.L)

lbC0209B0	BRA.S	lbC0209BC

	BRA.S	lbC0209C8

	BRA.S	lbC0209D4

	BRA.S	lbC020A12

	BRA.S	lbC0209E0

	BRA.S	lbC0209EC

lbC0209BC	MOVE.L	8(A6),-(SP)
	BSR	lbC02076E
	ADDQ.L	#4,SP
	BRA.S	lbC020A12

lbC0209C8	MOVE.L	8(A6),-(SP)
	BSR	lbC02091E
	ADDQ.L	#4,SP
	BRA.S	lbC020A12

lbC0209D4	MOVE.L	8(A6),-(SP)
	BSR	lbC0206B8
	ADDQ.L	#4,SP
	BRA.S	lbC020A12

lbC0209E0	MOVE.L	8(A6),-(SP)
	BSR	lbC020950
	ADDQ.L	#4,SP
	BRA.S	lbC020A12

lbC0209EC	MOVEA.L	8(A6),A0
	MOVE.B	$1B(A0),D0
	EXT.W	D0
	EXT.L	D0
	MOVE.W	$18(A0),D1
	EXT.L	D1
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	BSR	lbC0203D6
	ADDQ.L	#8,SP
	MOVEA.L	8(A6),A0
	MOVE.W	D0,$1C(A0)
	NOP
lbC020A12	MOVEA.L	8(A6),A0
	TST.L	14(A0)
	BEQ.S	lbC020A28
	MOVE.L	A0,-(SP)
	JSR	_ReplyMsg
	ADDQ.L	#4,SP
	BRA.S	lbC020A38

lbC020A28	MOVEQ	#$34,D0
	MOVE.L	D0,-(SP)
	MOVE.L	8(A6),-(SP)
	JSR	_FreeMem
	ADDQ.L	#8,SP
lbC020A38	UNLK	A6
	RTS

***************************************************************************************************************
	
runTaskSFX_lbC020A3C	LINK.W	A6,#-$2C
	MOVEM.L	D6/D7/A5,-(SP)
	PEA	-$2C(A6)
	BSR	MakePort
	
	ADDQ.L	#4,SP
	LEA	-$2C(A6),A0
	MOVE.L	A0,lbL020FA8
	BSR	OpenAD_etc
	
	MOVEQ	#$28,D0
	MOVE.L	D0,-(SP)
	PEA	timer
	JSR	_Zero
	
	ADDQ.L	#8,SP
	CLR.L	-(SP)
	PEA	timer
	MOVEQ	#1,D0
	MOVE.L	D0,-(SP)
	PEA	timerdevice_MSG
	JSR	_OpenDevice
	
	LEA	$10(SP),SP
	MOVE.L	lbL020FA8,lbL0216EC
	CLR.L	lbL0216FE
	MOVE.L	#$411A,lbL021702
	MOVE.W	#9,lbW0216FA
	PEA	timer
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
loop_lbC020ABA	MOVE.L	lbL020FA8,-(SP)
	JSR	_WaitPort
	
	ADDQ.L	#4,SP
	MOVE.L	lbL020FA8,-(SP)
	JSR	_GetMsg
	
	ADDQ.L	#4,SP
	MOVE.L	D0,D7
	LEA	timer,A0
	CMP.L	A0,D7
	BNE.S	lbC020B22
	CLR.L	lbL0216FE
	MOVE.L	#$411A,lbL021702
	MOVE.W	#9,lbW0216FA
	TST.W	lbW0211E4
	BEQ.S	lbC020B0C
	MOVE.W	#1,lbW0211E6
	BRA.S	loop_lbC020ABA

lbC020B0C	PEA	timer
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
	JSR	sfxtimer_lbC021740
	BRA.S	loop_lbC020ABA

lbC020B22	LEA	lbL021250,A0
	CMP.L	A0,D7
	BCS.S	lbC020B86
	LEA	lbL02142C,A0
	CMP.L	A0,D7
	BHI.S	lbC020B86
	LEA	lbL021250,A5
	MOVEQ	#0,D6
lbC020B3E	MOVE.L	A5,D0
	MOVE.L	A5,D1
	CMP.L	D1,D7
	BNE.S	lbC020B76
	MOVE.L	D6,D1
	EXT.W	D1
	EXT.L	D1
	MOVEA.L	D1,A0
	ADDA.L	#lbL020FD8,A0
	MOVE.B	(A0),D1
	BTST	#2,D1
	BNE	loop_lbC020ABA
	MOVE.L	D6,D0
	EXT.W	D0
	EXT.L	D0
	CLR.L	-(SP)
	MOVE.L	A5,-(SP)
	MOVE.L	D0,-(SP)
	BSR	lbC02057A
	LEA	12(SP),SP
	BRA	loop_lbC020ABA

lbC020B76	ADDA.W	#$44,A5
	ADDQ.B	#1,D6
	MOVE.L	D6,D0
	ANDI.B	#3,D0
	MOVE.L	D0,D6
	BRA.S	lbC020B3E

lbC020B86	MOVEA.L	D7,A0
	MOVE.L	A0,-(SP)
	BSR	lbC02098A
	ADDQ.L	#4,SP
	BRA	loop_lbC020ABA

	
	
InitSfx	LINK.W	A6,#-6
	MOVE.W	#1,_SfxOnOff
	MOVEQ	#0,D0
	MOVE.L	#$208,-(SP)
	PEA	lbL0214D6
	MOVE.W	D0,lbW0211E4
	MOVE.W	D0,lbW0211E6
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#$5C,D0
	MOVE.L	D0,-(SP)
	PEA	lbL021170
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#$38,D0
	MOVE.L	D0,-(SP)
	PEA	SfxStack
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#8,D0
	MOVE.L	D0,-(SP)
	PEA	lbL021004
	JSR	_Zero
	ADDQ.L	#8,SP
	MOVEQ	#4,D0
	MOVE.L	D0,-(SP)
	PEA	lbL021010
	JSR	_Zero
	ADDQ.L	#8,SP
	CLR.B	-1(A6)
lbC020C0E	MOVE.B	-1(A6),D0
	CMPI.B	#4,D0
	BGE.S	lbC020C50
	
	EXT.W	D0
	EXT.L	D0
	MOVE.L	D0,0(SP)
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL020FEC,A0
	MOVE.W	#1,(A0)
	
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL02100C,A0
	MOVE.B	#1,(A0)
	MOVEA.L	0(SP),A0
	ADDA.L	#lbL020FD8,A0
	MOVE.B	#$FF,(A0)
	ADDQ.B	#1,-1(A6)
	BRA.S	lbC020C0E

lbC020C50	MOVE.L	#lbL0214D6,lbL0211AA
	LEA	lbW0216DC,A0
	MOVE.L	A0,lbL0211AE
	MOVE.L	A0,lbL0211A6
	MOVE.B	#1,lbB021178
	MOVE.L	lbL020FA4,D0
	MOVE.B	D0,lbB021179		; XXX in SkyFox this is just directly hardcoded to 2
	
	MOVE.L	#SfxTask_MSG,lbL02117A
	LEA	lbL021170,A0
	CLR.L	-(SP)
	PEA	runTaskSFX_lbC020A3C(PC)
	MOVE.L	A0,-(SP)
	MOVE.L	A0,lbL02116C
	
;	XXX _AddTask pulls its A1-A3 input from the stack..
;  result = AddTask(task, initialPC, finalPC)
;  D0               A1    A2         A3
	JSR	_AddTask
	LEA	12(SP),SP
	UNLK	A6
	RTS

	
ResetSfx	LINK.W	A6,#-$5A
	MOVE.W	#1,lbW0211E4
	LEA	-$34(A6),A0
	PEA	-$5A(A6)
	MOVE.L	A0,-$38(A6)
	BSR	MakePort
	ADDQ.L	#4,SP
	MOVEQ	#$34,D0
	MOVE.L	D0,-(SP)
	MOVE.L	-$38(A6),-(SP)
	JSR	_Zero
	ADDQ.L	#8,SP
	LEA	-$5A(A6),A0
	MOVEA.L	-$38(A6),A1
	MOVE.L	A0,14(A1)
	MOVEQ	#-1,D0
	MOVE.L	D0,$14(A1)
	MOVE.B	#5,$1A(A1)
	MOVE.L	A1,-(SP)
	MOVE.L	lbL020FA8,-(SP)
	JSR	_PutMsg
	ADDQ.L	#8,SP
	PEA	-$5A(A6)
	JSR	_WaitPort
	ADDQ.L	#4,SP
	MOVEQ	#15,D0
	MOVE.L	D0,lbL021224
	PEA	lbL02120C
	JSR	_CloseDevice
	ADDQ.L	#4,SP
lbC020D24	TST.W	lbW0211E6
	BEQ.S	lbC020D24
	PEA	timer
	JSR	_CloseDevice
	ADDQ.L	#4,SP
	MOVEA.L	-$38(A6),A0
	CLR.B	$1A(A0)
	MOVE.L	A0,-(SP)
	MOVE.L	lbL020FA8,-(SP)
	JSR	_PutMsg
	ADDQ.L	#8,SP
	PEA	-$5A(A6)
	JSR	_WaitPort
	ADDQ.L	#4,SP
	PEA	-$5A(A6)
	JSR	_GetMsg
	ADDQ.L	#4,SP
	PEA	-$5A(A6)
	MOVE.L	D0,-$38(A6)
	BSR	UnmakePort
	ADDQ.L	#4,SP
	MOVE.L	lbL020FA8,-(SP)
	BSR	UnmakePort
	ADDQ.L	#4,SP
	PEA	lbL021170
	JSR	_RemTask
	ADDQ.L	#4,SP
	UNLK	A6
	RTS

StartSfx	LINK.W	A6,#-$5E
	TST.W	_SfxOnOff
	BNE.S	StartSfx_sub1
	MOVEQ	#-1,D0
	UNLK	A6
	RTS

StartSfx_sub1	LEA	-$38(A6),A0
	PEA	-$5E(A6)
	MOVE.L	A0,-$3C(A6)
	BSR	MakePort
	ADDQ.L	#4,SP
	MOVEQ	#$14,D0
	MOVE.L	D0,-(SP)
	MOVE.L	-$3C(A6),-(SP)
	JSR	_Zero
	ADDQ.L	#8,SP
	LEA	-$5E(A6),A0
	MOVEA.L	-$3C(A6),A1
	MOVE.L	A0,14(A1)
	MOVEQ	#-1,D0
	MOVE.L	D0,-4(A6)
	MOVE.B	#6,$1A(A1)
	MOVE.B	$1B(A6),$1B(A1)
	MOVE.W	$1E(A6),$18(A1)
	MOVE.L	A1,-(SP)
	MOVE.L	lbL020FA8,-(SP)
	JSR	_PutMsg
	
	ADDQ.L	#8,SP
	PEA	-$5E(A6)
	JSR	_WaitPort
	
	ADDQ.L	#4,SP
	PEA	-$5E(A6)
	JSR	_GetMsg
	
	ADDQ.L	#4,SP
	MOVEA.L	D0,A0
	MOVE.W	$1C(A0),D1
	MOVE.L	D0,-$3C(A6)
	TST.W	D1
	BEQ.S	lbC020E7A
	MOVEA.L	-$3C(A6),A0
	MOVE.L	8(A6),$20(A0)
	MOVE.L	12(A6),$24(A0)
	MOVE.L	$10(A6),$28(A0)
	MOVE.W	$16(A6),$1E(A0)
	MOVE.B	#1,$1A(A0)
	MOVE.L	A0,-(SP)
	MOVE.L	lbL020FA8,-(SP)
	JSR	_PutMsg
	
	ADDQ.L	#8,SP
	PEA	-$5E(A6)
	JSR	_WaitPort
	
	ADDQ.L	#4,SP
	PEA	-$5E(A6)
	JSR	_GetMsg
	
	ADDQ.L	#4,SP
	MOVEA.L	D0,A0
	MOVE.L	$14(A0),-4(A6)
	MOVE.L	D0,-$3C(A6)
lbC020E7A	PEA	-$5E(A6)
	BSR	UnmakePort
	
	ADDQ.L	#4,SP
	MOVE.L	-4(A6),D0
	UNLK	A6
	RTS

;SfxPosition	LINK.W	A6,#-4
;	MOVE.L	#$10001,-(SP)
;	MOVEQ	#$34,D0
;	MOVE.L	D0,-(SP)
;	JSR	_AllocMem
;	ADDQ.L	#8,SP
;	MOVEA.L	D0,A0
;	MOVE.L	8(A6),$14(A0)
;	MOVE.W	14(A6),$18(A0)
;	MOVE.B	#3,$1A(A0)
;	MOVE.L	D0,-(SP)
;	MOVE.L	lbL020FA8,-(SP)
;	MOVE.L	D0,-4(A6)
;	JSR	_PutMsg
;	ADDQ.L	#8,SP
;	UNLK	A6
;	RTS

StopSfx	LINK.W	A6,#-4
	MOVE.L	8(A6),-(SP)
	BSR	lbC02012C
	ADDQ.L	#4,SP
	TST.B	D0
	BPL.S	StopSfx_sub1
	UNLK	A6
	RTS

StopSfx_sub1	MOVE.L	#$10001,-(SP)
	MOVEQ	#$34,D0
	MOVE.L	D0,-(SP)
	JSR	_AllocMem
	
	ADDQ.L	#8,SP
	MOVEA.L	D0,A0
	MOVE.L	8(A6),$14(A0)
	MOVE.B	#2,$1A(A0)
	MOVE.L	D0,-(SP)
	MOVE.L	lbL020FA8,-(SP)
	MOVE.L	D0,-4(A6)
	JSR	_PutMsg
	
	ADDQ.L	#8,SP
	UNLK	A6
	RTS

;SfxActive	LINK.W	A6,#-6
;	MOVEM.L	D7,-(SP)
;	CLR.W	-6(A6)
;	MOVE.L	8(A6),-(SP)
;	BSR	lbC02012C
;	ADDQ.L	#4,SP
;	MOVE.B	D0,-3(A6)
;	TST.B	D0
;	BPL.S	lbC020F44
;	MOVEQ	#0,D0
;	MOVEM.L	(SP)+,D7
;	UNLK	A6
;	RTS

;lbC020F44	MOVE.B	-3(A6),D0
;	EXT.W	D0
;	EXT.L	D0
;	ASL.L	#1,D0
;	MOVEA.L	D0,A0
;	ADDA.L	#lbL021004,A0
;	MOVE.W	(A0),D7
;lbC020F58	MOVE.L	D7,D0
;	BTST	#0,D0
;	BEQ.S	lbC020F7C
;	MOVE.W	-6(A6),D0
;	EXT.L	D0
;	MOVEA.L	D0,A0
;	ADDA.L	#lbL021010,A0
;	TST.B	(A0)
;	BEQ.S	lbC020F7C
;	MOVEQ	#1,D0
;	MOVEM.L	(SP)+,D7
;	UNLK	A6
;	RTS

;lbC020F7C	ADDQ.W	#1,-6(A6)
;	MOVE.L	D7,D0
;	ASR.W	#1,D7
;	BNE.S	lbC020F58
;	MOVEQ	#0,D0
;	MOVEM.L	(SP)+,D7
;	UNLK	A6
;	RTS

;SfxOff	CLR.W	_SfxOnOff
;	RTS

;SfxOn	MOVE.W	#1,_SfxOnOff
;	RTS

;	dc.w	0

;	SECTION	MarbleMadness020FA4,DATA
lbL020FA4	dc.l	7
lbL020FA8	dc.l	0
lbL020FAC	dc.l	0
	dc.l	0
lbL020FB4	dc.l	0
	dc.l	0
lbL020FBC	dc.l	0
	dc.l	0
lbL020FC4	dc.l	0
	dc.l	0
lbL020FCC	dc.l	0
	dc.l	0
lbL020FD4	dc.l	0
lbL020FD8	dc.l	0
lbW020FDC	dc.w	2
	dc.w	4
lbW020FE0	dc.w	1
	dc.w	8
lbW020FE4	dc.w	3
	dc.w	5
	dc.w	10
	dc.w	12
lbL020FEC	dc.l	0
	dc.l	0
lbL020FF4	dc.l	0
	dc.l	0
lbL020FFC	dc.l	0
	dc.l	0
lbL021004	dc.l	0
	dc.l	0
lbL02100C	dc.l	0
lbL021010	dc.l	0
lbL021014	dc.l	0
lbL021018	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL0210A8	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL021138	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL021148	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbW021158	dc.w	1
	dc.w	0
	dc.w	1
	dc.w	0
	dc.w	0
	dc.w	0
lbL021164	dc.l	0
	dc.l	0
lbL02116C	dc.l	0
lbL021170	dc.l	0
	dc.l	0

;	XXX initialized to 1/7 
lbB021178	dc.b	0
lbB021179	dc.b	0

lbL02117A	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
lbL0211A6	dc.l	0
lbL0211AA	dc.l	0
lbL0211AE	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.w	0
lbL0211CC	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	lbL0211CC
lbW0211E4	dc.w	0
lbW0211E6	dc.w	0
_SfxOnOff	dc.w	1
audiodevice_MSG	dc.b	'audio.device',0
timerdevice_MSG	dc.b	'timer.device',0
SfxTask_MSG	dc.b	'SfxTask',0

;	SECTION	MarbleMadness02120C,BSS
lbL02120C	ds.l	3
	ds.w	1
lbL02121A	ds.l	2
	ds.w	1
lbL021224	ds.l	11
lbL021250	ds.l	$44
lbL021360	ds.l	$33
lbL02142C	ds.l	$11
lbL021470	ds.l	4
lbL021480	ds.l	4
SfxStack	ds.l	14
lbL0214C8	ds.l	3
	ds.w	1
lbL0214D6	ds.l	$81
	ds.w	1
lbW0216DC	ds.w	1
timer	ds.l	3
	ds.w	1
lbL0216EC	ds.l	3
	ds.w	1
lbW0216FA	ds.w	2
lbL0216FE	ds.l	1
lbL021702	ds.l	1
	ds.w	1


;	SECTION	MarbleMadness021708,CODE
lbC021708	MOVE.L	4(SP),D0
	ASL.L	#1,D0
	MOVE.L	D0,D1
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021148,A0
	MOVEA.L	(A0),A1
	MOVEA.L	#lbL020FAC,A0
	ADDA.L	D1,A0
	CMPI.L	#$10000,14(A1)
	BNE.S	lbC021734
	MOVEQ	#0,D0
	MOVE.W	(A0),D0
	RTS

lbC021734	MOVE.L	14(A1),D0
	ASR.L	#8,D0
	MULU.W	(A0),D0
	LSR.L	#8,D0
	RTS

sfxtimer_lbC021740	LINK.W	A6,#-$56
	MOVEM.L	D2/D4-D7/A5,-(SP)
	MOVEQ	#0,D7
	PEA	lbL0214C8
	JSR	_TickAudEG
	ADDQ.L	#4,SP
lbC021758	MOVEA.L	#lbL021010,A0
	ADDA.L	D7,A0
	TST.B	(A0)
	BEQ	lbC02183E
	MOVE.L	D7,D0
	ASL.L	#1,D0
	MOVEA.L	#lbL020FFC,A0
	ADDA.L	D0,A0
	TST.W	(A0)
	BNE.S	lbC021784
	MOVE.L	D7,-(SP)
	JSR	lbC02015E
	ADDQ.L	#4,SP
	BRA	lbC02183E

lbC021784	MOVEA.L	#lbL021004,A0
	ADDA.L	D0,A0
	MOVE.W	(A0),D6
	MOVEQ	#0,D5
	MOVEA.L	#lbL021250,A5
lbC021796	BTST	#0,D6
	BEQ	lbC021824
	MOVE.L	D5,-(SP)
	BSR	lbC021708
	MOVE.L	D0,D4
	MOVE.L	D5,-(SP)
	BSR	lbC021850
	ADDQ.L	#8,SP
	MOVE.L	D5,D2
	ASL.L	#1,D2
	MOVEA.L	#lbL020FC4,A0
	ADDA.L	D2,A0
	MOVE.W	D0,-14(A6)
	CMP.W	(A0),D0
	BNE.S	lbC0217CE
	MOVEA.L	#lbL020FCC,A0
	ADDA.L	D2,A0
	CMP.W	(A0),D4
	BEQ.S	lbC021824
lbC0217CE	MOVEQ	#$2C,D0
	MOVE.L	D0,-(SP)
	PEA	-$52(A6)
	MOVE.L	A5,-(SP)
	JSR	_Copy
	
	LEA	12(SP),SP
	MOVE.L	D4,D1
	CMPI.W	#$72,D4
	BCC.S	lbC0217EC
	MOVEQ	#$48,D1
lbC0217EC	MOVE.W	D1,-$28(A6)
	MOVEA.L	#lbL020FCC,A0
	ADDA.L	D2,A0
	MOVE.W	D1,(A0)
	MOVEA.L	#lbL020FC4,A0
	ADDA.L	D2,A0
	MOVE.W	-14(A6),D1
	MOVE.W	D1,-$26(A6)
	MOVE.W	D1,(A0)
	MOVE.W	#12,-$36(A6)
	MOVE.B	#1,-$34(A6)
	PEA	-$52(A6)
	JSR	_BeginIO
	
	ADDQ.L	#4,SP
lbC021824	ADDA.W	#$44,A5
	ADDQ.W	#1,D5
	ASR.W	#1,D6
	BNE	lbC021796
	MOVE.L	D7,D1
	ASL.L	#1,D1
	MOVEA.L	#lbL020FFC,A0
	ADDA.L	D1,A0
	SUBQ.W	#1,(A0)
lbC02183E	ADDQ.W	#1,D7
	CMPI.W	#4,D7
	BLT	lbC021758
	MOVEM.L	(SP)+,D2/D4-D7/A5
	UNLK	A6
	RTS

lbC021850	MOVE.L	D2,-(SP)
	MOVE.W	10(SP),D0
	EXT.L	D0
	ASL.L	#1,D0
	MOVE.L	D0,D2
	ASL.L	#1,D0
	MOVEA.L	D0,A0
	ADDA.L	#lbL021138,A0
	MOVEA.L	(A0),A1
	MOVE.L	14(A1),D0
	LSR.L	#8,D0
	MOVEA.L	#lbL020FBC,A0
	ADDA.L	D2,A0
	MOVE.W	(A0),D1
	CMPI.L	#$100,D0
	BNE.S	lbC02189E
	MOVEA.L	#lbL020FB4,A0
	ADDA.L	D2,A0
	CMPI.W	#$100,D1
	BNE.S	lbC021896
	MOVEQ	#0,D0
	MOVE.W	(A0),D0
	MOVE.L	(SP)+,D2
	RTS

lbC021896	MOVE.W	(A0),D0
	MULU.W	D1,D0
	LSR.L	#8,D0
	BRA.S	lbC0218C2

lbC02189E	MOVE.L	D0,10(SP)
	MOVEA.L	#lbL020FB4,A0
	ADDA.L	D2,A0
	CMPI.W	#$100,D1
	BNE.S	lbC0218B6
	MOVEQ	#0,D0
	MOVE.W	(A0),D0
	BRA.S	lbC0218BC

lbC0218B6	MOVE.W	(A0),D0
	MULU.W	D1,D0
	LSR.L	#8,D0
lbC0218BC	MULU.W	12(SP),D0
	LSR.L	#8,D0
lbC0218C2	MOVE.L	(SP)+,D2
	RTS

;	dc.w	0


;	SECTION	MarbleMadness0218C8,CODE
_Zero	MOVEA.L	4(SP),A0
	MOVE.L	8(SP),D0
	CLR.L	D1
	BTST	#0,D0
	BNE.S	lbC0218FE
	BTST	#0,7(SP)
	BNE.S	lbC0218FE
	BTST	#1,D0
	BEQ.S	lbC0218F2
	ASR.L	#1,D0
	SUBQ.L	#1,D0
lbC0218EA	MOVE.W	D1,(A0)+
	DBRA	D0,lbC0218EA
	RTS

lbC0218F2	ASR.L	#2,D0
	SUBQ.L	#1,D0
lbC0218F6	MOVE.L	D1,(A0)+
	DBRA	D0,lbC0218F6
	RTS

lbC0218FE	SUBQ.L	#1,D0
lbC021900	MOVE.B	D1,(A0)+
	DBRA	D0,lbC021900
	RTS

_Copy	MOVEA.L	4(SP),A0
	MOVEA.L	8(SP),A1
	MOVE.L	12(SP),D0
	BTST	#0,D0
	BNE.S	lbC021948
	BTST	#0,7(SP)
	BNE.S	lbC021948
	BTST	#0,11(SP)
	BNE.S	lbC021948
	BTST	#1,D0
	BEQ.S	lbC02193C
	ASR.L	#1,D0
	SUBQ.L	#1,D0
lbC021934	MOVE.W	(A0)+,(A1)+
	DBRA	D0,lbC021934
	RTS

lbC02193C	ASR.L	#2,D0
	SUBQ.L	#1,D0
lbC021940	MOVE.L	(A0)+,(A1)+
	DBRA	D0,lbC021940
	RTS

lbC021948	SUBQ.L	#1,D0
lbC02194A	MOVE.B	(A0)+,(A1)+
	DBRA	D0,lbC02194A
	RTS

;	dc.w	0


;	SECTION	MarbleMadness021954,CODE
; XXX below "tick" impl is identical to the one in SkyFox
_TickAudEG	MOVEA.L	4(SP),A1
	MOVEM.L	A5/A6,-(SP)
	JSR	_ALTickAudEG
	MOVEM.L	(SP)+,A5/A6
	RTS

_ALTickAudEG	MOVEA.L	0(A1),A0
	BRA.S	lbC021974

lbC02196E	RTS

lbC021970	MOVEA.L	0(A0),A0
lbC021974	TST.L	(A0)
	BEQ.S	lbC02196E
	MOVEA.L	$16(A0),A1
	MOVE.L	0(A1),D0
	BEQ.S	lbC021970
	BGE.S	lbC021992
	ADD.L	14(A0),D0
	BVS.S	lbC0219B0
	CMP.L	4(A1),D0
	BGT.S	lbC02199E
	BRA.S	lbC0219BE

lbC021992	ADD.L	14(A0),D0
	BVS.S	lbC0219D0
	CMP.L	4(A1),D0
	BGE.S	lbC0219DE
lbC02199E	MOVE.L	D0,14(A0)
lbC0219A2	MOVE.L	$12(A0),D1
	BEQ.S	lbC021970
	SWAP	D0
	MOVEA.L	D1,A1
	MOVE.W	D0,(A1)
	BRA.S	lbC021970

lbC0219B0	MOVE.L	#$80000000,D0
	BRA.S	lbC0219BE

lbC0219B8	CMP.L	4(A1),D0
	BGT.S	lbC0219F2
lbC0219BE	MOVEQ	#8,D1
	ADD.L	D1,$16(A0)
	MOVEA.L	$16(A0),A1
	MOVE.L	0(A1),D1
	BLT.S	lbC0219B8
	BRA.S	lbC0219EE

lbC0219D0	MOVE.L	#$7FFFFFFF,D0
	BRA.S	lbC0219DE

lbC0219D8	CMP.L	4(A1),D0
	BLT.S	lbC0219F2
lbC0219DE	MOVEQ	#8,D1
	ADD.L	D1,$16(A0)
	MOVEA.L	$16(A0),A1
	MOVE.L	0(A1),D1
	BGT.S	lbC0219D8
lbC0219EE	MOVE.L	-4(A1),D0
lbC0219F2	MOVE.L	D0,14(A0)
	TST.L	D1
	BNE.S	lbC0219A2
	MOVE.L	$1A(A0),D1
	BEQ.S	lbC021A04
	MOVE.L	D1,$16(A0)
lbC021A04	MOVE.L	$1E(A0),D1
	BEQ.S	lbC0219A2
	BRA	lbC0219A2

;	dc.w	0

;LoadSeg
;	SECTION	MarbleMadness021A10,CODE
;lbC021A10	MOVE.B	lbB021DD1,D0
;	MOVE.B	D0,lbB021DD0
;	TST.B	D0
;	BEQ.S	lbC021A4C
;	MOVE.W	lbW0005D6,D0			; song number
;	EXT.L	D0
;	ASL.L	#2,D0
;	MOVEA.L	D0,A0
;	ADDA.L	#lbL021E02,A0
;	MOVE.L	(A0),-(SP)
;	JSR	lbC008CF0
;	ADDQ.L	#4,SP
;	MOVE.L	D0,SampleDataPtr
;	TST.L	D0
;	BNE.S	lbC021A4C
;	CLR.B	lbB021DD0
;lbC021A4C	RTS

;lbC021A4E	LINK.W	A6,#-10
;	TST.L	SampleDataPtr
;	BEQ.S	lbC021AA6
;	MOVEA.L	SampleDataPtr,A0
;	MOVE.W	(A0),-2(A6)
;	ADDQ.L	#2,A0
;	MOVE.L	A0,-6(A6)
;lbC021A6A	MOVEA.L	-6(A6),A0
;	MOVE.W	(A0),D0
;	SUBQ.W	#3,D0
;	BNE.S	lbC021A92
;	MOVE.L	4(A0),D0
;	MOVE.L	D0,-10(A6)
;	TST.L	D0
;	BEQ.S	lbC021A8A
;	MOVE.L	D0,-(SP)
;	JSR	StopSfx
;	ADDQ.L	#4,SP
;lbC021A8A	MOVEA.L	-6(A6),A0
;	CLR.W	2(A0)
;lbC021A92	SUBQ.W	#1,-2(A6)
;	CMPI.W	#0,-2(A6)
;	BHI.S	lbC021A6A
;	BSR	lbC021DB2
;	BSR	lbC021D9C
;lbC021AA6	UNLK	A6
;	RTS

;UnloadSeg
;lbC021AAA	TST.L	SampleDataPtr
;	BEQ.S	lbC021AD6
;	TST.B	lbB021DD0
;	BEQ.S	lbC021AC2
;	JSR	StopMus
;	BSR.S	lbC021A4E
;lbC021AC2	MOVE.L	SampleDataPtr,-(SP)
;	JSR	lbC008D24
;	ADDQ.L	#4,SP
;	CLR.L	SampleDataPtr
;lbC021AD6	RTS

;	MOVEQ	#-1,D0
;	RTS

InitMusic
lbC021ADC	LINK.W	A6,#-$12
	MOVEM.L	D2,-(SP)
	MOVEA.L	SampleDataPtr,A0
	ADDQ.L	#2,A0
	MOVE.L	A0,-4(A6)
	TST.B	lbB021DD0
	BNE.S	lbC021B00
	MOVEM.L	(SP)+,D2
	UNLK	A6
	RTS

lbC021B00	MOVEQ	#0,D0
	MOVEA.L	SampleDataPtr,A0
	MOVE.W	(A0),D0
	MOVE.L	8(A6),D1
	CMP.L	D0,D1
	BCS.S	lbC021B1A
	MOVEM.L	(SP)+,D2
	UNLK	A6
	RTS

lbC021B1A	MOVE.L	8(A6),D0
	ASL.L	#3,D0
	ADD.L	D0,-4(A6)
	MOVEQ	#0,D0
	MOVEA.L	-4(A6),A0
	MOVE.W	(A0),D0
	CMPI.L	#6,D0
	BCC	lbC021CC8
	ASL.L	#2,D0
	JMP	lbC021B3C(PC,D0.L)

lbC021B3C	BRA	lbC021B54

	BRA	lbC021B8C

	BRA	lbC021C80

	BRA	lbC021C88

	BRA	lbC021CB6

	BRA	lbC021CC0

lbC021B54	MOVEA.L	-4(A6),A1
	MOVEA.L	4(A1),A0
	MOVE.L	A0,-8(A6)
	CMPA.L	#0,A0
	BEQ	lbC021CC8
	MOVE.L	A0,-(SP)
	BSR	lbC021D1E
	ADDQ.L	#4,SP
	CLR.L	-(SP)
	MOVEQ	#$63,D0
	MOVE.L	D0,-(SP)
	MOVEA.L	-8(A6),A0
	MOVE.L	(A0),-(SP)
	JSR	lbC01F6D0
	LEA	12(SP),SP
	BRA	lbC021CC8

lbC021B8C	MOVEA.L	-4(A6),A1
	MOVEA.L	4(A1),A0
	MOVE.L	A0,-8(A6)
	CMPA.L	#0,A0
	BEQ	lbC021CC8
	MOVE.W	$10(A0),-14(A6)
	TST.B	15(A6)
	BNE.S	lbC021BB6
	MOVE.W	#$FFFF,-14(A6)
	BRA.S	lbC021BC4

lbC021BB6	CMPI.B	#1,15(A6)
	BNE.S	lbC021BC4
	MOVE.W	#$FFFE,-14(A6)
lbC021BC4	MOVEA.L	-4(A6),A0
	MOVE.W	2(A0),D0
	ADDQ.W	#1,D0
	BNE.S	lbC021C0A
	MOVEQ	#0,D0
	MOVEA.L	-8(A6),A0
	MOVE.W	12(A0),D0
	MOVE.B	14(A0),D1
	EXT.W	D1
	EXT.L	D1
	MOVE.W	-14(A6),D2
	EXT.L	D2
	MOVE.L	D2,-(SP)
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	MOVE.L	8(A0),-(SP)
	MOVE.L	4(A0),-(SP)
	MOVE.L	(A0),-(SP)
	JSR	StartSfx
	
	LEA	$18(SP),SP
	MOVE.L	D0,-12(A6)
	BRA	lbC021CC8

lbC021C0A	MOVEA.L	SampleDataPtr,A0
	ADDQ.L	#2,A0
	MOVEA.L	-4(A6),A1
	MOVE.W	2(A1),D0
	EXT.L	D0
	ASL.L	#3,D0
	ADDA.L	D0,A0
	MOVE.L	4(A0),D0
	MOVE.L	D0,-12(A6)
	MOVE.L	A0,-$12(A6)
	TST.L	D0
	BEQ.S	lbC021C3A
	MOVE.L	D0,-(SP)
	JSR	StopSfx
	ADDQ.L	#4,SP
lbC021C3A	MOVEQ	#0,D0
	MOVEA.L	-8(A6),A0
	MOVE.W	12(A0),D0
	MOVE.B	14(A0),D1
	EXT.W	D1
	EXT.L	D1
	MOVE.W	-14(A6),D2
	EXT.L	D2
	MOVE.L	D2,-(SP)
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	MOVE.L	8(A0),-(SP)
	MOVE.L	4(A0),-(SP)
	MOVE.L	(A0),-(SP)
	JSR	StartSfx
	
	LEA	$18(SP),SP
	MOVEA.L	-$12(A6),A0
	MOVE.L	D0,4(A0)
	MOVE.W	2(A0),D0
	ADDQ.W	#1,D0
	MOVE.W	D0,2(A0)
	BRA.S	lbC021CC8

lbC021C80	JSR	StopMus
	BRA.S	lbC021CC8

lbC021C88	MOVEA.L	-4(A6),A0
	MOVE.W	2(A0),D0
	TST.W	D0
	BLE.S	lbC021CC8
	SUBQ.W	#1,D0
	MOVE.W	D0,2(A0)
	TST.W	D0
	BNE.S	lbC021CC8
	MOVE.L	4(A0),D0
	MOVE.L	D0,-8(A6)
	TST.L	D0
	BEQ.S	lbC021CC8
	MOVE.L	D0,-(SP)
	JSR	StopSfx
	
	ADDQ.L	#4,SP
;	BRA.S	lbC021CC8

lbC021CB6
;	MOVE.B	#1,lbB021DD2
;	BRA.S	lbC021CC8

lbC021CC0
;	CLR.B	lbB021DD2
;	NOP
lbC021CC8	MOVEM.L	(SP)+,D2
	UNLK	A6
	RTS

;lbC021CD0	LINK.W	A6,#-2
;	MOVE.W	lbW0006C4,D0
;	MOVE.W	lbW0006C6,D1
;	CMP.W	D1,D0
;	BGE.S	lbC021CE8
;	MOVEQ	#1,D0
;	BRA.S	lbC021CEA

;lbC021CE8	MOVEQ	#0,D0
;lbC021CEA	MOVE.B	D0,-1(A6)
;	EXT.W	D0
;	EXT.L	D0
;	MOVE.L	D0,-(SP)
;	MOVE.L	8(A6),-(SP)
;	BSR	lbC021ADC
;	ADDQ.L	#8,SP
;	UNLK	A6
;	RTS

;lbC021D02	MOVEQ	#15,D0
;	MOVE.L	D0,-(SP)
;	BSR.S	lbC021CD0
;	ADDQ.L	#4,SP
;	MOVE.W	lbW0005D6,D0
;	SUBQ.W	#2,D0
;	BEQ.S	lbC021D1C
;	MOVEQ	#$10,D0
;	MOVE.L	D0,-(SP)
;	BSR.S	lbC021CD0
;	ADDQ.L	#4,SP
;lbC021D1C	RTS

lbC021D1E	LINK.W	A6,#-6
	MOVEA.L	8(A6),A0
	MOVE.L	4(A0),-6(A6)
	CLR.B	-1(A6)
lbC021D30	MOVE.B	-1(A6),D0
	CMPI.B	#8,D0
	BCC.S	lbC021D82
	MOVEQ	#0,D1
	MOVE.B	-1(A6),D1
	MOVEA.L	-6(A6),A0
	MOVE.L	12(A0),-(SP)
	MOVE.L	8(A0),-(SP)
	MOVE.L	(A0),-(SP)
	MOVE.L	4(A0),-(SP)
	MOVE.L	D1,-(SP)
	JSR	lbC01F782
	LEA	$14(SP),SP
	MOVEQ	#0,D0
	MOVE.B	-1(A6),D0
	MOVEA.L	-6(A6),A0
	MOVE.B	$10(A0),D1
	EXT.W	D1
	EXT.L	D1
	MOVE.L	D1,-(SP)
	MOVE.L	D0,-(SP)
	JSR	lbC01F8DA
	ADDQ.L	#8,SP
	ADDQ.B	#1,-1(A6)
	BRA.S	lbC021D30

lbC021D82	MOVEA.L	8(A6),A0
	MOVE.W	8(A0),D0
	EXT.L	D0
	ASL.L	#7,D0
	MOVE.L	D0,-(SP)
	JSR	SetTempo
	ADDQ.L	#4,SP
	UNLK	A6
	RTS

StartMusic
lbC021D9C	TST.B	lbB021DD0
	BEQ.S	lbC021DB0
	JSR	InitSfx
	JSR	Initmus
lbC021DB0	RTS

StopMusic
lbC021DB2	TST.B	lbB021DD0
	BEQ.S	lbC021DCC
	JSR	StopMus
	JSR	ResetMus
	JSR	ResetSfx
lbC021DCC	RTS

;	dc.w	0

;	SECTION	MarbleMadness021DD0,DATA
lbB021DD0	dc.b	1
lbB021DD1	dc.b	1
lbB021DD2	dc.b	0
	dc.b	0
SampleDataPtr	dc.l	SampleData
;prcsnd.MSG	dc.b	'prcsnd',0
;begsnd.MSG	dc.b	'begsnd',0
;intsnd.MSG	dc.b	'intsnd',0
;aersnd.MSG	dc.b	'aersnd',0
;silsnd.MSG	dc.b	'silsnd',0
;ultsnd.MSG	dc.b	'ultsnd',0
;lbL021E02	dc.l	prcsnd.MSG
;	dc.l	begsnd.MSG
;	dc.l	intsnd.MSG
;	dc.l	aersnd.MSG
;	dc.l	silsnd.MSG
;	dc.l	ultsnd.MSG
;	dc.w	0


;	SECTION	MarbleMadness023E80,CODE	-> size $78
SetTempo_sub1	MOVE.L	D2,-(SP)
	MOVE.L	D3,-(SP)
	TST.L	D1
	BEQ.S	lbC023EAA
	TST.L	D0
	BEQ.S	lbC023EA8
	CLR.L	D2
	MOVEQ	#$1F,D3
lbC023E90	ASL.L	#1,D0
	ROXL.L	#1,D2
	CMP.L	D1,D2
	BCS.S	lbC023EA0
	SUB.L	D1,D2
	ADD.L	#1,D0
lbC023EA0	DBRA	D3,lbC023E90
	MOVE.L	D2,D1
	BRA.S	lbC023EAC

lbC023EA8	CLR.L	D1
lbC023EAA	CLR.L	D0
lbC023EAC	MOVE.L	(SP)+,D3
	MOVE.L	(SP)+,D2
	RTS

;	SECTION	MarbleMadness023EF8,CODE	-> size $30
lbC023EF8	MOVEM.L	D1-D3,-(SP)
	MOVE.L	D0,D2
	BEQ.S	lbC023F20
	TST.L	D1
	BNE.S	lbC023F08
	CLR.L	D0
	BRA.S	lbC023F20

lbC023F08	MOVE.L	D0,D3
	MULU.W	D1,D3
	SWAP	D2
	MULU.W	D1,D2
	SWAP	D2
	CLR.W	D2
	ADD.L	D2,D3
	SWAP	D1
	MULU.W	D1,D0
	SWAP	D0
	CLR.W	D0
	ADD.L	D3,D0
lbC023F20	MOVEM.L	(SP)+,D1-D3
	RTS


;	SECTION	MarbleMadness023F28,CODE
_CXM33	MOVEM.L	D1-D4,-(SP)
	MOVE.L	D0,D4
	EOR.L	D1,D4
	TST.L	D0
	BEQ.S	lbC023F64
	BPL.S	lbC023F38
	NEG.L	D0
lbC023F38	MOVE.L	D0,D2
	TST.L	D1
	BNE.S	lbC023F42
	CLR.L	D0
	BRA.S	lbC023F64

lbC023F42	BPL.S	lbC023F46
	NEG.L	D1
lbC023F46	MOVE.L	D0,D3
	MULU.W	D1,D3
	SWAP	D2
	MULU.W	D1,D2
	SWAP	D2
	CLR.W	D2
	ADD.L	D2,D3
	SWAP	D1
	MULU.W	D1,D0
	SWAP	D0
	CLR.W	D0
	ADD.L	D3,D0
	TST.L	D4
	BPL.S	lbC023F64
	NEG.L	D0
lbC023F64	MOVEM.L	(SP)+,D1-D4
	RTS


;_AddIntServer	MOVE.L	A6,-(SP)
;	MOVEA.L	SysBase,A6
;	MOVEM.L	8(SP),D0/A1
;	JSR	-$A8(A6)
;	MOVEA.L	(SP)+,A6
;	RTS

;_RemIntServer	MOVE.L	A6,-(SP)
;	MOVEA.L	SysBase,A6
;	MOVEM.L	8(SP),D0/A1
;	JSR	-$AE(A6)
;	MOVEA.L	(SP)+,A6
;	RTS

_AllocMem	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEM.L	8(SP),D0/D1
	JSR	-$C6(A6)
	MOVEA.L	(SP)+,A6
	RTS

_FreeMem	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	MOVE.L	12(SP),D0
	JSR	-$D2(A6)
	MOVEA.L	(SP)+,A6
	RTS

_AddHead	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEM.L	8(SP),A0/A1
	JSR	-$F0(A6)
	MOVEA.L	(SP)+,A6
	RTS

_AddTask	MOVEM.L	A2/A3/A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEM.L	$10(SP),A1-A3
	JSR	-$11A(A6)
	MOVEM.L	(SP)+,A2/A3/A6
	RTS

_RemTask	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	JSR	-$120(A6)
	MOVEA.L	(SP)+,A6
	RTS

_FindTask	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	JSR	-$126(A6)
	MOVEA.L	(SP)+,A6
	RTS

_Wait	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVE.L	8(SP),D0
	JSR	-$13E(A6)
	MOVEA.L	(SP)+,A6
	RTS

_Signal	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	MOVE.L	12(SP),D0
	JSR	-$144(A6)
	MOVEA.L	(SP)+,A6
	RTS

_AllocSignal	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVE.L	8(SP),D0
	JSR	-$14A(A6)
	MOVEA.L	(SP)+,A6
	RTS

_FreeSignal	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVE.L	8(SP),D0
	JSR	-$150(A6)
	MOVEA.L	(SP)+,A6
	RTS

_PutMsg	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEM.L	8(SP),A0/A1
	JSR	-$16E(A6)
	MOVEA.L	(SP)+,A6
	RTS

_GetMsg	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A0
	JSR	-$174(A6)
	MOVEA.L	(SP)+,A6
	RTS

_ReplyMsg	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	JSR	-$17A(A6)
	MOVEA.L	(SP)+,A6
	RTS

_WaitPort	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A0
	JSR	-$180(A6)
	MOVEA.L	(SP)+,A6
	RTS

_OpenDevice	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A0
	MOVEM.L	12(SP),D0/A1
	MOVE.L	$14(SP),D1
	JSR	-$1BC(A6)
	MOVEA.L	(SP)+,A6
	RTS

_CloseDevice	MOVE.L	A6,-(SP)
	MOVEA.L	SysBase,A6
	MOVEA.L	8(SP),A1
	JSR	-$1C2(A6)
	MOVEA.L	(SP)+,A6
	RTS

;	SECTION	MarbleMadness024A3C,CODE
_BeginIO	MOVEA.L	4(SP),A1
	MOVE.L	A6,-(SP)
	MOVEA.L	$14(A1),A6
	JSR	-$1E(A6)
	MOVEA.L	(SP)+,A6
	RTS

;	SECTION	MarbleMadness024A50,CODE
_NewList	MOVEA.L	4(SP),A0
	MOVE.L	A0,(A0)
	ADDQ.L	#4,(A0)
	CLR.L	4(A0)
	MOVE.L	A0,8(A0)
	RTS


SysBase
	dc.l	0
SampleData
;	SECTION	UltSnd000000,DATA,CHIP
;ProgStart
	dc.w	$22
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$5FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	UbL001256
	dc.l	$1FFFF
	dc.l	UbL0036EA
	dc.l	$1000A
	dc.l	UbL005E6E
	dc.l	$30000
	dc.l	0
	dc.l	$1000C
	dc.l	UbL001046
	dc.l	$30000
	dc.l	0
	dc.l	$1FFFF
	dc.l	UbL0025AA
	dc.l	$1000F
	dc.l	UbL0011DA
	dc.l	$30000
	dc.l	0
	dc.l	$1FFFF
	dc.l	0
	dc.l	$1FFFF
	dc.l	UbL00619E
	dc.l	$1FFFF
	dc.l	UbL002B92
	dc.l	$1FFFF
	dc.l	UbL001656
	dc.l	$1FFFF
	dc.l	UbL001656
	dc.l	$1FFFF
	dc.l	UbL001656
	dc.l	$1FFFF
	dc.l	UbL006292
	dc.l	$1FFFF
	dc.l	UbL0017E2
	dc.l	$1FFFF
	dc.l	UbL00197E
	dc.l	$FFFF
	dc.l	UbL006068
	dc.l	$1FFFF
	dc.l	UbL0034BE
	dc.l	$1FFFF
	dc.l	UbL005842
	dc.l	$1FFFF
	dc.l	UbL001352
	dc.l	$1FFFF
	dc.l	UbL004A16
	dc.l	$FFFF
	dc.l	UbL0001F2
	dc.l	$2FFFF
	dc.l	UbL0001F2
	dc.l	$FFFF
	dc.l	UbL00023C
	dc.l	$2FFFF
	dc.l	UbL00023C
UbL000112	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.l	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
UbW0001A2	dc.w	$C17F
	dc.w	$8000
UbW0001A6	dc.w	1
	dc.l	UbW0001A2
	dc.l	0
	dc.w	0
UbL0001B2	dc.l	UbW000BC2
	dc.l	$FFFF0100
	dc.l	UbW000BD4
	dc.l	$FFFF0000
	dc.l	UbW000C10
	dc.l	$FFFE0000
	dc.l	UbW000C4C
	dc.l	$FFFF0000
	dc.l	UbW000C88
	dc.l	$FFFE0000
	dc.l	UbW0001A6
	dc.l	$FFFFFF00
	dc.l	UbW000CDC
	dc.l	$FFFE0100
	dc.l	UbW0001A6
	dc.l	$FFFEFF00
UbL0001F2	dc.l	UbL0001B2
	dc.l	UbL000112
	dc.w	$A9
UbL0001FC	dc.l	UbW000ED0
	dc.l	$FFFF0100
	dc.l	UbW000F06
	dc.l	$FFFE0000
	dc.l	UbW000F3C
	dc.l	$FFFF0000
	dc.l	UbW000FEA
	dc.l	$FFFE0200
	dc.l	UbW0001A6
	dc.l	$FFFFFF00
	dc.l	UbW0001A6
	dc.l	$FFFEFF00
	dc.l	UbW0001A6
	dc.l	$FFFFFF00
	dc.l	UbW0001A6
	dc.l	$FFFEFF00
UbL00023C	dc.l	UbL0001FC
	dc.l	UbL000112
	dc.l	$A90000

;	SECTION	UltSnd000248,DATA,CHIP
UbL000248	dc.l	0
	dc.l	4
	dc.l	4
	dc.l	$291E0700
	dc.l	$10000
	dc.l	$FBFBD6FB
UbL000260	dc.l	$F6D619FB
	dc.l	$7CF6E9D6
	dc.l	$4019B5FB
UbL00026C	dc.l	$D7C08F6
	dc.l	$F0E9C5D6
	dc.l	$F3404419
	dc.l	$4B5BEFB
	dc.l	$40D447C
	dc.l	$2208FEF6
	dc.l	$F2F0EFE9
	dc.l	$CAC5CBD6
	dc.l	$E3F30340
	dc.l	$64442B19
	dc.l	$D04FCB5
	dc.l	$89BEE4FB
UbL00029C	dc.l	$FF04090D
	dc.l	$F447E7C
	dc.l	$58221008
	dc.l	$2FEFAF6
	dc.l	$F4F2F1F0
	dc.l	$F0EFEFE9
	dc.l	$DACAC5C5
	dc.l	$C7CBD0D6
	dc.l	$DCE3EBF3
	dc.l	$FB031540
	dc.l	$61645444
	dc.l	$362B2119
	dc.l	$130D0804
	dc.l	$FCECB5
	dc.l	$8F89A4BE
	dc.l	$D3E4F1FB
	dc.l	$FBFF0104
	dc.l	$7090B0D
	dc.l	$E0F1644
	dc.l	$7C7E7E7C
	dc.l	$72583A22
	dc.l	$16100C08
	dc.l	$50200FE
	dc.l	$FBFAF8F6
	dc.l	$F5F4F3F2
	dc.l	$F1F1F0F0
	dc.l	$F0F0EFEF
	dc.l	$EFEFEDE9
	dc.l	$E3DAD1CA
	dc.l	$C6C5C5C5
	dc.l	$C6C7C9CB
	dc.l	$CDD0D3D6
	dc.l	$D9DCE0E3
	dc.l	$E7EBEFF3
	dc.l	$F7FBFF03
	dc.l	$8152A40
	dc.l	$54616664
	dc.l	$5D544B44
	dc.l	$3D36302B
	dc.l	$26211D19
	dc.l	$1613100D
	dc.l	$A080604
	dc.l	$200FFFC
	dc.l	$F6ECD5B5
	dc.l	$9E8F8789
	dc.l	$95A4B2BE
	dc.l	$CAD3DCE4
	dc.l	$EAF1F6FB
UbL00035C	dc.l	$F9FBFDFF
	dc.l	$10304
	dc.l	$6070809
	dc.l	$A0B0C0D
	dc.l	$D0E0F0F
	dc.l	$10162844
	dc.l	$647C7F7E
	dc.l	$7E7E7D7C
	dc.l	$79726758
	dc.l	$493A2D22
	dc.l	$1A161310
	dc.l	$E0C0A08
	dc.l	$6050302
	dc.l	$FFFE
	dc.l	$FCFBFAFA
	dc.l	$F9F8F7F6
	dc.l	$F6F5F4F4
	dc.l	$F3F3F3F2
	dc.l	$F2F1F1F1
	dc.l	$F1F0F0F0
	dc.l	$F0F0F0F0
	dc.l	$EFEFEFEF
	dc.l	$EFEFEFEF
	dc.l	$EEEDECE9
	dc.l	$E6E3DFDA
	dc.l	$D5D1CDCA
	dc.l	$C7C6C5C5
	dc.l	$C5C5C5C5
	dc.l	$C6C6C7C7
	dc.l	$C8C9CACB
	dc.l	$CCCDCFD0
	dc.l	$D1D3D4D6
	dc.l	$D8D9DBDC
	dc.l	$DEE0E2E3
	dc.l	$E5E7E9EB
	dc.l	$EDEFF1F3
	dc.l	$F5F7F9FB
	dc.l	$FDFF0003
	dc.l	$5080E15
	dc.l	$1F2A3540
	dc.l	$4B545B61
	dc.l	$64666564
	dc.l	$615D5854
	dc.l	$504B4844
	dc.l	$403D3936
	dc.l	$33302D2B
	dc.l	$28262321
	dc.l	$1F1D1B19
	dc.l	$18161413
	dc.l	$11100E0D
	dc.l	$C0A0908
	dc.l	$7060504
	dc.l	$3020100
	dc.l	$FFFEFC
	dc.l	$F9F6F2EC
	dc.l	$E3D5C6B5
	dc.l	$A89E958F
	dc.l	$8A878789
	dc.l	$8E959CA4
	dc.l	$ABB2B8BE
	dc.l	$C4CACFD3
	dc.l	$D8DCE0E4
	dc.l	$E7EAEEF1
	dc.l	$F3F6F8FB
UbL00045C	dc.l	$4000
	dc.l	$10000
	dc.l	$FFFFF000
	dc.l	$8000
	dc.l	$FFFFFD71
	dc.l	$2000
	dc.l	0

;	SECTION	UltSnd000478,DATA,CHIP
UbL000478	dc.l	$C837C37F
	dc.l	$81418141
	dc.l	$81418141
	dc.l	$81418141
	dc.l	$C17FC87F
	dc.l	$8352C152
	dc.l	$C17F7F7F
	dc.b	$80
UbB000495	dc.b	$CD
	dc.l	$35CE35CD
	dc.l	$35CE35CD
	dc.l	$35CE3535
	dc.l	$3638CD35
	dc.l	$CE35CD35
	dc.l	$CE35CD35
	dc.l	$CE353331
	dc.l	$2ECD35CE
	dc.l	$35CD35CE
	dc.l	$35CD35CE
	dc.l	$35353638
	dc.l	$C135C17F
	dc.l	$CD41CE41
	dc.l	$CD41CE41
	dc.l	$CD41CE41
	dc.l	$414244CD
	dc.l	$41CE41CD
	dc.l	$41CE41CD
	dc.l	$41CE413F
	dc.l	$3D3ACD41
	dc.l	$CE41CD41
	dc.l	$CE41CD41
	dc.l	$CE414142
	dc.l	$44C141C1
	dc.l	$7FCE4646
	dc.l	$46464646
	dc.l	$CE464442
	dc.l	$CE413F3D
	dc.l	$CE3C3A38
	dc.l	$363533CE
	dc.l	$31302E2C
	dc.l	$2A29C27F
	dc.l	$CE3C3A38
	dc.l	$363533CE
	dc.l	$31333536
	dc.l	$383ACE3C
	dc.l	$3D3FCE41
	dc.l	$4244CE43
	dc.l	$413FCE3D
	dc.l	$3B3ACE38
	dc.l	$3735CE33
	dc.l	$312FCE2E
	dc.l	$2C2BCE29
	dc.l	$2827CE26
	dc.l	$25242322
	dc.l	$20CD35CE
	dc.l	$3BC33AC2
	dc.l	$7FCD41CE
	dc.l	$47C346C2
	dc.l	$7FC23A3C
	dc.l	$C83D833F
	dc.l	$833FC846
	dc.l	$81468146
	dc.l	$C17FC17F
	dc.b	$80
UbB00056F	dc.b	$C1
	dc.l	$7FC27F82
	dc.l	$23812381
	dc.l	$23812381
	dc.l	$7F7FC37F
	dc.l	$337F337F
	dc.l	$337F337F
	dc.l	$337F33C3
	dc.l	$7F337F33
	dc.l	$C37F337F
	dc.l	$33C37F33
	dc.w	$7F33
	dc.b	$80
UbB00059B	dc.b	$C3
	dc.l	$7F337F33
	dc.l	$7F337F33
	dc.l	$7F337F33
	dc.l	$C37FC933
	dc.l	$C47F8333
	dc.l	$C4337FC9
	dc.l	$33C47FC3
	dc.w	$3380
UbL0005BA	dc.l	$C17FC27F
	dc.l	$CE414141
	dc.l	$414141CE
	dc.l	$413F3DCE
	dc.l	$3C3A38CE
	dc.l	$363533CE
	dc.l	$31302ECE
	dc.l	$2C2A29C8
	dc.l	$7FC17FC1
	dc.l	$7FC37F33
	dc.l	$C27FC37F
	dc.l	$33C27FC3
	dc.l	$7F337F33
	dc.l	$C37F3333
	dc.l	$7FC37F36
	dc.l	$2AC336C3
	dc.l	$7F337F33
	dc.l	$C37F337F
	dc.l	$33C37F33
	dc.l	$7F33C37F
	dc.l	$337F3380
UbL00060E	dc.l	$C17F7FC1
	dc.l	$43C27FC4
	dc.l	$7FC65552
	dc.l	$504D8349
	dc.l	$C149C17F
	dc.l	$7FC37F30
	dc.l	$7F307F30
	dc.l	$7F307F30
	dc.l	$7F30C37F
	dc.l	$307F30C3
	dc.l	$7F307F30
	dc.l	$C37F307F
	dc.w	$3080
UbL000640	dc.l	$C37F307F
	dc.l	$307F307F
	dc.l	$307F307F
	dc.l	$30C37FC9
	dc.l	$2FC47F83
	dc.l	$2FC42F7F
	dc.l	$C92FC47F
	dc.w	$C32F
	dc.b	$80
UbB00065F	dc.b	$C1
	dc.l	$7F7F7FC3
	dc.l	$7FCE3A3A
	dc.l	$3A3A3A3A
	dc.l	$CE3A3836
	dc.l	$CE353432
	dc.l	$C87FC17F
	dc.l	$C37F30C2
	dc.l	$7FC37F30
	dc.l	$C27FC37F
	dc.l	$307F30C3
	dc.l	$7F2F2F7F
	dc.l	$C37F3030
	dc.l	$30C37F30
	dc.l	$7F30C37F
	dc.l	$307F30C3
	dc.l	$7F307F30
	dc.l	$C37F307F
	dc.w	$3080
UbL0006A6	dc.l	$C17F7FC2
	dc.l	$7FC53F41
	dc.l	$43458346
	dc.l	$C146C17F
	dc.l	$C17F7FC3
	dc.l	$7F2E7F2E
	dc.l	$7F2E7F2E
	dc.l	$7F2E7F2E
	dc.l	$C37F2E7F
	dc.l	$2EC37F2E
	dc.l	$7F2EC37F
	dc.l	$2E7F2E80
UbL0006D6	dc.l	$C37F2E7F
	dc.l	$2E7F2E7F
	dc.l	$2E7F2E7F
	dc.l	$2EC37FC9
	dc.l	$2CC47F83
	dc.l	$2CC42C7F
	dc.l	$C92CC47F
	dc.w	$C32C
	dc.b	$80
UbB0006F5	dc.b	$C1
	dc.l	$7F7F7F7F
	dc.l	$7F7FC37F
	dc.l	$2EC27FC3
	dc.l	$7F2EC27F
	dc.l	$C37F2E7F
	dc.l	$2DC37F2D
	dc.l	$2D7FC37F
	dc.l	$2D2D2DC3
	dc.l	$7F2E7F2E
	dc.l	$7F2E7F2E
	dc.l	$7F2E7F2E
	dc.l	$7F2E7F2E
	dc.b	$80
UbB000727	dc.b	$C1
	dc.l	$7F7F7F7F
	dc.l	$7FC17F7F
	dc.l	$C37F2A7F
	dc.l	$2A7F2A7F
	dc.l	$2A7F2A7F
	dc.l	$2AC37F2A
	dc.l	$7F2AC37F
	dc.l	$297F29C3
	dc.l	$7F297F29
	dc.b	$80
UbB00074D	dc.b	$C3
	dc.l	$7F297F29
	dc.l	$7F297F29
	dc.l	$7F297F29
	dc.l	$C37FC929
	dc.l	$C47F8329
	dc.l	$C4297FC9
	dc.l	$29C47FC3
	dc.w	$2980
UbL00076C	dc.l	$C17F7F7F
	dc.l	$C17F7F7F
	dc.l	$C37F2AC2
	dc.l	$7FC37F2A
	dc.l	$C27FC37F
	dc.l	$2A7F2AC3
	dc.l	$7F2A2A7F
	dc.l	$C37F2A2A
	dc.l	$2AC37F29
	dc.l	$7F29C37F
	dc.l	$297F29C3
	dc.l	$7F297F29
	dc.l	$C37F297F
	dc.w	$2980
UbL0007A2	dc.l	$C27F8243
	dc.l	$C143C17F
	dc.l	$7F7FC17F
	dc.l	$C17FC17F
	dc.l	$C17FCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.l	$CE2BCD2B
	dc.w	$CE2B
	dc.b	$80
UbB0007F5	dc.b	$CD
	dc.l	$4DCE4DCD
	dc.l	$4DCE4DCD
	dc.l	$4DCE4D4D
	dc.l	$4E50CD4D
	dc.l	$CE4DCD4D
	dc.l	$CE4DCD4D
	dc.l	$CE4D4B49
	dc.l	$46CD4DCE
	dc.l	$4DCD4DCE
	dc.l	$4DCD4DCE
	dc.l	$4D4D4E50
	dc.l	$C37F4F7F
	dc.l	$4F7F4F7F
	dc.l	$4FCD4DCE
	dc.l	$4DCD4DCE
	dc.l	$4DCD4DCE
	dc.l	$4D4D4E50
	dc.l	$CD4DCE4D
	dc.l	$CD4DCE4D
	dc.l	$CD4DCE4D
	dc.l	$4B4946CD
	dc.l	$4DCE4DCD
	dc.l	$4DCE4DCD
	dc.l	$4DCE4D4D
	dc.l	$4E50C37F
	dc.l	$4F7F4F7F
	dc.l	$4F7F4FC1
	dc.l	$7F7F7FC1
	dc.l	$7F7F7FC2
	dc.l	$7FCE3A3A
	dc.l	$3A3A3B3D
	dc.l	$C27FCE46
	dc.l	$46464647
	dc.l	$49C37F4F
	dc.l	$7F4FC37F
	dc.l	$4F4F7FC3
	dc.l	$7F4F4F4F
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.l	$CD2BCE2B
	dc.b	$80
UbB0008CB	dc.b	$C1
	dc.l	$7F7F7F7F
	dc.l	$7FCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1DCD1DCE
	dc.l	$1D1D1E20
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.b	$80
UbB000951	dc.b	$CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE2280
UbL0009A2	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CE22201E
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$CD1DCE1D
	dc.l	$1D1E20CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE2280
UbL000A92	dc.l	$C17F7F7F
	dc.l	$7F7FC17F
	dc.l	$C17FC17F
	dc.l	$7F7FC87F
	dc.l	$CE111214
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.b	$80
UbB000AC7	dc.b	$CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE1680
UbL000B18	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$C37FC17F
	dc.l	$7F7F7FC8
	dc.l	$7FCE1112
	dc.l	$14CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.l	$16CD16CE
	dc.w	$1680
UbW000BC2	dc.w	1
	dc.l	UbL000478
	dc.w	4
	dc.l	UbB000495
	dc.l	0
	dc.w	0
UbW000BD4	dc.w	1
	dc.l	UbB00056F
	dc.w	2
	dc.l	UbB00059B
	dc.w	1
	dc.l	UbL0005BA
	dc.w	2
	dc.l	UbB00059B
	dc.w	1
	dc.l	UbL0005BA
	dc.w	2
	dc.l	UbB00059B
	dc.w	1
	dc.l	UbL0005BA
	dc.w	2
	dc.l	UbB00059B
	dc.w	1
	dc.l	UbL0005BA
	dc.w	0
	dc.w	0
	dc.w	0
UbW000C10	dc.w	1
	dc.l	UbL00060E
	dc.w	2
	dc.l	UbL000640
	dc.w	1
	dc.l	UbB00065F
	dc.w	2
	dc.l	UbL000640
	dc.w	1
	dc.l	UbB00065F
	dc.w	2
	dc.l	UbL000640
	dc.w	1
	dc.l	UbB00065F
	dc.w	2
	dc.l	UbL000640
	dc.w	1
	dc.l	UbB00065F
	dc.w	0
	dc.w	0
	dc.w	0
UbW000C4C	dc.w	1
	dc.l	UbL0006A6
	dc.w	2
	dc.l	UbL0006D6
	dc.w	1
	dc.l	UbB0006F5
	dc.w	2
	dc.l	UbL0006D6
	dc.w	1
	dc.l	UbB0006F5
	dc.w	2
	dc.l	UbL0006D6
	dc.w	1
	dc.l	UbB0006F5
	dc.w	2
	dc.l	UbL0006D6
	dc.w	1
	dc.l	UbB0006F5
	dc.w	0
	dc.w	0
	dc.w	0
UbW000C88	dc.w	1
	dc.l	UbB000727
	dc.w	2
	dc.l	UbB00074D
	dc.w	1
	dc.l	UbL00076C
	dc.w	2
	dc.l	UbB00074D
	dc.w	1
	dc.l	UbL00076C
	dc.w	2
	dc.l	UbB00074D
	dc.w	1
	dc.l	UbL00076C
	dc.w	2
	dc.l	UbB00074D
	dc.w	1
	dc.l	UbL00076C
	dc.w	0
	dc.w	0
	dc.w	0
	dc.w	1
	dc.l	UbL0007A2
	dc.w	2
	dc.l	UbB0007F5
	dc.w	2
	dc.l	UbB0007F5
	dc.w	0
	dc.w	0
	dc.w	0
UbW000CDC	dc.w	1
	dc.l	UbB0008CB
	dc.w	2
	dc.l	UbB000951
	dc.w	1
	dc.l	UbL0009A2
	dc.w	2
	dc.l	UbB000951
	dc.w	1
	dc.l	UbL0009A2
	dc.w	2
	dc.l	UbB000951
	dc.w	1
	dc.l	UbL0009A2
	dc.w	2
	dc.l	UbB000951
	dc.w	1
	dc.l	UbL0009A2
	dc.w	0
	dc.w	0
	dc.w	0
	dc.w	1
	dc.l	UbL000A92
	dc.w	2
	dc.l	UbB000AC7
	dc.w	1
	dc.l	UbL000B18
	dc.w	2
	dc.l	UbB000AC7
	dc.w	1
	dc.l	UbL000B18
	dc.w	2
	dc.l	UbB000AC7
	dc.w	1
	dc.l	UbL000B18
	dc.w	2
	dc.l	UbB000AC7
	dc.w	1
	dc.l	UbL000B18
	dc.l	0
	dc.w	0

;	SECTION	UltSnd000D54,DATA,CHIP
UbL000D54	dc.l	$CE2B2BCE
	dc.l	$2FCE2A2A
	dc.l	$CE25CE2F
	dc.l	$2FCE2A2A
	dc.l	$CE252580
UbL000D68	dc.l	$C37FCD2B
	dc.l	$CE2BC37F
	dc.l	$CD2BCE2B
	dc.l	$C37FCD2B
	dc.l	$CE2BC37F
	dc.l	$CE2FCE2A
	dc.w	$CE25
	dc.b	$80
UbB000D83	dc.b	$C3
	dc.l	$7FCD2BCE
	dc.l	$2BC37FCD
	dc.l	$2BCE2BC3
	dc.l	$7FCD2BCE
	dc.l	$2BC37FCE
	dc.l	$2FCE2ACE
	dc.l	$25C37FCD
	dc.l	$2BCE2BC3
	dc.l	$7FCD2BCE
	dc.l	$2BC37FCD
	dc.l	$2BCE2BC3
	dc.l	$7FCE2FCE
	dc.l	$2ACE2580
	dc.w	1
	dc.l	UbL000D54
	dc.w	8
	dc.l	UbL000D68
	dc.w	1
	dc.l	UbB000D83
	dc.w	8
	dc.l	UbL000D68
	dc.w	1
	dc.l	UbB000D83
	dc.w	0
	dc.w	0
	dc.w	0
UbW000DDC	dc.w	$C17F
	dc.b	$80
UbB000DDF	dc.b	$CD
	dc.l	$17CE17CD
	dc.l	$17CE17CD
	dc.l	$17CE17CD
	dc.l	$17CE1780
UbL000DF0	dc.l	$CD17CE17
	dc.l	$CD17CE17
	dc.l	$CD17CE17
	dc.l	$CD17CE17
	dc.l	$80000001
	dc.l	UbW000DDC
	dc.w	$10
	dc.l	UbB000DDF
	dc.w	4
	dc.l	UbL000DF0
	dc.w	$10
	dc.l	UbB000DDF
	dc.w	4
	dc.l	UbL000DF0
	dc.w	0
	dc.w	0
	dc.w	0
UbW000E26	dc.w	$C17F
	dc.b	$80
UbB000E29	dc.b	$C3
	dc.l	$3E3E3E3E
	dc.b	$80
UbB000E2F	dc.b	$C3
	dc.l	$3E3E3E3E
	dc.l	$80000001
	dc.l	UbW000E26
	dc.w	$10
	dc.l	UbB000E29
	dc.w	4
	dc.l	UbB000E2F
	dc.w	$10
	dc.l	UbB000E29
	dc.w	4
	dc.l	UbB000E2F
	dc.l	0
	dc.w	0
UbL000E5A	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.l	$CD22CE22
	dc.b	$80
UbB000E7B	dc.b	$CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE22CD
	dc.l	$22CE2280
	dc.w	1
	dc.l	UbL000E5A
	dc.w	$48
	dc.l	UbB000E7B
	dc.w	0
	dc.w	0
	dc.w	0
UbL000E9E	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.l	$CD16CE16
	dc.b	$80
UbB000EBF	dc.b	$CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE16CD
	dc.l	$16CE1680
UbW000ED0	dc.w	1
	dc.l	UbL000E9E
	dc.w	$48
	dc.l	UbB000EBF
	dc.w	0
	dc.w	0
	dc.w	0
UbL000EE2	dc.l	$C17F7F80
UbL000EE6	dc.l	$CD7FCE29
	dc.l	$CD35CE29
	dc.l	$353529CD
	dc.l	$35CE33CD
	dc.l	$7FCE29CD
	dc.l	$33CE2933
	dc.l	$3329CD33
	dc.l	$CE318000
UbW000F06	dc.w	1
	dc.l	UbL000EE2
	dc.w	$24
	dc.l	UbL000EE6
	dc.w	0
	dc.w	0
	dc.w	0
UbL000F18	dc.l	$C17F7F80
UbL000F1C	dc.l	$CD7FCE22
	dc.l	$CD31CE22
	dc.l	$313122CD
	dc.l	$31CE30CD
	dc.l	$7FCE22CD
	dc.l	$30CE2230
	dc.l	$3022CD30
	dc.l	$CE2E8000
UbW000F3C	dc.w	1
	dc.l	UbL000F18
	dc.w	$24
	dc.l	UbL000F1C
	dc.l	0
	dc.w	0
UbL000F4E	dc.l	$C17F7F80
UbL000F52	dc.l	$7F7F8544
	dc.l	$88468A46
	dc.l	$C346CE44
	dc.l	$42413F3D
	dc.l	$3C3ACE38
	dc.l	$36CF383A
	dc.l	$CE3ACF38
	dc.l	$3ACE3ACE
	dc.l	$3835CD38
	dc.l	$CE38833A
	dc.l	$C23A7FCF
	dc.l	$4B4DCE4D
	dc.l	$CF4B4DCE
	dc.l	$4DCF4B4D
	dc.l	$CE4D844B
	dc.l	$894DC34D
	dc.l	$CE4B4948
	dc.l	$464442CE
	dc.l	$444648CE
	dc.l	$46444241
	dc.l	$3F3DCE3F
	dc.l	$4142413F
	dc.l	$3DCE413D
	dc.l	$3A413D3A
	dc.l	$85418343
	dc.l	$CA43CE41
	dc.l	$3D3A413D
	dc.l	$3A853683
	dc.l	$38CA38CE
	dc.l	$413D3A41
	dc.l	$3D3ACF44
	dc.l	$46CE46CF
	dc.l	$4446CE46
	dc.l	$4441CF44
	dc.l	$8846CE46
	dc.l	$CF46C146
	dc.l	$C17F7F7F
	dc.l	$7F867F80
UbW000FEA	dc.w	1
	dc.l	UbL000F4E
	dc.w	4
	dc.l	UbL000F52
	dc.w	0
	dc.w	0
	dc.w	0

;	SECTION	UltSnd000FFC,DATA,CHIP
UbL000FFC	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFFDDE
	dc.l	$2000
	dc.l	0
UbL001010	dc.l	$E1F1
	dc.l	$E1F1
	dc.l	1
	dc.l	$E200
	dc.l	$7D
	dc.l	$11D00
	dc.l	0
UbL00102C	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL001038	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL00102C
UbL001046	dc.l	UbL001038
	dc.l	UbL000FFC
	dc.l	UbL001010
	dc.l	$870A00
	dc.w	$FFFE

;	SECTION	UltSnd001058,DATA,CHIP
UbL001058	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFFF93
	dc.l	$4000
	dc.l	0
UbL00106C	dc.l	$D600
	dc.l	$D600
	dc.l	$FFFFFBC4
	dc.l	$8E00
	dc.l	$11CC
	dc.l	$14000
	dc.l	$FFFFF0DC
	dc.l	$D600
	dc.l	$A99
	dc.l	$14000
	dc.l	$FFFFF24F
	dc.l	$8E00
	dc.l	$1800
	dc.l	$D600
	dc.l	$A99
	dc.l	$14000
	dc.l	$FFFFF0DC
	dc.l	$D600
	dc.l	$827
	dc.l	$14000
	dc.l	$FFFFC4AB
	dc.l	$8E00
	dc.l	$733
	dc.l	$D600
	dc.l	$2355
	dc.l	$14000
	dc.l	$FFFFF000
	dc.l	$A000
	dc.l	$FFFFFB80
	dc.l	$8E00
	dc.l	$733
	dc.l	$D600
	dc.l	$FFFFF800
	dc.l	$8E00
	dc.l	$A49
	dc.l	$D600
	dc.l	$FFFFF8CD
	dc.l	$8E00
	dc.l	$589
	dc.l	$D600
	dc.l	$FFFFF84A
	dc.l	$A000
	dc.l	$FFFFFEE0
	dc.l	$8E00
	dc.l	$A49
	dc.l	$D600
	dc.l	$FFFFFBC4
	dc.l	$8E00
	dc.l	$6D8
	dc.l	$14000
	dc.l	$FFFFF24F
	dc.l	$8E00
	dc.l	$8E6
	dc.l	$14000
	dc.l	$FFFFF588
	dc.l	$8E00
	dc.l	$DB1
	dc.l	$14000
	dc.l	$FFFFF24F
	dc.l	$8E00
	dc.l	$52
	dc.l	$A000
	dc.l	$FFFFFEF1
	dc.l	$8E00
	dc.l	$399
	dc.l	$D600
	dc.l	$FFFFF400
	dc.l	$8E00
	dc.l	$4CF
	dc.l	$14000
	dc.l	$FFFFF24F
	dc.l	$8E00
	dc.l	$8E6
	dc.l	$14000
	dc.l	$FFFFF4E0
	dc.l	$8E00
	dc.l	$CB6
	dc.l	$14000
	dc.l	$FFFFF24F
	dc.l	$8E00
	dc.l	$52
	dc.l	$A000
	dc.l	$FFFFFEF1
	dc.l	$8E00
	dc.l	0
UbL0011C0	dc.l	8
	dc.l	$C0002F
	dc.l	UbL00026C
UbL0011CC	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0011C0
UbL0011DA	dc.l	UbL0011CC
	dc.l	UbL001058
	dc.l	UbL00106C
	dc.l	$2620A00
	dc.w	$FFFE

;	SECTION	UltSnd0011EC,DATA,CHIP
UbL0011EC	dc.l	$80
	dc.l	$140003F
	dc.l	UbL00035C
UbL0011F8	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0011EC
UbL001206	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFC000
	dc.l	$8000
	dc.l	$6000
	dc.l	$E000
	dc.l	$FFFFC000
	dc.l	$8000
	dc.l	$4000
	dc.l	$C000
	dc.l	$FFFFE000
	dc.l	$8000
	dc.l	$FFFFFEB9
	dc.l	$4000
	dc.l	0
UbL001242	dc.l	$F1C7
	dc.l	$F1C7
	dc.l	$38E
	dc.l	$10000
	dc.l	0
UbL001256	dc.l	UbL0011F8
	dc.l	UbL001206
	dc.l	UbL001242
	dc.l	$4E0900
	dc.w	$FFFE

;	SECTION	UltSnd001268,DATA,CHIP
UbL001268	dc.l	$18180
	dc.l	$18180
	dc.l	$FFFFFF40
	dc.l	$17D00
	dc.l	$FFFF76C0
	dc.l	$F3C0
	dc.l	$FFFFFF40
	dc.l	$F000
	dc.l	$FFFFC7C0
	dc.l	$B7C0
	dc.l	$FFFFFF40
	dc.l	$B400
	dc.l	$B880
	dc.l	$16C80
	dc.l	$FFFFFF40
	dc.l	$16800
	dc.l	$FFFFA8C0
	dc.l	$110C0
	dc.l	$FFFFFF40
	dc.l	$10D00
	dc.l	$FFFFC0C0
	dc.l	$CDC0
	dc.l	$FFFFFF40
	dc.l	$CA00
	dc.l	$4E80
	dc.l	$11880
	dc.l	$C0
	dc.l	$11D00
	dc.l	$D40
	dc.l	$12A40
	dc.l	$C0
	dc.l	$12E00
	dc.l	$E40
	dc.l	$13C40
	dc.l	$C0
	dc.l	$14000
	dc.l	$D00
	dc.l	$14D00
	dc.l	$C0
	dc.l	$15300
	dc.l	$12C0
	dc.l	$165C0
	dc.l	$C0
	dc.l	$16800
	dc.l	$1500
	dc.l	$17D00
	dc.l	0
	dc.l	$4000
	dc.l	$10000
	dc.l	$FFFFFCCD
	dc.l	$5555
	dc.l	0
UbL001338	dc.l	$20
	dc.l	$100003F
	dc.l	UbL00029C
UbL001344	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL001338
UbL001352	dc.l	UbL001344
	dc.l	0
	dc.l	UbL001268
	dc.l	$4A0A00
	dc.w	$FFFE

;	SECTION	UltSnd001364,DATA,CHIP
UbL001364	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FFFE
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	0
UbL001518	dc.l	$8603
	dc.l	$8603
	dc.l	$FFFFFFFF
	dc.l	$8600
	dc.l	$1103
	dc.l	$9703
	dc.l	$FFFFFFFF
	dc.l	$9700
	dc.l	$1303
	dc.l	$AA03
	dc.l	$FFFFFFFF
	dc.l	$AA00
	dc.l	$1403
	dc.l	$BE03
	dc.l	$FFFFFFFF
	dc.l	$BE00
	dc.l	$1903
	dc.l	$D703
	dc.l	$FFFFFFFF
	dc.l	$D700
	dc.l	$FFFFA103
	dc.l	$7803
	dc.l	$FFFFFFFF
	dc.l	$7800
	dc.l	$9503
	dc.l	$10D03
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$2103
	dc.l	$12E03
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$FFFF7C03
	dc.l	$AA03
	dc.l	$FFFFFFFF
	dc.l	$AA00
	dc.l	$2C03
	dc.l	$D603
	dc.l	$FFFFFFFF
	dc.l	$D600
	dc.l	$1A03
	dc.l	$F003
	dc.l	$FFFFFFFF
	dc.l	$F000
	dc.l	$1D03
	dc.l	$10D03
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$2103
	dc.l	$12E03
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$2503
	dc.l	$15303
	dc.l	$FFFFFFFF
	dc.l	$15300
	dc.l	$FFFFBA03
	dc.l	$10D03
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$9F03
	dc.l	$1AC03
	dc.l	$FFFFFFFF
	dc.l	$1AC00
	dc.l	$FFFFA703
	dc.l	$15303
	dc.l	$FFFFFFFF
	dc.l	$15300
	dc.l	$2A03
	dc.l	$17D03
	dc.l	$FFFFFFFF
	dc.l	$17D00
	dc.l	0
UbL00163C	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL001648	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL00163C
UbL001656	dc.l	UbL001648
	dc.l	UbL001364
	dc.l	UbL001518
	dc.l	$480A00
	dc.w	$FFFE

;	SECTION	UltSnd001668,DATA,CHIP
UbL001668	dc.l	$FFFC
	dc.l	$FFFC
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFC
	dc.l	$FFFC
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFC
	dc.l	$FFFC
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFF
	dc.l	$FFFF
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0003
	dc.l	0
	dc.l	$FFFFFFFF
	dc.l	0
	dc.l	$FFFF
	dc.l	$FFFF
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0003
	dc.l	0
	dc.l	$FFFFFFFF
	dc.l	0
	dc.l	$FFFF
	dc.l	$FFFF
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0003
	dc.l	0
	dc.l	$FFFFFFFF
	dc.l	0
	dc.l	$FFFF
	dc.l	$FFFF
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0003
	dc.l	0
	dc.l	$FFFFFFFF
	dc.l	0
	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFF800
	dc.l	0
	dc.l	0
UbL001744	dc.l	$19405
	dc.l	$19405
	dc.l	$FFFFFFFF
	dc.l	$19400
	dc.l	$FFFF9A05
	dc.l	$12E05
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$FFFFB405
	dc.l	$E205
	dc.l	$FFFFFFFF
	dc.l	$E200
	dc.l	$FFFFE805
	dc.l	$CA05
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$CA04
	dc.l	$CA04
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$CA04
	dc.l	$CA04
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$CA04
	dc.l	$CA04
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$CA11
	dc.l	$CA11
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	0
UbL0017C8	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL0017D4	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0017C8
UbL0017E2	dc.l	UbL0017D4
	dc.l	UbL001668
	dc.l	UbL001744
	dc.l	$3C0A00
	dc.w	$FFFE

;	SECTION	UltSnd0017F4,DATA,CHIP
UbL0017F4	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFF2
	dc.l	$FFF2
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	$FFFA
	dc.l	$FFFA
	dc.l	1
	dc.l	$10000
	dc.l	$FFFF0000
	dc.l	0
	dc.l	0
UbL0018D0	dc.l	$16807
	dc.l	$16807
	dc.l	$FFFFFFFF
	dc.l	$16800
	dc.l	$FFFFA507
	dc.l	$10D07
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$FFFFD507
	dc.l	$E207
	dc.l	$FFFFFFFF
	dc.l	$E200
	dc.l	$FFFFE80F
	dc.l	$CA0F
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$1807
	dc.l	$E207
	dc.l	$FFFFFFFF
	dc.l	$E200
	dc.l	$2B07
	dc.l	$10D07
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$FFFFBD07
	dc.l	$CA07
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	$1807
	dc.l	$E207
	dc.l	$FFFFFFFF
	dc.l	$E200
	dc.l	$2B07
	dc.l	$10D07
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	0
UbL001964	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL001970	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL001964
UbL00197E	dc.l	UbL001970
	dc.l	UbL0017F4
	dc.l	UbL0018D0
	dc.l	$500A00
	dc.w	$FFFE

;	SECTION	UltSnd001990,DATA,CHIP
UbL001990	dc.l	$FE
	dc.l	$FCFCFBFB
	dc.l	$FAF9FCFD
	dc.l	$FF000001
	dc.l	$2030505
	dc.l	$5040404
	dc.l	$4050505
	dc.l	$5050505
	dc.l	$5050403
	dc.l	$3030F21
	dc.l	$1C16140D
	dc.l	$802FEFA
	dc.l	$F6F2EFEC
	dc.l	$EAE8E8E8
	dc.l	$E9EAEAEB
	dc.l	$ECEEF0F1
	dc.l	$F3F3F4F5
	dc.l	$F6F8FAFC
	dc.l	$FEFF0001
	dc.l	$2030505
	dc.l	$6050506
	dc.l	$6070805
	dc.l	$F4E4EFF4
	dc.l	$FB062039
	dc.l	$302B261E
	dc.l	$1B171411
	dc.l	$D0B0807
	dc.l	$5030100
	dc.l	$102D23
	dc.l	$130C00FA
	dc.l	$F4EFECEA
	dc.l	$E8E5E3E1
	dc.l	$E0E0E1E2
	dc.l	$E4E6E7E8
	dc.l	$EAECEEF0
	dc.l	$F0EFE1AE
	dc.l	$9BC6D8EA
	dc.l	$FA020C13
	dc.l	$1A202426
	dc.l	$28292A2B
	dc.l	$2C2C2C2C
	dc.l	$2A292726
	dc.l	$24221F1B
	dc.l	$18141210
	dc.l	$F0F1949
	dc.l	$5E2C1B0F
	dc.l	$FFF7ECE3
	dc.l	$DCD5D2CF
	dc.l	$CDCBCAC9
	dc.l	$C9C9CBCC
	dc.l	$CDCBC18E
	dc.l	$81A6BDCB
	dc.l	$DFE9F700
	dc.l	$911161A
	dc.l	$1D1F2224
	dc.l	$25272829
	dc.l	$29282827
	dc.l	$26262421
	dc.l	$1E1C1A18
	dc.l	$17161413
	dc.l	$110F0E0C
	dc.l	$A080502
	dc.l	$FEFDFC
	dc.l	$FCFBFAF9
	dc.l	$F8F7F7F6
	dc.l	$F5F4F2F1
	dc.l	$F1F1F1F3
	dc.l	$F3F3F4F5
	dc.l	$F5F6F7F6
	dc.l	$F6F5F4F4
	dc.l	$F5F6F8F9
	dc.l	$F9FAFBFC
	dc.l	$FDFEFFFE
	dc.l	$FEFDFEFF
	dc.l	$10828
	dc.l	$5C3E1C1B
	dc.l	$B05FDF5
	dc.l	$F0EAE7E5
	dc.l	$E3E2E1E1
	dc.l	$E1E2E4E5
	dc.l	$E6E7E8E9
	dc.l	$EBEDF0F3
	dc.l	$F6F8FBFE
	dc.l	$20406
	dc.l	$6070708
	dc.l	$90B0D0E
	dc.l	$E0F0CF2
	dc.l	$B7CBF6F9
	dc.l	$911181F
	dc.l	$242B3250
	dc.l	$7474494A
	dc.l	$3930271B
	dc.l	$130A03FF
	dc.l	$FAF6F2EF
	dc.l	$EDECEBEA
	dc.l	$E9E8E7E6
	dc.l	$E5E5E6E7
	dc.l	$E7E8E9EA
	dc.l	$EBECEAD6
	dc.l	$9794CCD0
	dc.l	$E0EEF600
	dc.l	$70D1317
	dc.l	$1B1F2022
	dc.l	$2221201F
	dc.l	$1F1E1D1B
	dc.l	$1A181715
	dc.l	$14110E0A
	dc.l	$7040100
	dc.l	$FFFEFCFA
	dc.l	$F9F8F7F7
	dc.l	$F6F4F2F0
	dc.l	$EFEEEEEE
	dc.l	$EEEDEDED
	dc.l	$EEEEEEEF
	dc.l	$EFEFEDED
	dc.l	$EDEEEFF0
	dc.l	$F1F1F3F4
	dc.l	$F6F7F9FA
	dc.l	$FAFAFAFB
	dc.l	$FCFDFF00
	dc.l	$10304
	dc.l	$5070707
	dc.l	$6060910
	dc.l	$36623922
	dc.l	$1F100B02
	dc.l	$FDF7F0E6
	dc.l	$B881A0CA
	dc.l	$CCE1E9F5
	dc.l	$FF060F14
	dc.l	$191C1E20
	dc.l	$21232424
	dc.l	$25252424
	dc.l	$23232221
	dc.l	$201E1C19
	dc.l	$1817171A
	dc.l	$346F5827
	dc.l	$28160C02
	dc.l	$F8F0E8E2
	dc.l	$DEDAD8D5
	dc.l	$D3D2D2D3
	dc.l	$D5D6D7D8
	dc.l	$D9DADCDE
	dc.l	$E1E4E7E9
	dc.l	$ECEFF2F5
	dc.l	$F7F8F9FA
	dc.l	$FBFCFE00
	dc.l	$1020405
	dc.l	$7090A0A
	dc.l	$6EAAFC5
	dc.l	$F0F0020A
	dc.l	$121B2B68
	dc.l	$7451423E
	dc.l	$2C281D15
	dc.l	$E0703FF
	dc.l	$FAF7F3F1
	dc.l	$EEE7BD81
	dc.l	$ABCECFE3
	dc.l	$EBF60018
	dc.l	$6474302E
	dc.l	$24161207
	dc.l	$F9F4F0
	dc.l	$EDEAE7E5
	dc.l	$E4E4E5E5
	dc.l	$E5E5E5E5
	dc.l	$E4DFB781
	dc.l	$AED2D5EC
	dc.l	$F6010B12
	dc.l	$181B1E20
	dc.l	$22242526
	dc.l	$27262626
	dc.l	$25242220
	dc.l	$1D1A1815
	dc.l	$13110F0D
	dc.l	$B133F5F
	dc.l	$2A191200
	dc.l	$FAEFE8E1
	dc.l	$DBD7D4D1
	dc.l	$CFCECDCD
	dc.l	$CECDC9AC
	dc.l	$8181BABD
	dc.l	$D5E50961
	dc.l	$62262E22
	dc.l	$1B18110E
	dc.l	$A070604
	dc.l	$3010101
	dc.l	$2030405
	dc.l	$5050606
	dc.l	$708090A
	dc.l	$B0B0C0D
	dc.l	$F0F0F0E
	dc.l	$E0D0D0C
	dc.l	$C0C0C0C
	dc.l	$C0D0D0D
	dc.l	$D0BFEC2
	dc.l	$A7E3ECF6
	dc.l	$5091217
	dc.l	$1C202326
	dc.l	$27272625
	dc.l	$2422211F
	dc.l	$1D1A1816
	dc.l	$1412100E
	dc.l	$A070300
	dc.l	$FEFCFAF8
	dc.l	$F5F3F2F0
	dc.l	$F0EFEEEE
	dc.l	$F7283F09
	dc.l	$FFF5E6E1
	dc.l	$D8D3CECA
	dc.l	$C9C7C5C3
	dc.l	$C3C2C2C3
	dc.l	$C5C7CACD
	dc.l	$CFD3D7DB
	dc.l	$DFE1E4E4
	dc.l	$D69A8DCF
	dc.l	$D9EBFD05
	dc.l	$121A2329
	dc.l	$2E323437
	dc.l	$393A3B3C
	dc.l	$3B3A3938
	dc.l	$37353431
	dc.l	$2F2C2825
	dc.l	$23201D1B
	dc.l	$18161311
	dc.l	$F0D0B08
	dc.l	$50200FE
	dc.l	$FE032E55
	dc.l	$200D08F6
	dc.l	$F0E0AD81
	dc.l	$96B8B5C9
	dc.l	$CDD6DDE3
	dc.l	$EAF0F5FA
	dc.l	$FF02092B
	dc.l	$6B4B2023
	dc.l	$100A02FA
	dc.l	$F5F0ECE9
	dc.l	$E6E4E1DE
	dc.l	$D6A581A9
	dc.l	$C9CDE5ED
	dc.l	$FB060E16
	dc.l	$1A1E2124
	dc.l	$272A366C
	dc.l	$744D3E38
	dc.l	$2520140C
	dc.l	$4FDF8F3
	dc.l	$EFECEAE7
	dc.l	$E6E6E7E7
	dc.l	$E8E8E8E8
	dc.l	$E6D5968F
	dc.l	$CDD2E2F2
	dc.l	$F9050C14
	dc.l	$191C2023
	dc.l	$25272929
	dc.l	$29292928
	dc.l	$28272625
	dc.l	$294B7452
	dc.l	$2E2D180F
	dc.l	$3F9F2EB
	dc.l	$E6E1DDDA
	dc.l	$D7D4D1CE
	dc.l	$C18781A8
	dc.l	$B7C2D5DD
	dc.l	$EBF4FC02
	dc.l	$70B0F12
	dc.l	$14161718
	dc.l	$18191A1A
	dc.l	$19191716
	dc.l	$15141312
	dc.l	$110F0E0E
	dc.l	$D0D0D0C
	dc.l	$A090705
	dc.l	$4020100
	dc.l	$FFFEFDFC
	dc.l	$FBFBFAF9
	dc.l	$F8F7F6F6
	dc.l	$F6F6F6F6
	dc.l	$F5F5F6F6
	dc.l	$F7F7F8F8
	dc.l	$F8F8F9F9
	dc.l	$FAFBFD0A
	dc.l	$42551E19
	dc.l	$F02FEF4
	dc.l	$EEE9E4E1
	dc.l	$DEDCDBDA
	dc.l	$DADBDDDF
	dc.l	$E0E2E3E6
	dc.l	$E8EBEDF0
	dc.l	$F2F5F8FB
	dc.l	$FD000205
	dc.l	$7090A0C
	dc.l	$D0F1011
	dc.l	$12141516
	dc.l	$17181919
	dc.l	$19191818
	dc.l	$18181817
	dc.l	$16161515
	dc.l	$15141311
	dc.l	$100F0E0D
	dc.l	$C0B0909
	dc.l	$8070605
	dc.l	$5040302
	dc.l	$10000FF
	dc.l	$FFFEFDFC
	dc.l	$FBFAF8E4
	dc.l	$A5A3D9D9
	dc.l	$E8F3F902
	dc.l	$60C1013
	dc.l	$16181A1A
	dc.l	$1A1A1918
	dc.l	$19215171
	dc.l	$33211906
	dc.l	$F3EAE1
	dc.l	$D4A5818F
	dc.l	$B4B2C6CB
	dc.l	$D4DDE4EC
	dc.l	$F1F6FAFD
	dc.l	$10304
	dc.l	$5050505
	dc.l	$5050505
	dc.l	$5050504
	dc.l	$4061345
	dc.l	$511F1A0B
	dc.l	$E192A5D8
	dc.l	$CFE0E6EB
	dc.l	$F3F7001B
	dc.l	$5D501D21
	dc.l	$BD08BC3
	dc.l	$E0DCF0F3
	dc.l	$FC00040A
	dc.l	$D101215
	dc.l	$17191B1B
	dc.l	$1B1C1C1B
	dc.l	$1B1A1A19
	dc.l	$18181716
	dc.l	$15131211
	dc.l	$100F0D0C
	dc.l	$B0A0908
	dc.l	$7070605
	dc.l	$4030201
	dc.l	$FF
	dc.l	$FEFDFCFC
	dc.l	$FCFBFBFB
	dc.l	$FBFBFBFD
	dc.l	$9342300
	dc.l	$14BFEA20
	dc.l	$D3E4A5E6
	dc.l	$141FD3E8
	dc.l	$DA042401
	dc.l	$F902DED3
	dc.l	$EDE212F3
	dc.l	$B8FE02BB
	dc.l	$D2EAF9B2
	dc.l	$3339F7F7
	dc.l	$210BDACC
	dc.l	$32414F5
	dc.l	$F2F63BD1
	dc.l	$7741BDE
	dc.l	$54DCCAD
	dc.l	$D133E11
	dc.l	$CF366B3C
	dc.l	$F2301D
	dc.l	$F72CF7DC
	dc.l	$CEFB3B12
	dc.l	$D2B30013
	dc.l	$1F2A252A
	dc.l	$BFDF044
	dc.l	$22DC0322
	dc.l	$1A0C11CB
	dc.l	$97E6B0CE
	dc.l	$10F902C9
	dc.l	$EB19FC36
	dc.l	$FE1022
	dc.l	$DAEB630D
	dc.l	$C404FA0A
	dc.l	$21CAC81F
	dc.l	$2BF7BCC2
	dc.l	$1D0D8C6
	dc.l	$BA402D05
	dc.l	$E5EFB9E3
	dc.l	$20DD50E
	dc.l	$3BB83B54
	dc.l	$CED0615
	dc.l	$96ED5A19
	dc.l	$CD1817DC
	dc.l	$371CE0F8
	dc.l	$3CC5BE11
	dc.l	$3809BFF4
	dc.l	$E515B10
	dc.l	$C8E5681E
	dc.l	$9AD91713
	dc.l	$E9EA0DE4
	dc.l	$6108E914
	dc.l	$CCFAB0BF
	dc.l	$47620EF1
	dc.l	$E61A1EB5
	dc.l	$F40EBD37
	dc.l	$44E7090F
	dc.l	$1AF583E4
	dc.l	$F7FE47EE
	dc.l	$B84809CC
	dc.l	$CDE82527
	dc.l	$2BC51F50
	dc.l	$BEE6C112
	dc.l	$66C5C31E
	dc.l	$5139FEEE
	dc.l	$1F5F805
	dc.l	$FFEECF2
	dc.l	$C3E71B34
	dc.l	$EE08E5
	dc.l	$560AB802
	dc.l	$E83100B2
	dc.l	$3A41170F
	dc.l	$F040ECFD
	dc.l	$1CDEF43B
	dc.l	$E6D90A03
	dc.l	$EC6EF05
	dc.l	$1AD30BC1
	dc.l	$FB7406C3
	dc.l	$E3FD0B33
	dc.l	$E6C4E6DE
	dc.l	$CB2EFFFE
	dc.l	$22D3ED3D
	dc.l	$C8190910
	dc.l	$49E106E7
	dc.l	$CBCDDF07
	dc.l	$103F581E
	dc.l	$F0FBDDA7
	dc.l	$EC0D1F66
	dc.l	$E78A1C22
	dc.l	$2F3F9ADD
	dc.l	$6414E01C
	dc.l	$1628E834
	dc.l	$2B42048
	dc.l	$F6121C1F
	dc.l	$CDD54F00
	dc.l	$DDC9BE3B
	dc.l	$ECA9C0EF
	dc.l	$21CE29DD
	dc.l	$DE691FE0
	dc.l	$F226E0EF
	dc.l	$382AB8D8
	dc.l	$62501CC
	dc.l	$24F711C9
	dc.l	$FDE6F244
	dc.l	$6C8E740
	dc.l	$3DD0E212
	dc.l	$7117E010
	dc.l	$F025EDA5
	dc.l	$4422A5E2
	dc.l	$18F00CF0
	dc.l	$21411119
	dc.l	$11220C6
	dc.l	$EFE8C853
	dc.l	$21AACB0D
	dc.l	$AB732FD
	dc.l	$218E9E1
	dc.l	$3A0689A2
	dc.l	$10310EBF
	dc.l	$176907EF
	dc.l	$122214A0
	dc.l	$CB42E7A6
	dc.l	$ED1A3638
	dc.l	$AD1E2CF9
	dc.l	$FFA36FE
	dc.l	$B41AFBC9
	dc.l	$F72D121E
	dc.l	$32C8DA5C
	dc.l	$3D01C515
	dc.l	$1509F295
	dc.l	$D51EF9F7
	dc.l	$2851E48C
	dc.l	$1E664106
	dc.l	$E31CFEE8
	dc.l	$14CEE14B
	dc.l	$EB0926D5
	dc.l	$C7AED8FD
	dc.l	$27E21046
	dc.l	$EACB1C4F
	dc.l	$15D91905
	dc.l	$EACEEC
	dc.l	$310EE65D
	dc.l	$561CF7F7
	dc.l	$180908C1
	dc.l	$150BE513
	dc.l	$3F189B1
	dc.l	$4C40F8AB
	dc.l	$83E1F605
	dc.l	$F61313C5
	dc.l	$F15374F6
	dc.l	$A2F26509
	dc.l	$962834D5
	dc.l	$35F3E448
	dc.l	$2AEBE430
	dc.l	$102AF9BE
	dc.l	$11CEE874
	dc.l	$FD8EC13D
	dc.l	$F6EF13D6
	dc.l	$22FDF1DA
	dc.l	$3B25AE33
	dc.l	$FDE612A9
	dc.l	$2D078432
	dc.l	$2CE4EF3F
	dc.l	$4CDBB32F
	dc.l	$36DED5D2
	dc.l	$D91000D9
	dc.l	$1E224505
	dc.l	$94DEED2
	dc.l	$CD1F36D4
	dc.l	$D005341E
	dc.l	$EEF8DFFB
	dc.l	$6303A37
	dc.l	$3C3E71A
	dc.l	$7342CE7
	dc.l	$C8CDE700
	dc.l	$10E5242A
	dc.l	$AD1C24CE
	dc.l	$DBF22A29
	dc.l	$BF2D5604
	dc.l	$B70016B0
	dc.l	$DF2C24E5
	dc.l	$EA035E02
	dc.l	$B0EEE6C4
	dc.l	$FD2F0EBA
	dc.l	$CD001A39
	dc.l	$E9CC1E44
	dc.l	$23E93411
	dc.l	$E528E008
	dc.l	$13126FD
	dc.l	$705270B
	dc.l	$CDB4EEF1
	dc.l	$23ECEFEC
	dc.l	$A0D52D5E
	dc.l	$DC4B5F8
	dc.l	$F400393F
	dc.l	$32E6E103
	dc.l	$F20420D8
	dc.l	$FB105D10
	dc.l	$B6C0F016
	dc.l	$2503E714
	dc.l	$94629F5
	dc.l	$F6D1DBFB
	dc.l	$2326E319
	dc.l	$15172223
	dc.l	$D3AEFB0F
	dc.l	$E6E8341E
	dc.l	$AF74927
	dc.l	$D3CEF1FF
	dc.l	$E1263FF2
	dc.l	$B8EBF6D1
	dc.l	$E6211E04
	dc.l	$3DC07
	dc.l	$1A0FFFEF
	dc.l	$FAF32C05
	dc.l	$CEF34810
	dc.l	$F805B8E7
	dc.l	$E0F84623
	dc.l	$BE021BB9
	dc.l	$E52928E5
	dc.l	$13ECC109
	dc.l	$14270CF8
	dc.l	$DE09080C
	dc.l	$28D7FD37
	dc.l	$FA3F2AFC
	dc.l	$3CCCE43
	dc.l	$16D8D9D5
	dc.l	$2E0FB2D9
	dc.l	$FDDA0838
	dc.l	$DC0721D6
	dc.l	$3A27E4E5
	dc.l	$CFD3052E
	dc.l	$DCE13C46
	dc.l	$15D4DE2D
	dc.l	$42F0FFF3
	dc.l	$DAFFF83A
	dc.l	$30152517
	dc.l	$E21325C7
	dc.l	$BEF31EEB
	dc.l	$CD423EDD
	dc.l	$DB0FCFD1
	dc.l	$103D32CA
	dc.l	$DCF3FC2B
	dc.l	$27FD0F3A
	dc.l	$DE1303D7
	dc.l	$F81204EC
	dc.l	$ECBF3E20
	dc.l	$D51E210F
	dc.l	$F2D0D300
	dc.l	$D6F6FAD3
	dc.l	$121021D6
	dc.l	$F611C9CF
	dc.l	$818F42E
	dc.l	$4DDFE315
	dc.l	$2D2C00DB
	dc.l	$FB1D0BE8
	dc.l	$E1DD815
	dc.l	$CFEE41EE
	dc.l	$ED6D33A
	dc.l	$F02ECB
	dc.l	$F530022C
	dc.l	$2919EFEC
	dc.l	$2AE4E5E7
	dc.l	$29F3CBE1
	dc.l	$F020042D
	dc.l	$8EB0CF7
	dc.l	$1C0DE0CC
	dc.l	$1813F714
	dc.l	$19E6D113
	dc.l	$DFC00721
	dc.l	$DFEEE25
	dc.l	$13B9E3E9
	dc.l	$FA1F2212
	dc.l	$BF3FC3F
	dc.l	$EDD73702
	dc.l	$F038EEFF
	dc.l	$1310E50B
	dc.l	$12DBEBF6
	dc.l	$19101110
	dc.l	$E52DF5E5
	dc.l	$2A0C09E0
	dc.l	$B0DE4DE
	dc.l	$1F1C06E4
	dc.l	$C4FB22C0
	dc.l	$FD40D9CA
	dc.l	$30CEAD7
	dc.l	$129F2E8
	dc.l	$F83CE7
	dc.l	$F835F3C9
	dc.l	$EFFB2E28
	dc.l	$EB0603F0
	dc.l	$ECC7FD00
	dc.l	$A3633ED
	dc.l	$FE3EF8D6
	dc.l	$E41DF8E2
	dc.l	$ECF22200
	dc.l	$E73601FC
	dc.l	$41F3040E
	dc.l	$D31F36FA
	dc.l	$D1FB4016
	dc.l	$EE11EBDB
	dc.l	$22F00B32
	dc.l	$F0F9DD
	dc.l	$F41A14D7
	dc.l	$2A16D7DC
	dc.l	$7EEE10E
	dc.l	$DA351BB6
	dc.l	$FC2219EF
	dc.l	$181DCDD5
	dc.l	$1610F9EE
	dc.l	$E801180E
	dc.l	$14FF250A
	dc.l	$DBF133E6
	dc.l	$E028FF0F
	dc.l	$1E15FDDA
	dc.l	$AF5F60C
	dc.l	$C0CCE2E8
	dc.l	$140FD0F3
	dc.l	$16290408
	dc.l	$19D9F42F
	dc.l	$DD72411
	dc.l	$D12D05D8
	dc.l	$1F1E0904
	dc.l	$FCDE12E4
	dc.l	$5F20532
	dc.l	$D0ED3E10
	dc.l	$DA1028D1
	dc.l	$F20DB0F
	dc.l	$E8F10DDF
	dc.l	$2913D716
	dc.l	$EEB1810
	dc.l	$EAE2E800
	dc.l	$D81338FB
	dc.l	$FA0BCFF7
	dc.l	$24DBC6F3
	dc.l	$C07EBEF
	dc.l	$2126E009
	dc.l	$3CDD1050
	dc.l	$F6E02C2C
	dc.l	$ED0D1707
	dc.l	$DE7F6FA
	dc.l	$C8E116FE
	dc.l	$F808E90D
	dc.l	$F910F9F7
	dc.l	$2217EBF1
	dc.l	$FBFC200E
	dc.l	$DCD7ECEA
	dc.l	$F115251F
	dc.l	$8F20A02
	dc.l	$F412D4E2
	dc.l	$1611D9EE
	dc.l	$1E0F1416
	dc.l	$8EA191B
	dc.l	$EBE5180F
	dc.l	$F3E4E304
	dc.l	$F505300D
	dc.l	$D82009F6
	dc.l	$1D1611EF
	dc.l	$EA1DFEE4
	dc.l	$EAFAFCED
	dc.l	$1324F7F2
	dc.l	$1E0AF605
	dc.l	$FFF0FAFE
	dc.l	$ECE904F5
	dc.l	$1B0BEAEB
	dc.l	$5FAF02A
	dc.l	$CE90106
	dc.l	$F3E10F17
	dc.l	$FDF1FF13
	dc.l	$20AF1E6
	dc.l	$1B2F0AE4
	dc.l	$333D8E8
	dc.l	$181E2B01
	dc.l	$F900E9DF
	dc.l	$F8F5081B
	dc.l	$9EAF62A
	dc.l	$D7FA1FF0
	dc.l	$8020811
	dc.l	$E4F3140E
	dc.l	$11170CF0
	dc.l	$D9F2EBDA
	dc.l	$9150BE8
	dc.l	$E6FD18F2
	dc.l	$F328EDBC
	dc.l	$F00D1F9
	dc.l	$251FF606
	dc.l	$20F3E90E
	dc.l	$3F3D6FE
	dc.l	$1107F019
	dc.l	$1CD3001A
	dc.l	$18F6EDFB
	dc.l	$FD1BE608
	dc.l	$11EC2306
	dc.l	$305F7
	dc.l	$F91A0DE9
	dc.l	$F407EC11
	dc.l	$2409E0E2
	dc.l	$FD202D04
	dc.l	$DBFC24F9
	dc.l	$1115FF19
	dc.l	$7FBFEF6
	dc.l	$E00AF4DC
	dc.l	$21FCD2FE
	dc.l	$290BE716
	dc.l	$F70200E6
	dc.l	$FE0811FC
	dc.l	$1F2ED1E
	dc.l	$15EAFBFE
	dc.l	$1F130007
	dc.l	$AE7FE13
	dc.l	$9EA150F
	dc.l	$F3F60019
	dc.l	$F3D3F6E6
	dc.l	$FC18E1F5
	dc.l	$FE01EB01
	dc.l	$16060711
	dc.l	$EE9FD0E
	dc.l	$E0DC0E0C
	dc.l	$E8F1E000
	dc.l	$30A2312
	dc.l	$CFCFFE6
	dc.l	$FA1105E9
	dc.l	$F6FAFA1A
	dc.l	$8F31821
	dc.l	$EF0326F3
	dc.l	$F0FF131B
	dc.l	$9140D1A
	dc.l	$F5EC1AEC
	dc.l	$1109F82D
	dc.l	$F9F90BCF
	dc.l	$C8ECF2F8
UbL002590	dc.l	$600
	dc.l	$142003F
	dc.l	UbL001990
UbL00259C	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL002590
UbL0025AA	dc.l	UbL00259C
	dc.l	0
	dc.l	0
	dc.l	$110A00
	dc.w	$FFFE

;	SECTION	UltSnd0025BC,DATA,CHIP
UbL0025BC	dc.l	$E0CF1420
	dc.l	$D4E32712
	dc.l	$7011E
	dc.l	$15070723
	dc.l	$240C0A1F
	dc.l	$1D242006
	dc.l	$F23131F
	dc.l	$2192601
	dc.l	$113D0A
	dc.l	$E2091313
	dc.l	$4EE0A0A
	dc.l	$EFFE1507
	dc.l	$E4052FE4
	dc.l	$E92F29E9
	dc.l	$1B1126
	dc.l	$FFF82D2B
	dc.l	$DFF03C0D
	dc.l	$E7FC0E00
	dc.l	$F7E70703
	dc.l	$E3EFFB00
	dc.l	$F7DBEA16
	dc.l	$BDBDA10
	dc.l	$18D4DD02
	dc.l	$FFDAE520
	dc.l	$ECE9FFF7
	dc.l	$BF80010
	dc.l	$FC110E09
	dc.l	$11319FE
	dc.l	$FEE21024
	dc.l	$C1F029E6
	dc.l	$CF0215E7
	dc.l	$D8ED080E
	dc.l	$D7D22A28
	dc.l	$CAC62A3E
	dc.l	$E1D0212E
	dc.l	$EDE0049
	dc.l	$20DCFA4A
	dc.l	$21DA0845
	dc.l	$3EA101D
	dc.l	$16E6DD27
	dc.l	$25D0C04C
	dc.l	$31A5D12F
	dc.l	$39E3B708
	dc.l	$3EEFB50D
	dc.l	$2503C8E9
	dc.l	$233E0AA
	dc.l	$212AB1BB
	dc.l	$4BFEC4F8
	dc.l	$CF110DBD
	dc.l	$AC2315B9
	dc.l	$C4F831F9
	dc.l	$B1DC3C48
	dc.l	$BDDF275A
	dc.l	$F0BB414A
	dc.l	$D4BF1442
	dc.l	$DBE53B
	dc.l	$23C5F144
	dc.l	$9B8E024
	dc.l	$2AB7C12B
	dc.l	$29A5AA55
	dc.l	$1481CD4D
	dc.l	$DF96EC18
	dc.l	$D1D0DDD1
	dc.l	$26DEC4EC
	dc.l	$E05DA0C
	dc.l	$AE3FB18
	dc.l	$20F7F2FD
	dc.l	$1D02ED0A
	dc.l	$2225DAFB
	dc.l	$3C48C4CD
	dc.l	$5102C7E0
	dc.l	$1F12B4D3
	dc.l	$810F9BB
	dc.l	$E437FBB8
	dc.l	$B2313C2
	dc.l	$30321226
	dc.l	$1C362032
	dc.l	$40293437
	dc.l	$2C3C5CF3
	dc.l	$535CE71D
	dc.l	$6240DA1C
	dc.l	$462318E4
	dc.l	$452911D8
	dc.l	$2B5DD7CA
	dc.l	$2D4E02D1
	dc.l	$F57A17AE
	dc.l	$10614B93
	dc.l	$EA4E38D7
	dc.l	$D6F20019
	dc.l	$99E127F2
	dc.l	$81004C81
	dc.l	$D319B8B6
	dc.l	$DFEDBDEA
	dc.l	$92B908D9
	dc.l	$C1CDDCFB
	dc.l	$FC812422
	dc.l	$92A31BFA
	dc.l	$A704DDD4
	dc.l	$F195E405
	dc.l	$789933C
	dc.l	$38FE4F1
	dc.l	$ECC78C3A
	dc.l	$AC2B70C
	dc.l	$24C7FEE6
	dc.l	$1000D3E9
	dc.l	$4961B8C6
	dc.l	$6876EA01
	dc.l	$57481D2A
	dc.l	$50476B0E
	dc.l	$17685A24
	dc.l	$267C1125
	dc.l	$51645BFF
	dc.l	$3B3B303B
	dc.l	$2A161228
	dc.l	$EA0A231A
	dc.l	$B0E642C5
	dc.l	$FC020112
	dc.l	$B5EC1F0C
	dc.l	$C5B317AE
	dc.l	$D5C60BF9
	dc.l	$81C6E7F5
	dc.l	$CECEABF0
	dc.l	$5812635
	dc.l	$81C42D2C
	dc.l	$AC007CAD
	dc.l	$F14019DF
	dc.l	$3B29D8F1
	dc.l	$2F321C00
	dc.l	$B61A4FF9
	dc.l	$860263B4
	dc.l	$811B7181
	dc.l	$813FFF99
	dc.l	$D7FFFCDA
	dc.l	$F0D20234
	dc.l	$FAC1FE57
	dc.l	$D5F039FA
	dc.l	$D129FF1C
	dc.l	$21E8E30B
	dc.l	$58D3E10D
	dc.l	$F127E8E2
	dc.l	$5A03E9EE
	dc.l	$5924FF2E
	dc.l	$1E141613
	dc.l	$210D0DF7
	dc.l	$FB19FB0B
	dc.l	$1DE51FEA
	dc.l	$293607CE
	dc.l	$2058D017
	dc.l	$57C9087C
	dc.l	$ACE67C03
	dc.l	$97ED7103
	dc.l	$D8EC1902
	dc.l	$DFC711E9
	dc.l	$FF978153
	dc.l	$3381CB72
	dc.l	$9B8F02F6
	dc.l	$EAFF90E4
	dc.l	$2E029BDA
	dc.l	$170AABC2
	dc.l	$1204B9AC
	dc.l	$143DA40D
	dc.l	$21FDF231
	dc.l	$3FDD1616
	dc.l	$19160DFF
	dc.l	$F14DD9B8
	dc.l	$5716A2F0
	dc.l	$77B5CA1F
	dc.l	$1524AEAF
	dc.l	$20728993
	dc.l	$4739C391
	dc.l	$2C76FB81
	dc.l	$F57CFC89
	dc.l	$17C259B
	dc.l	$116A6DBD
	dc.l	$A57C6DD3
	dc.l	$C8456DFE
	dc.l	$AC687CDC
	dc.l	$E91C6B39
	dc.l	$B6E37CFB
	dc.l	$834B37A1
	dc.l	$EF1AE715
	dc.l	$EDE90A27
	dc.l	$C4EE2615
	dc.l	$1CB1651
	dc.l	$4A30651
	dc.l	$3CECA6B
	dc.l	$3ECCD3E
	dc.l	$43B2013B
	dc.l	$1AE2C15E
	dc.l	$27A0E641
	dc.l	$1B99D00A
	dc.l	$17D3C2E8
	dc.l	$2707A60B
	dc.l	$F0EFDDD6
	dc.l	$FC29D6C4
	dc.l	$BB23458E
	dc.l	$C42C1AD1
	dc.l	$CEEF29E2
	dc.l	$AEEA0EFC
	dc.l	$D84FB6D
	dc.l	$CBA61F2B
	dc.l	$A9E214FC
	dc.l	$A8BED7C
	dc.l	$A1817CF7
	dc.l	$D708E7FA
	dc.l	$34DDAF39
	dc.l	$26E6F8F1
	dc.l	$2B20D233
	dc.l	$FDF3430C
	dc.l	$BE0579C1
	dc.l	$D848E8F4
	dc.l	$51E0CE5A
	dc.l	$3AD7FB5B
	dc.l	$1FC2791D
	dc.l	$DA3642F2
	dc.l	$F739F51E
	dc.l	$37E3DB49
	dc.l	$CECE714
	dc.l	$F09BE16
	dc.l	$44DCA228
	dc.l	$5ECBAC26
	dc.l	$56D0AC35
	dc.l	$55E7A128
	dc.l	$300FB6CE
	dc.l	$540CB2E4
	dc.l	$EF2ADBE8
	dc.l	$F6DF0D00
	dc.l	$BCEEF9F6
	dc.l	$D79EE8F2
	dc.l	$21A58170
	dc.l	$2081FE7C
	dc.l	$C7A7551F
	dc.l	$F6F81220
	dc.l	$E62627C4
	dc.l	$172FD0EF
	dc.l	$640B9611
	dc.l	$7CA1D455
	dc.l	$CF0B0E
	dc.l	$2C3E91EA
	dc.l	$7CEC8155
	dc.l	$7C85867C
	dc.l	$46CEAF30
	dc.l	$49F6A8FD
	dc.l	$7CB28155
	dc.l	$74819B7C
	dc.l	$F684CF4C
	dc.l	$1BAEAE51
	dc.l	$38A9F836
	dc.l	$1AD6F3
	dc.l	$6DFB8E33
	dc.l	$5EE3A829
	dc.l	$67C3BB3C
	dc.l	$47F8B100
	dc.l	$7CBABF27
	dc.l	$FE04C6E3
	dc.l	$290CB2F6
	dc.l	$3701C116
	dc.l	$FAFA17E4
	dc.l	$E23E3E93
	dc.l	$F87CD2C7
	dc.l	$1A3BE3AE
	dc.l	$2044CEE0
	dc.l	$51510A3
	dc.l	$D2DEDC6
	dc.l	$A12FFFB
	dc.l	$EE0039DB
	dc.l	$A25D2EA0
	dc.l	$FF67ECDA
	dc.l	$43400011
	dc.l	$2F26F61D
	dc.l	$E3DC2AFF
	dc.l	$AB0621DA
	dc.l	$E6FAFEE9
	dc.l	$FFE1E611
	dc.l	$FFC7D314
	dc.l	$13AFF107
	dc.l	$ECDAE002
	dc.l	$6C5E302
	dc.l	$1EDFC11E
	dc.l	$BFBDD06
	dc.l	$2EE9DF26
	dc.l	$11E01C0E
	dc.l	$EF1C03ED
	dc.l	$2F24D600
	dc.l	$42FCE41A
	dc.l	$1900EC1C
	dc.l	$BF3091A
	dc.l	$F9EE290C
	dc.l	$E10B1FF8
	dc.l	$213F3FE
	dc.l	$2707DC1E
	dc.l	$35FBEC46
	dc.l	$21CE2C3E
	dc.l	$F2042AF6
	dc.l	$160CEB15
	dc.l	$DED0504
	dc.l	$F5EFFFF9
	dc.l	$E3F3FFEC
	dc.l	$D9F501D8
	dc.l	$EAFCE8EC
	dc.l	$F5F6F2F8
	dc.l	$EFE9F2F5
	dc.l	$E1DAF7EB
	dc.l	$C5E20BD4
	dc.l	$A4DC16FC
	dc.l	$E00D0BE0
	dc.l	$EF1CF9D9
	dc.l	$F90AE9C4
	dc.l	$E40BEBCB
	dc.l	$422F3EF
	dc.l	$283F0A04
	dc.l	$555B002A
	dc.l	$7C29FE5A
	dc.l	$57E8FF62
	dc.l	$F681FE23
	dc.l	$8FB70B0A
	dc.l	$DCE9FBFF
	dc.l	$2020104
	dc.l	$5050607
	dc.l	$5080500
UbL002ABC	dc.l	$280
	dc.l	$142003F
	dc.l	UbL0025BC
UbL002AC8	dc.l	0
	dc.l	0
UbL002AD0	dc.l	4
	dc.l	$142003F
	dc.l	UbL002AC8
UbL002ADC	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL002ABC
UbL002AEA	dc.l	UbL002ADC
	dc.l	$320000
	dc.w	0
	dc.l	UbL002AD0
UbL002AF8	dc.l	UbL002AEA
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B06	dc.l	UbL002AF8
	dc.l	$320000
	dc.w	0
	dc.l	UbL002AD0
UbL002B14	dc.l	UbL002B06
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B22	dc.l	UbL002B14
	dc.l	$1F40000
	dc.w	0
	dc.l	UbL002AD0
UbL002B30	dc.l	UbL002B22
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B3E	dc.l	UbL002B30
	dc.l	$320000
	dc.w	0
	dc.l	UbL002AD0
UbL002B4C	dc.l	UbL002B3E
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B5A	dc.l	UbL002B4C
	dc.l	$320000
	dc.w	0
	dc.l	UbL002AD0
UbL002B68	dc.l	UbL002B5A
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B76	dc.l	UbL002B68
	dc.l	$4B00000
	dc.w	0
	dc.l	UbL002AD0
UbL002B84	dc.l	UbL002B76
	dc.l	$10000
	dc.w	0
	dc.l	UbL002ABC
UbL002B92	dc.l	UbL002B84
	dc.l	0
	dc.l	0
	dc.l	$8C0A00
	dc.w	$FFFE

;	SECTION	UltSnd002BA4,DATA,CHIP
UbL002BA4	dc.l	0
	dc.l	0
	dc.l	$FF
	dc.l	$FFFFFFFF
	dc.l	$FFFFFFFF
	dc.l	$FFFF0005
	dc.l	$60300FF
	dc.l	$10302
	dc.l	$FF0008
	dc.l	$160FFCF8
	dc.l	$60603
	dc.l	$3020100
	dc.l	0
	dc.l	$FFFFFEFD
	dc.l	$FDFCFBFB
	dc.l	$FCFCFBFC
	dc.l	$FCFCFCFC
	dc.l	$FCFCFCFC
	dc.l	$FBFBFCFC
	dc.l	$FBFBFBFB
	dc.l	$FBFBFCFC
	dc.l	$FBFBFCFD
	dc.l	$FCFCFCFD
	dc.l	$FDFDFDFE
	dc.l	$FF000000
	dc.l	$2040000
	dc.l	$4030003
	dc.l	$4010103
	dc.l	$2000201
	dc.l	0
	dc.l	0
	dc.l	$FFFFFF
	dc.l	$FFFFFFFF
	dc.l	0
	dc.l	$1FF0102
	dc.l	$1040505
	dc.l	$80F1409
	dc.l	$5050507
	dc.l	$8070703
	dc.l	$2010000
	dc.l	$2000002
	dc.l	0
	dc.l	$2FF
	dc.l	$1000002
	dc.l	$20001
	dc.l	$2000000
	dc.l	$1000000
	dc.l	$1010100
	dc.l	$FF02
	dc.l	$100
	dc.l	0
	dc.l	$FF
	dc.l	0
	dc.l	$FF0000FF
	dc.l	$FF00
	dc.l	$FF0000
	dc.l	0
	dc.l	$FFFE
	dc.l	$FF00FF00
	dc.l	$FFFEFFFE
	dc.l	$FFFFFEFF
	dc.l	$FEFFFF
	dc.l	$FFFFFEFE
	dc.l	$FEFFFFFD
	dc.l	$FEFDFF
	dc.l	$10100
	dc.l	$90B02
	dc.l	$FE0102
	dc.l	$F7F9F8F7
	dc.l	$FBF1F2F5
	dc.l	$F4F9F2F5
	dc.l	$FBFAF9FC
	dc.l	$FAFB01FD
	dc.l	$FD00FE04
	dc.l	$FCFC01FD
	dc.l	$2F9FDFC
	dc.l	$F1E4CF02
	dc.l	$37E10847
	dc.l	$21240E20
	dc.l	$3A15080E
	dc.l	$1010FCF4
	dc.l	$FFFDF5EC
	dc.l	$EFF7F0EE
	dc.l	$F0F5F4F2
	dc.l	$F5F5F6F6
	dc.l	$F6F8F8FA
	dc.l	$F9F5F9FB
	dc.l	$FAF8F700
	dc.l	$FBF4FCFE
	dc.l	$FBFCFC00
	dc.l	$FF03FF
	dc.l	$FF010000
	dc.l	$FFFF0000
	dc.l	$1000107
	dc.l	$9090707
	dc.l	$A050706
	dc.l	$4070000
	dc.l	$700FC00
	dc.l	$FFFCFCFD
	dc.l	$FAFB00
	dc.l	3
	dc.l	$4FF01FC
	dc.l	$FD01FF00
	dc.l	$7090707
	dc.l	$B0B0607
	dc.l	$50002FF
	dc.l	$1FD00
	dc.l	$FEF9FFFE
	dc.l	$FD060801
	dc.l	$9070404
	dc.l	$200F9F6
	dc.l	$FBF7F503
	dc.l	$FC0102
	dc.l	$FD0103FE
	dc.l	$204FE01
	dc.l	$5020005
	dc.l	$A06060D
	dc.l	$7091208
	dc.l	$60B0904
	dc.l	$3050204
	dc.l	$FFF9FCFD
	dc.l	$FBF4FC
	dc.l	$FFF5F600
	dc.l	$FFFB00FF
	dc.l	$FDFFF9FC
	dc.l	$200FE02
	dc.l	$1FD0001
	dc.l	$FD00FE
	dc.l	$FD01
	dc.l	$5000004
	dc.l	$4010506
	dc.l	$3000505
	dc.l	$101
	dc.l	$1000006
	dc.l	$FDFE04FF
	dc.l	$F8FA00F7
	dc.l	$F1FCFCF3
	dc.l	$F1F5F1F5
	dc.l	$702FD0B
	dc.l	$D02080D
	dc.l	$90D0A05
	dc.l	$900FD00
	dc.l	$FDF7FC
	dc.l	$FBF4F6F6
	dc.l	$F4F6F4FA
	dc.l	$F7F5FCFC
	dc.l	$F7F9FC00
	dc.l	$FBF9FE
	dc.l	$FAF8FF
	dc.l	$FEF8F7FC
	dc.l	$FEFB00FC
	dc.l	$F40100F6
	dc.l	$F700
	dc.l	$2FF0606
	dc.l	$FE040B09
	dc.l	$50A110B
	dc.l	$5080B0A
	dc.l	$A010309
	dc.l	$2F9FA00
	dc.l	$2F8F704
	dc.l	$FFF80005
	dc.l	$500030A
	dc.l	$4000608
	dc.l	$20302FF
	dc.l	$FCFB
	dc.l	$FBF9F6EF
	dc.l	$F3FAFCF3
	dc.l	$F2FE0503
	dc.l	$FCFE080C
	dc.l	$806060B
	dc.l	$900FE00
	dc.l	$F7F7F8F4
	dc.l	$F1F2F2F0
	dc.l	$EAE9F1F6
	dc.l	$F5F80511
	dc.l	$12121214
	dc.l	$1B16161C
	dc.l	$130C140C
	dc.l	$FC0300F3
	dc.l	$FCFAF1F3
	dc.l	$F8EFF1F9
	dc.l	$EBEEF9F1
	dc.l	$EEFCFAF2
	dc.l	$F5FAFFFC
	dc.l	$404FB00
	dc.l	$B08FA02
	dc.l	$D0BFEFC
	dc.l	$201FD
	dc.l	$F6F8FBF5
	dc.l	$DECBCDE0
	dc.l	$B1C1F3E
	dc.l	$43333945
	dc.l	$3E322722
	dc.l	$201F1513
	dc.l	$1B161407
	dc.l	$60EFDF3
	dc.l	$FFF6E7E8
	dc.l	$E8E8E4DD
	dc.l	$DCE0E0DD
	dc.l	$DFE5DEDF
	dc.l	$E1DBDAE5
	dc.l	$F1F7F4EE
	dc.l	$ED060AFA
	dc.l	$F2FC0408
	dc.l	$F8F3FCFC
	dc.l	$304FCF9
	dc.l	$7FD02
	dc.l	$8FBFA05
	dc.l	$5010C32
	dc.l	$411BFB16
	dc.l	$38351B10
	dc.l	$26382D0F
	dc.l	$10232314
	dc.l	$FA0604
	dc.l	$ECE5EAF4
	dc.l	$F1E1DCE3
	dc.l	$E3DAD2D0
	dc.l	$D9E1D1CB
	dc.l	$CDC8BBA5
	dc.l	$97BD0D21
	dc.l	$E2FF3D
	dc.l	$57352C49
	dc.l	$573A232B
	dc.l	$30271A0E
	dc.l	$11170F0B
	dc.l	$B0A0F0E
	dc.l	$FEF7F1E1
	dc.l	$DA0E3103
	dc.l	$B1A6D9F2
	dc.l	$D2C8F205
	dc.l	$EDD8EC01
	dc.l	$FEF4F6F9
	dc.l	$F2EFECDF
	dc.l	$DE03352F
	dc.l	$5F0082A
	dc.l	$1CF1EF02
	dc.l	$3F4E5EC
	dc.l	$F7F0E2E9
	dc.l	$F2F4F4F4
	dc.l	$FE070708
	dc.l	$70E1714
	dc.l	$9FD0008
	dc.l	$ECD3CAE6
	dc.l	$1D58704E
	dc.l	$1D155173
	dc.l	$48100C3C
	dc.l	$4C27FFFB
	dc.l	$D1807E8
	dc.l	$CCD0E0DC
	dc.l	$C1BABEC0
	dc.l	$C0C7D6C6
	dc.l	$9FAC357F
	dc.l	$41EDEA5D
	dc.l	$7F0AD4EE
	dc.l	$1C24FEE8
	dc.l	$DDE4ED03
	dc.l	$5E6DCEA
	dc.l	$1E0C8
	dc.l	$C3C5D40A
	dc.l	$566B451C
	dc.l	$34707F5D
	dc.l	$392A3D4C
	dc.l	$3710F4F6
	dc.l	$F9FCEDD0
	dc.l	$C2C1D2D5
	dc.l	$BFB0B0C0
	dc.l	$D8DABFAD
	dc.l	$AFC2CBC1
	dc.l	$B1A09796
	dc.l	$9BA9B7C0
	dc.l	$BECCFA39
	dc.l	$52381516
	dc.l	$343B0AD8
	dc.l	$F3567F7F
	dc.l	$523D597C
	dc.l	$7F66401D
	dc.l	$2247625F
	dc.l	$43211320
	dc.l	$3018E4B6
	dc.l	$B0D7F7E6
	dc.l	$AA82829A
	dc.l	$B6DA0B35
	dc.l	$2BF4C8C5
	dc.l	$E513343F
	dc.l	$370AE3C4
	dc.l	$D2134A49
	dc.l	$3E383F2D
	dc.l	$F5C9B9AD
	dc.l	$B6077F7F
	dc.l	$53F6BC32
	dc.l	$7F41ECAC
	dc.l	$B1E7FD00
	dc.l	$D098ABFC
	dc.l	$4345F9A4
	dc.l	$84A3EA12
	dc.l	$ECAEA2C2
	dc.l	$F000FAEF
	dc.l	$DEEF0515
	dc.l	$CFAEAD8
	dc.l	$CDD2E9E7
	dc.l	$D4CBCFE2
	dc.l	$E4E6F7EB
	dc.l	$DAD2E000
	dc.l	$204A5331
	dc.l	$50C3E61
	dc.l	$532E1106
	dc.l	$E222815
	dc.l	$EDDCDDEF
	dc.l	$90F09F6
	dc.l	$DCDBD2DA
	dc.l	$DCE70515
	dc.l	$1B14131F
	dc.l	$3852675D
	dc.l	$483C394A
	dc.l	$4B545D5A
	dc.l	$53423830
	dc.l	$2C291902
	dc.l	$F1F40D1D
	dc.l	$4A716D50
	dc.l	$3A28C9D
	dc.l	$E7487E5D
	dc.l	$2858282
	dc.l	$CF2B6B6A
	dc.l	$3FEEADAA
	dc.l	$D8F9FCED
	dc.l	$C3998DA6
	dc.l	$C2C8BEBC
	dc.l	$BFB9A39A
	dc.l	$ACDD1C3F
	dc.l	$3A1CFDFB
	dc.l	$8273C3A
	dc.l	$290CFEFF
	dc.l	$80B03F4
	dc.l	$DBCBC5D1
	dc.l	$DBD4C2B1
	dc.l	$A7AAAAAE
	dc.l	$AFACA89D
	dc.l	$9CA6B6C4
	dc.l	$C4C0BCBF
	dc.l	$CEE6F5F4
	dc.l	$E6D4CFD6
	dc.l	$EB05140F
	dc.l	$F8000D
	dc.l	$16191502
	dc.l	$E6D4D804
	dc.l	$577F7F77
	dc.l	$52363658
	dc.l	$7F7F7F7F
	dc.l	$7F7F7F7C
	dc.l	$7874716E
	dc.l	$6B686560
	dc.l	$5549453E
	dc.l	$2E15EFC9
	dc.l	$A68F9297
	dc.l	$A09894BE
	dc.l	$E81C1E0C
	dc.l	$FDDAD7CC
	dc.l	$CEC7B7AD
	dc.l	$A7A49383
	dc.l	$82858F92
	dc.l	$9BA6C6E3
	dc.l	$F70A1C21
	dc.l	$1C141D2C
	dc.l	$353E4550
	dc.l	$53555B5E
	dc.l	$5B5C5E64
	dc.l	$6362666D
	dc.l	$736F6658
	dc.l	$45331E0C
	dc.l	$1EFD5B6
	dc.l	$94828282
	dc.l	$82828282
	dc.l	$82828589
	dc.l	$8C9199A8
	dc.l	$BBC5C6C2
	dc.l	$B4ADB3BC
	dc.l	$C9D5E8F8
	dc.l	$614273D
	dc.l	$4F5F6C74
	dc.l	$7576787B
	dc.l	$7A776E63
	dc.l	$524B4D5B
	dc.l	$6B6B634E
	dc.l	$3215FCF4
	dc.l	$EFF1F3ED
	dc.l	$E6DFE0E3
	dc.l	$E2E9F001
	dc.l	$14273028
	dc.l	$1C09F5ED
	dc.l	$EEFE050B
	dc.l	$151B2222
	dc.l	$21242A39
	dc.l	$474D4D4C
	dc.l	$4E4F4F4A
	dc.l	$402F18FE
	dc.l	$E3CFC2BA
	dc.l	$AEA4978B
	dc.l	$828392AA
	dc.l	$CEEF0410
	dc.l	$E0A0403
	dc.l	$400FCF4
	dc.l	$E8DED4CF
	dc.l	$CCC9CDD2
	dc.l	$D9E1E9F1
	dc.l	$F4F0ECEB
	dc.l	$E5DCD4D0
	dc.l	$CAC6C4C5
	dc.l	$C2C7D1DD
	dc.l	$E8F6091A
	dc.l	$2A384347
	dc.l	$4743403A
	dc.l	$35343333
	dc.l	$33302F2A
	dc.l	$231B1413
	dc.l	$F0B05FE
	dc.l	$F1E5D7CC
	dc.l	$C1BAB7AE
	dc.l	$A8A09D9D
	dc.l	$999A9892
	dc.l	$8682848B
	dc.l	$95B0DD0E
	dc.l	$3B525646
	dc.l	$2F1603FF
	dc.l	$9213F61
	dc.l	$7B7F7F7D
	dc.l	$78716B63
	dc.l	$5D565150
	dc.l	$4E493F32
	dc.l	$1F06EBD6
	dc.l	$D1D2D7DD
	dc.l	$DCCFBDA3
	dc.l	$85828282
	dc.l	$82828388
	dc.l	$8E9FC3F1
	dc.l	$23475C5F
	dc.l	$533F2610
	dc.l	$1FAFC02
	dc.l	$B171E23
	dc.l	$26262622
	dc.l	$1E19120D
	dc.l	$F1B272C
	dc.l	$2D2C2928
	dc.l	$2B333D46
	dc.l	$4C4E4E49
	dc.l	$3F32200B
	dc.l	$F4DFCFC2
	dc.l	$BAB4AFA8
	dc.l	$A19E9D9E
	dc.l	$9F9F9FA1
	dc.l	$A5AFBCCA
	dc.l	$D8E7F808
	dc.l	$192A3E50
	dc.l	$616D716E
	dc.l	$6860574E
	dc.l	$463B3229
	dc.l	$2018100B
	dc.l	$600FAF4
	dc.l	$F0EDE9E5
	dc.l	$E1DEDBDA
	dc.l	$DBDDDEDE
	dc.l	$DAD8D8DC
	dc.l	$E3ECF500
	dc.l	$80F1213
	dc.l	$15191D20
	dc.l	$2221201E
	dc.l	$1D1B1A19
	dc.l	$18161411
	dc.l	$F0D0E0F
	dc.l	$10111212
	dc.l	$110F0C08
	dc.l	$401FFFB
	dc.l	$F6F1EDEC
	dc.l	$ECEDEFF2
	dc.l	$F3F2F0EF
	dc.l	$EFF0F1F1
	dc.l	$F0EEECEA
	dc.l	$E7E6E6E7
	dc.l	$E8E8E7E4
	dc.l	$E0E0E2E6
	dc.l	$EAECEDEE
	dc.l	$EEEEEDED
	dc.l	$EEEFF0F1
	dc.l	$F2F2F2F1
	dc.l	$F0F0EFF0
	dc.l	$F1F3F4F5
	dc.l	$F6F8FBFF
	dc.l	$2050505
	dc.l	$4040405
	dc.l	$80D1114
	dc.l	$13110E0B
	dc.l	$9080807
	dc.l	$5030200
	dc.l	$FEFCFB
	dc.l	$FBFBFBFC
	dc.l	$FCFCFDFE
	dc.l	1
	dc.l	$3050708
	dc.l	$90A0B0C
	dc.l	$D0F1010
	dc.l	$11121212
	dc.l	$1211100D
	dc.l	$C0C0D0F
	dc.l	$1111100D
	dc.l	$A080809
	dc.l	$9090908
	dc.l	$90A0B0C
	dc.l	$D0E0E0D
	dc.l	$B0A0A09
	dc.l	$7060505
	dc.l	$3010000
	dc.l	0
	dc.l	$10101
	dc.l	$FEFDFB
	dc.l	$FBFBFCFD
	dc.l	$FEFEFEFF
	dc.l	$10305
	dc.l	$7080706
	dc.l	$6070908
	dc.l	$8080706
	dc.l	$3010000
	dc.l	$1010000
	dc.l	$FEFCFCFC
	dc.l	$FCFCFCFC
	dc.l	$FCFCFCFC
	dc.l	$FBFBFCFD
	dc.l	$FDFCFCFB
	dc.l	$FBFBFCFD
	dc.l	$FDFDFDFE
	dc.l	$10204
	dc.l	$5050505
	dc.l	$6080909
	dc.l	$8080604
	dc.l	$3030302
	dc.l	$1FFFDF9
	dc.l	$F6F4F3F2
	dc.l	$F2F0EFEE
	dc.l	$EEEEF0F1
	dc.l	$F3F4F5F7
	dc.l	$F8FAFCFD
	dc.l	$FEFFFEFF
	dc.l	$204
	dc.l	$5040302
	dc.l	$2010000
	dc.l	$FFFDFBF9
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
UbL0034A4	dc.l	$480
	dc.l	$142003F
	dc.l	UbL002BA4
UbL0034B0	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0034A4
UbL0034BE	dc.l	UbL0034B0
	dc.l	0
	dc.l	0
	dc.l	$D0A00
	dc.w	$FFFE

;	SECTION	UltSnd0034D0,DATA,CHIP
UbL0034D0	dc.l	$100
	dc.l	0
	dc.l	$FF
	dc.l	$FDF4C581
	dc.l	$DF696981
	dc.l	$A669691A
	dc.l	$2D2F28
	dc.l	$E981C169
	dc.l	$1C86DA09
	dc.l	$21D681
	dc.l	$833A1481
	dc.l	$81F9B981
	dc.l	$CDB9B7F7
	dc.l	$2D3CC34
	dc.l	$5BE6FB52
	dc.l	$1B0F5053
	dc.l	$2F48695C
	dc.l	$FE5B6931
	dc.l	$6769511E
	dc.l	$696900F1
	dc.l	$6932E466
	dc.l	$5DB7C232
	dc.l	$26DF0E55
	dc.l	$705591B
	dc.l	$D3DCE4A8
	dc.l	$9CE0D692
	dc.l	$AEE29A81
	dc.l	$9E818181
	dc.l	$BB81819A
	dc.l	$9F8181BA
	dc.l	$818AF307
	dc.l	$DFEE2102
	dc.l	$E9364B14
	dc.l	$47695036
	dc.l	$69695340
	dc.l	$69693A66
	dc.l	$661D5869
	dc.l	$69390B22
	dc.l	$3B696966
	dc.l	$151512E2
	dc.l	$EA170FED
	dc.l	$41EE7BD
	dc.l	$BC9381BF
	dc.l	$D1988196
	dc.l	$9A8186A5
	dc.l	$8191CFC8
	dc.l	$C1F008F1
	dc.l	$D3EC0C16
	dc.l	$34392346
	dc.l	$603A213D
	dc.l	$5F4F3B54
	dc.l	$4F3E5A47
	dc.l	$172F481F
	dc.l	$F713504E
	dc.l	$3A31F0C9
	dc.l	$FB05E7EC
	dc.l	$FDF8E7F2
	dc.l	$EBABACE8
	dc.l	$D5B5CFCC
	dc.l	$9FA1C5B5
	dc.l	$98C6EAC5
	dc.l	$CCF9E5CE
	dc.l	$F80FF1F5
	dc.l	$2009DF06
	dc.l	$2B0C1543
	dc.l	$33081631
	dc.l	$1811413D
	dc.l	$B19320E
	dc.l	$201BF7
	dc.l	$19563704
	dc.l	$1105E720
	dc.l	$592AF608
	dc.l	$12E5DF07
	dc.l	$FADCFB00
	dc.l	$D4D600F0
	dc.l	$C3CEE5CA
	dc.l	$C5E9D8B4
	dc.l	$CFE7C9C2
	dc.l	$E6E0C3DD
	dc.l	$FCECEC0A
	dc.l	$5F50E29
	dc.l	$E043134
	dc.l	$1826311B
	dc.l	$1731300F
	dc.l	$E201C28
	dc.l	$3C21FC04
	dc.l	$2832302D
	dc.l	$FF1FE0B
	dc.l	$F7F20406
	dc.l	$FE0007F9
	dc.l	$E4ECEAD8
	dc.l	$E1EADFDA
	dc.l	$D9CDC7D4
	dc.l	$DFD7D9EA
	dc.l	$EDE8F0F2
	dc.l	$EAF40605
	dc.l	$FE040704
	dc.l	$24504D2E
	dc.l	$1F131233
	dc.l	$4F38110F
	dc.l	$180C0C10
	dc.l	$FCEF0510
	dc.l	$FCF0F4E8
	dc.l	$D5DEE2D1
	dc.l	$DAF4EBD4
	dc.l	$D7DCCED1
	dc.l	$EBE8D9EC
	dc.l	$FAF30425
	dc.l	$1BF9FA12
	dc.l	$171F2F1D
	dc.l	$B1706
	dc.l	$141A08
	dc.l	$E1D06EF
	dc.l	$F8F6E1E7
	dc.l	$FFFAE9F1
	dc.l	$EFD5E20A
	dc.l	$BFF070B
	dc.l	$51D17
	dc.l	$41007
	dc.l	$D1908FB
	dc.l	$B1A0F06
	dc.l	$C01EDF6
	dc.l	$EFE7F1
	dc.l	$EEE2EC00
	dc.l	$F7FCFC
	dc.l	$FF172718
	dc.l	$3010703
	dc.l	$50C0805
UbL0036D0	dc.l	$100
	dc.l	$142003F
	dc.l	UbL0034D0
UbL0036DC	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0036D0
UbL0036EA	dc.l	UbL0036DC
	dc.l	0
	dc.l	0
	dc.l	$30A00
	dc.w	$FFFE

;	SECTION	UltSnd0036FC,DATA,CHIP
UbL0036FC	dc.l	$3172A1A
	dc.l	$15E8F105
	dc.l	$D0D2FAED
	dc.l	$1B002418
	dc.l	$7341642
	dc.l	$442B0308
	dc.l	$1E7C3D8
	dc.l	$C2E9EBBD
	dc.l	$DDEF0526
	dc.l	$EF1E47F9
	dc.l	$E0DE3DB
	dc.l	$DEE5F1DC
	dc.l	$C4D0E8EE
	dc.l	$F3204B21
	dc.l	$6161311
	dc.l	$303A03EF
	dc.l	$2CF8FA01
	dc.l	$E90E152C
	dc.l	$38FF020D
	dc.l	$FCEECBFB
	dc.l	$BFD10D6
	dc.l	$FD09E8FD
	dc.l	$F20F1D1B
	dc.l	$D4BCE8D8
	dc.l	$D1E8E302
	dc.l	$F70611D9
	dc.l	$213F1D2D
	dc.l	$373524F6
	dc.l	$E306DBE1
	dc.l	$E1F8FD16
	dc.l	$12051B17
	dc.l	$22344F18
	dc.l	$EAF2D3CB
	dc.l	$BCBFDFEF
	dc.l	$BCCFD1B2
	dc.l	$9F10138
	dc.l	$2B213E01
	dc.l	$18EBCD
	dc.l	$FD10E4EC
	dc.l	$EDE50202
	dc.l	$3A4A3838
	dc.l	$200D1610
	dc.l	$21111C1A
	dc.l	$17F9CE06
	dc.l	$E9E5F802
	dc.l	$26FDEBE7
	dc.l	$D2FAEB03
	dc.l	$FDEF7E2
	dc.l	$DFFDF604
	dc.l	$FF13F1F0
	dc.l	$3EFCBDD
	dc.l	$FF081E17
	dc.l	$FD082818
	dc.l	$343C2C1C
	dc.l	$1429F8EC
	dc.l	$FAE5D808
	dc.l	$9EAD204
	dc.l	$DE90A04
	dc.l	$12341D
	dc.l	$C2F5D6D3
	dc.l	$E4B3B9D0
	dc.l	$C2E0E1ED
	dc.l	$F70E2430
	dc.l	$5B4B3023
	dc.l	$2FA2204
	dc.l	$E5F8F6F6
	dc.l	$F10E0B0C
	dc.l	$115A0621
	dc.l	$38092308
	dc.l	$1204EC11
	dc.l	$FFD9C8DA
	dc.l	$D5C6D931
	dc.l	$18040ADE
	dc.l	$1F7EBEF
	dc.l	$E91CF1D1
	dc.l	$FCBB1015
	dc.l	$EA47EDE2
	dc.l	$16C0F612
	dc.l	$111A0F49
	dc.l	$143A2EE7
	dc.l	$275E021A
	dc.l	$2F09C2B2
	dc.l	$F5E5D814
	dc.l	$EBE6EDE3
	dc.l	$E3EC05F6
	dc.l	$413106E6
	dc.l	$BCD8E8EF
	dc.l	$C4C7E7CA
	dc.l	$FCEFEA13
	dc.l	$4E274960
	dc.l	$30233D0C
	dc.l	$E8EFE2DD
	dc.l	$2AFAD707
	dc.l	$12100C00
	dc.l	$3C553309
	dc.l	$E08DBEE
	dc.l	$CEEAFCCF
	dc.l	$E0CCD7B8
	dc.l	$D2E10A30
	dc.l	$FA16FED6
	dc.l	$22F3EDE2
	dc.l	$F312FDFF
	dc.l	$217290D
	dc.l	$8032312
	dc.l	$FC000526
	dc.l	$28060B26
	dc.l	$3401E4FF
	dc.l	$2B000516
	dc.l	$F0E5C7F3
	dc.l	$E3B1EFF2
	dc.l	$BAEC0BEA
	dc.l	$1E1616E2
	dc.l	$1A06E500
	dc.l	$F8DFEDED
	dc.l	$F500FADF
	dc.l	$1A10F40F
	dc.l	$4A505315
	dc.l	$241E0836
	dc.l	$DDE51EF0
	dc.l	$DF00D8EF
	dc.l	$FFDD0D2B
	dc.l	$A163C28
	dc.l	$F2E3F0EF
	dc.l	$E7B6DF14
	dc.l	$DCB2F0D4
	dc.l	$ECFC1B2C
	dc.l	$C91401DE
	dc.l	$300904EF
	dc.l	$410D12F2
	dc.l	$E2370F03
	dc.l	$FF22121A
	dc.l	$F6DD1F07
	dc.l	$DA0D1537
	dc.l	$D90A1504
	dc.l	$17D8F5F7
	dc.l	$FA06DDC6
	dc.l	$D1C3DAD5
	dc.l	$D3221923
	dc.l	$30D7172F
	dc.l	$FAF63E11
	dc.l	$F1DD08F8
	dc.l	$D1FB170E
	dc.l	$15013163
	dc.l	$23FAFAE3
	dc.l	$50A121A
	dc.l	$D7EBDCC1
	dc.l	$D9FFDB21
	dc.l	$34F60BEF
	dc.l	$1C2ADD0D
	dc.l	$F7E5DB
	dc.l	$EA07BC
	dc.l	$DEF8E426
	dc.l	$F1313AD1
	dc.l	$F8221C15
	dc.l	$22CEDFB
	dc.l	$CB3063D
	dc.l	$E0123532
	dc.l	$F29E5F5
	dc.l	$E8BC064A
	dc.l	$810E8DB
	dc.l	$E82833D6
	dc.l	$D00C1D00
	dc.l	$F3C3DEFA
	dc.l	$D4D4F331
	dc.l	$3A14101A
	dc.l	$FC172712
	dc.l	$C1129D3
	dc.l	$E000B3AC
	dc.l	$D60A205D
	dc.l	$45DD303D
	dc.l	$1821D00A
	dc.l	$22F509B6
	dc.l	$B2ECDBC9
	dc.l	$5F0E223
	dc.l	$1D00F4EF
	dc.l	$1BDE141D
	dc.l	$EF0FEBB8
	dc.l	$C806FBF9
	dc.l	$9261826
	dc.l	$4002044D
	dc.l	$1F0525F4
	dc.l	$FB0CACDA
	dc.l	$1FF9090F
	dc.l	$291805F8
	dc.l	$F104E80A
	dc.l	$2BABFCFD
	dc.l	$DEF2C9DE
	dc.l	$D2E60512
	dc.l	$27B9D616
	dc.l	$EE0915CE
	dc.l	$3D03FF42
	dc.l	$E9E3482A
	dc.l	$E6B4D02
	dc.l	$107FE9A
	dc.l	$CD15EA33
	dc.l	$E2E4EEF2
	dc.l	$3E200E37
	dc.l	$1A20E1CE
	dc.l	$EDB5BDC0
	dc.l	$B1B8DB03
	dc.l	$E5171111
	dc.l	$25573828
	dc.l	$2F0215CC
	dc.l	$E733C9B9
	dc.l	$900FBEC
	dc.l	$FB253816
	dc.l	$26F64936
	dc.l	$3ED50A35
	dc.l	$E3C0E814
	dc.l	$F4DE00CA
	dc.l	$A8130BFA
	dc.l	$3DAF500
	dc.l	$C100E9E7
	dc.l	$1D0AD915
	dc.l	$E101CF9
	dc.l	$11DDEBED
	dc.l	$4732130B
	dc.l	$162201F5
	dc.l	$2C514B11
	dc.l	$EFECF2ED
	dc.l	$E016EDFF
	dc.l	$16C1F3D7
	dc.l	$291AB565
	dc.l	$4CD71F02
	dc.l	$C8BEB2DB
	dc.l	$3D39810
	dc.l	$B8EC42D2
	dc.l	$13120B12
	dc.l	$FF5A17F5
	dc.l	$D4C13DB
	dc.l	$FEE0D924
	dc.l	$DE6D50C
	dc.l	$263A0740
	dc.l	$3EFE09EC
	dc.l	$421AA5CF
	dc.l	$14F0E9F6
	dc.l	$BC00DCF9
	dc.l	$C0AE6EF
	dc.l	$E5FA48DB
	dc.l	$30EDE5D3
	dc.l	$D70D5F0C
	dc.l	$DCEFE938
	dc.l	$1457EDF6
	dc.l	$41D9FDEF
	dc.l	$FE1B0058
	dc.l	$2F3E5
	dc.l	$2D23843
	dc.l	$980CB9CC
	dc.l	$14FFBCE4
	dc.l	$43EEFC15
	dc.l	$105FE50
	dc.l	$B3AEEDC1
	dc.l	$FCD6E1FB
	dc.l	$43F0F956
	dc.l	$2F103B46
	dc.l	$1E0DEE21
	dc.l	$FDF130EB
	dc.l	$A3E7FCF9
	dc.l	$44321BE2
	dc.l	$1C2218FD
	dc.l	$C6ECE2ED
	dc.l	$E5C1AFB1
	dc.l	$3E3FBC8
	dc.l	$D017441A
	dc.l	$B1FED12
	dc.l	$164011F5
	dc.l	$4619F84C
	dc.l	$EEE521E8
	dc.l	$34284B03
	dc.l	$60025FA
	dc.l	$C5FEEF01
	dc.l	$D1FA3799
	dc.l	$DEDDCEC6
	dc.l	$FC2328CF
	dc.l	$E62109E0
	dc.l	$D35101A4
	dc.l	$4C13E70A
	dc.l	$1D2AE9FE
	dc.l	$A9212CE8
	dc.l	$B22B3ADB
	dc.l	$E2F57F4B
	dc.l	$F052D8E5
	dc.l	$EFF0024
	dc.l	$4AFA12F7
	dc.l	$81CCF9C8
	dc.l	$2B16B2CC
	dc.l	$FE1DE434
	dc.l	$1C02DBF2
	dc.l	$B3D4DAB9
	dc.l	$F9DDE0E2
	dc.l	$D2650C7F
	dc.l	$4CE94412
	dc.l	$F733110F
	dc.l	$17DFD929
	dc.l	$1204A07
	dc.l	$A7FDAE6
	dc.l	$41B39B10
	dc.l	$34C40700
	dc.l	$D63BEEF2
	dc.l	$681AB5E
	dc.l	$CD900E8
	dc.l	$DDEFD2AF
	dc.l	$7750CFF9
	dc.l	$15E554DF
	dc.l	$C73CC71F
	dc.l	$21A40439
	dc.l	$F9E21A26
	dc.l	$27F5344A
	dc.l	$3CF1CF28
	dc.l	$20CBCFD2
	dc.l	$99F315A9
	dc.l	$444D10
	dc.l	$E2E31CFF
	dc.l	$CF063AFF
	dc.l	$FB061FAF
	dc.l	$E74B81D8
	dc.l	$2C9351F
	dc.l	$1F1F2126
	dc.l	$5965113F
	dc.l	$2CFED5D8
	dc.l	$19A2CD23
	dc.l	$FF032BD6
	dc.l	$81AA4AFF
	dc.l	$2517CF4E
	dc.l	$48FDE6D4
	dc.l	$15E5EC19
	dc.l	$E0BBFD2C
	dc.l	$E6CEBECE
	dc.l	$6044D9D2
	dc.l	$4609F7C6
	dc.l	$DA19D316
	dc.l	$33A4617
	dc.l	$25CD0E41
	dc.l	$68D6C2E8
	dc.l	$E2EFA07
	dc.l	$E30AD307
	dc.l	$4F30AEF4
	dc.l	$F2D4D2F7
	dc.l	$90C916DD
	dc.l	$284DE608
	dc.l	$16140B11
	dc.l	$11EE392E
	dc.l	$F021B9A5
	dc.l	$29D94342
	dc.l	$F81CCAFB
	dc.l	$6E53E4A5
	dc.l	$3D330F41
	dc.l	$AAAFEDC6
	dc.l	$E615D8B0
	dc.l	$1FF60C30
	dc.l	$B4B32A5F
	dc.l	$30EACCE8
	dc.l	$DA0EAA04
	dc.l	$26B3FFF5
	dc.l	$7BFEE217
	dc.l	$FB1F1246
	dc.l	$FC1DE8FD
	dc.l	$FD050001
	dc.l	$4AF1EEC3
	dc.l	$614DD8ED
	dc.l	$1160420
	dc.l	$D5813606
	dc.l	$BC7F1DE8
	dc.l	$4219C0BF
	dc.l	$D40907C1
	dc.l	$B02EA0DD
	dc.l	$2E662713
	dc.l	$2AA41846
	dc.l	$F52316F6
	dc.l	$D3F3E1AE
	dc.l	$C7E9063B
	dc.l	$5733D036
	dc.l	$42DACD10
	dc.l	$55FDF5D4
	dc.l	$13BF8E8B
	dc.l	$B8D00EE6
	dc.l	$E9331D3D
	dc.l	$1535200C
	dc.l	$D11B24C5
	dc.l	$E41027F2
	dc.l	$EDD6E60A
	dc.l	$461E2AD4
	dc.l	$4D371805
	dc.l	$D8DC150D
	dc.l	$3CEEF12F
	dc.l	$263E14
	dc.l	$D2094838
	dc.l	$ECCFF7AB
	dc.l	$E5CCAD0B
	dc.l	$E7D5F0FE
	dc.l	$15F0F0CE
	dc.l	$9091636
	dc.l	$FAD2E906
	dc.l	$5340B3F8
	dc.l	$2F502C
	dc.l	$3ED1023
	dc.l	$D5D7FDD7
	dc.l	$6FAD11D
	dc.l	$56FDE704
	dc.l	$D54435FD
	dc.l	$D7C7C9CA
	dc.l	$B3B7D7FA
	dc.l	$E4F4FE57
	dc.l	$D51C072F
	dc.l	$164F3DBC
	dc.l	$CF15E50A
	dc.l	$43FE1402
	dc.l	$122A3300
	dc.l	$72B09FF
	dc.l	$1BE069F9
	dc.l	$EC22BAE5
	dc.l	$FA0B1DD0
	dc.l	$9A03151D
	dc.l	$B8E34DEB
	dc.l	$D9208BAF
	dc.l	$64D58FDD
	dc.l	$4FB4E06
	dc.l	$D04B1307
	dc.l	$1D3F07FC
	dc.l	$27E56025
	dc.l	$E63622DC
	dc.l	$FB4EE5E2
	dc.l	$33D74399
	dc.l	$E622C0F4
	dc.l	$A5001A04
	dc.l	$FB2501AE
	dc.l	$BEAFF39
	dc.l	$3FB32AB
	dc.l	$BB9EE5F8
	dc.l	$F4F6F324
	dc.l	$DE742E5C
	dc.l	$53F5DF2A
	dc.l	$CEFC6E5
	dc.l	$2F05F01D
	dc.l	$F748AE
	dc.l	$5500B240
	dc.l	$515B2E9
	dc.l	$F181D6D6
	dc.l	$F6DD6D9
	dc.l	$344DFED6
	dc.l	$103231E7
	dc.l	$DB400BBB
	dc.l	$E01CE751
	dc.l	$C5E21C07
	dc.l	$1D161418
	dc.l	$1DDD1FC4
	dc.l	$B646F545
	dc.l	$3D4E2C0
	dc.l	$2F2FD81C
	dc.l	$2CDD9F5
	dc.l	$81DB3BEB
	dc.l	$F815DC2A
	dc.l	$2FF618E5
	dc.l	$FC30F1F2
	dc.l	$1DCCE9D1
	dc.l	$F3F915E3
	dc.l	$6B7F3728
	dc.l	$1DFC00E6
	dc.l	$2E02BAE3
	dc.l	$16DF01FC
	dc.l	$FA150536
	dc.l	$7FC6FF50
	dc.l	$47BFEEB8
	dc.l	$C927C3E2
	dc.l	$FDEFB3A4
	dc.l	$A61F5520
	dc.l	$19DAC2F6
	dc.l	$F43650E8
	dc.l	$F20AE606
	dc.l	$F62F0B14
	dc.l	$F8FA0759
	dc.l	$CA4134
	dc.l	$FE27F11E
	dc.l	$49C3070D
	dc.l	$B81F09C4
	dc.l	$BD097DA
	dc.l	$13BEF2D5
	dc.l	$CFE3033F
	dc.l	$1ADF1A0A
	dc.l	$310AF01A
	dc.l	$1E06E2F1
	dc.l	$EEE647F1
	dc.l	$EC18006F
	dc.l	$D1C4ED9
	dc.l	$453A2339
	dc.l	$96A712F9
	dc.l	$193E23E
	dc.l	$DDF7FCFA
	dc.l	$322900E6
	dc.l	$DDC41CEC
	dc.l	$D0C3A00A
	dc.l	$C4A33DF5
	dc.l	$E5E77E45
	dc.l	$F96555E4
	dc.l	$96BDDB7
	dc.l	$351A0E10
	dc.l	$2D6E81F
	dc.l	$C8306905
	dc.l	$87454DEE
	dc.l	$AA00FEF
	dc.l	$EA43B89C
	dc.l	$2FE73C0C
	dc.l	$83CE9FD9
	dc.l	$2AD040EE
	dc.l	$211B242D
	dc.l	$C9A62C25
	dc.l	$A201902
	dc.l	$4DB9360B
	dc.l	$ECE8281B
	dc.l	$AB03531A
	dc.l	$26151432
	dc.l	$F2CC6473
	dc.l	$FA9DA5CE
	dc.l	$535CAD0
	dc.l	$B431EEE5
	dc.l	$25FF2A2B
	dc.l	$15B8E0E6
	dc.l	$AE20D9AC
	dc.l	$1AEFF7
	dc.l	$F5022BEF
	dc.l	$E16A171C
	dc.l	$34101BF6
	dc.l	$1AED810C
	dc.l	$EA4F3100
	dc.l	$32FACB62
	dc.l	$E0D537F3
	dc.l	$1A0FBCE4
	dc.l	$2894E8B0
	dc.l	$FA1ADF1B
	dc.l	$19AEEC41
	dc.l	$D4C87F32
	dc.l	$A1240266
	dc.l	$4F0AE617
	dc.l	$14C4472A
	dc.l	$E8ECDCE2
	dc.l	$F2B9F504
	dc.l	$1DEA2E46
	dc.l	$42C9455C
	dc.l	$E1C70F0C
	dc.l	$C38110C9
	dc.l	$A5C1DC33
	dc.l	$2231351A
	dc.l	$125CF0BB
	dc.l	$5B03BD12
	dc.l	$B5E9F3F3
	dc.l	$E61518E3
	dc.l	$38DE1B60
	dc.l	$1142BBFD
	dc.l	$58C5E42B
	dc.l	$7FBFC0CE
	dc.l	$C28BC9E
	dc.l	$F64DFBFF
	dc.l	$4BCCE616
	dc.l	$A5E415E5
	dc.l	$F62CCD17
	dc.l	$F9CE0CF1
	dc.l	$2C35CFE6
	dc.l	$CCD60F03
	dc.l	$36F90B59
	dc.l	$E63242E7
	dc.l	$2DE90D31
	dc.l	$CFC81749
	dc.l	$C7DD3FCB
	dc.l	$331B0926
	dc.l	$C4292BED
	dc.l	$EFE61CAF
	dc.l	$CB189CDD
	dc.l	$22D9D34E
	dc.l	$8DD3730
	dc.l	$52509C7
	dc.l	$4D1EB0E4
	dc.l	$C1E0F700
	dc.l	$10D1F112
	dc.l	$4E3D2734
	dc.l	$24422206
	dc.l	$153CEFB1
	dc.l	$D985A720
	dc.l	$D5C10F1D
	dc.l	$1C53FCFA
	dc.l	$F83812
	dc.l	$94DCD6F1
	dc.l	$C89420C
	dc.l	$D564C0D8
	dc.l	$254D17C9
	dc.l	$181A220C
	dc.l	$FEF24A2A
	dc.l	$CBE53348
	dc.l	$13C342F8
	dc.l	$A1DB0400
	dc.l	$BAFEBCE8
	dc.l	$FF0CDF0D
	dc.l	$720033F
	dc.l	$1AD0E6EF
	dc.l	$D543DCB3
	dc.l	$11FC39F7
	dc.l	$127F131D
	dc.l	$64E01B1D
	dc.l	$C607F3FC
	dc.l	$D2A7004B
	dc.l	$20F500CC
	dc.l	$45420CF8
	dc.l	$EA3FF0D2
	dc.l	$BFC89EFA
	dc.l	$BBCBF2B1
	dc.l	$11400D24
	dc.l	$2A4FFB4D
	dc.l	$2FD3CFF
	dc.l	$E7F5F2F2
	dc.l	$F7F649FB
	dc.l	$FA38200A
	dc.l	$C400F83A
	dc.l	$27F5D3AF
	dc.l	$FD0D6FD
	dc.l	$3B0ED23B
	dc.l	$F2C81D1A
	dc.l	$B9ABAAF2
	dc.l	$39E5DCE9
	dc.l	$F3F10631
	dc.l	$1D221EEB
	dc.l	$390CFD15
	dc.l	$DFEEEEB
	dc.l	$6220830
	dc.l	$522414F5
	dc.l	$39F9D62B
	dc.l	$E7B6CFAA
	dc.l	$D512FD00
	dc.l	$152DE806
	dc.l	$1D3628E8
	dc.l	$2EF4DA0F
	dc.l	$C2D59EB3
	dc.l	$F90C08D3
	dc.l	$D4383037
	dc.l	$F7B92163
	dc.l	$47F9FB39
	dc.l	$24E60800
	dc.l	$A00F57C9
	dc.l	$E50BEE1B
	dc.l	$F5291A21
	dc.l	$4EC5B90B
	dc.l	$8E0A725
	dc.l	$EF829F1
	dc.l	$FFC7C74A
	dc.l	$2A19E3C9
	dc.l	$E20ADA07
	dc.l	$23030914
	dc.l	$E5FFEF00
	dc.l	$45DE0707
	dc.l	$F4110A27
	dc.l	$22E5013F
	dc.l	$1FE22B25
	dc.l	$FAFAF4E0
	dc.l	$E9BF1DCD
	dc.l	$DA3FD7FB
	dc.l	$5CABD2B
	dc.l	$1AEF1017
	dc.l	$2611D418
	dc.l	$E0C4CDEA
	dc.l	$E9BFCC34
	dc.l	$F7334751
	dc.l	$144E2115
	dc.l	$2B14E8C4
	dc.l	$1A04B60A
	dc.l	$AD12826
	dc.l	$62CEED9
	dc.l	$4E303BA
	dc.l	$BD0026E5
	dc.l	$1DD000E4
	dc.l	$E2072828
	dc.l	$DD0719DD
	dc.l	$2AF3A53A
	dc.l	$223C3E0B
	dc.l	$E81E05FB
	dc.l	$BD60B19
	dc.l	$FDF1C
	dc.l	$C7DE49FB
	dc.l	$FB4B0C06
	dc.l	$D30B3F35
	dc.l	$ABF5E597
	dc.l	$F3B5C450
	dc.l	$ECF30BBB
	dc.l	$2B4EE7F7
	dc.l	$6901140F
	dc.l	$D729B6ED
	dc.l	$A0D5EDE3
	dc.l	$FB265503
	dc.l	$2B723328
	dc.l	$D1339FA
	dc.l	$EEF3B7
	dc.l	$ABF1F4CC
	dc.l	$107EE28
	dc.l	$CEF909F1
	dc.l	$2BEB28F1
	dc.l	$CCE6D001
	dc.l	$D310F7D9
	dc.l	$D4005897
	dc.l	$FA4B3418
	dc.l	$EF181D16
	dc.l	$180BE441
	dc.l	$4AE9CD2F
	dc.l	$EE2A4ED6
	dc.l	$4EEA537
	dc.l	$201FBDC4
	dc.l	$4023823
	dc.l	$C9E233DB
	dc.l	$D645F4C5
	dc.l	$A3B145D7
	dc.l	$3F32A22
	dc.l	$131DC56
	dc.l	$3DF10206
	dc.l	$ADC91CEE
	dc.l	$F7FD0732
	dc.l	$19E74226
	dc.l	$C41E9
	dc.l	$D6ECEC3F
	dc.l	$FFDFE5D5
	dc.l	$2B6FB07
	dc.l	$FC1B290E
	dc.l	$3925044C
	dc.l	$BAE9CDD8
	dc.l	$64A1AB19
	dc.l	$D90BEAFD
	dc.l	$7F62E11
	dc.l	$F9DDF704
	dc.l	$5EFC406
	dc.l	$25EF0E3E
	dc.l	$E1FF21D4
	dc.l	$3D2EF51F
	dc.l	$40B0925
	dc.l	$CD171B1B
	dc.l	$42235504
	dc.l	$D3E6F55B
	dc.l	$FDA4CE21
	dc.l	$E8A7DAC1
	dc.l	$8CBDD10
	dc.l	$F12925A5
	dc.l	$44A03DD
	dc.l	$BCFB009A
	dc.l	$C9FA090F
	dc.l	$D064A19
	dc.l	$1EFD010A
	dc.l	$3E3711D8
	dc.l	$14D98118
	dc.l	$E5E3151F
	dc.l	$2E3344F4
	dc.l	$29232D25
	dc.l	$E2290012
	dc.l	$F1DAF514
	dc.l	$31C6DA20
	dc.l	$130BCE46
	dc.l	$5808DD11
	dc.l	$E1E6E6E9
	dc.l	$F118ECC6
	dc.l	$E2DEF5C8
	dc.l	$C2F6E6EF
	dc.l	$DDF41FF5
	dc.l	$BF0F0D31
	dc.l	$F4351A16
	dc.l	$1107FACA
	dc.l	$D5021FE6
	dc.l	$EFFEEDE5
	dc.l	$1E444020
	dc.l	$1A285BEB
	dc.l	$DAE52C25
	dc.l	$CEECFCD8
	dc.l	$1D34747
	dc.l	$FCFF1544
	dc.l	$208F81A
	dc.l	$4E19B6D5
	dc.l	$E4B9CA98
	dc.l	$E301FC01
	dc.l	$8F51243
	dc.l	$E71244D6
	dc.l	$CD3A00D2
	dc.l	$E8F2D6F9
	dc.l	$44CEAC2B
	dc.l	$243142D5
	dc.l	$EAF4D600
	dc.l	$1A03E1E9
	dc.l	$361A1ABD
	dc.l	$3C3EF717
	dc.l	$24E81A03
	dc.l	$23041F00
	dc.l	$F4123C26
	dc.l	$E9E4DC4D
	dc.l	$F6B913F3
	dc.l	$F4F8B4C8
	dc.l	$DAF3122D
	dc.l	$F9D13327
	dc.l	$D8D75CEB
	dc.l	$B9C0E334
	dc.l	$C99CFED1
	dc.l	$F7171A13
	dc.l	$C2FA0112
	dc.l	$7FFC182C
	dc.l	$D3F8CEE4
	dc.l	$F9D5EC05
	dc.l	$11431E00
	dc.l	$2935023A
	dc.l	$16D60C49
	dc.l	$17F2DF0C
	dc.l	$10CDD4FE
	dc.l	$4AE6F736
	dc.l	$4A6CFDF9
	dc.l	$18C1F3C7
	dc.l	$A803F7E8
	dc.l	$D3B700E2
	dc.l	$F805FD16
	dc.l	$2CFAD20F
	dc.l	$3FBF900
	dc.l	$CFD53B
	dc.l	$2BCF2234
	dc.l	$14F6CFDC
	dc.l	$A25D90B
	dc.l	$F3D31F18
	dc.l	$D7410F13
	dc.l	$5B22F415
	dc.l	$18C602EF
	dc.l	$DBBEE535
	dc.l	$A05043A
	dc.l	$393D5E11
	dc.l	$FEEC1AFF
	dc.l	$F0B298F4
	dc.l	$AEDD01EE
	dc.l	$5EF1535
	dc.l	$3F04F334
	dc.l	$E6DCF7AC
	dc.l	$D8E7CBFD
	dc.l	$DC0129E0
	dc.l	$FF0DD8DF
	dc.l	$1A2B043E
	dc.l	$34D8F6FC
	dc.l	$D5CA1608
	dc.l	$FDE81214
	dc.l	$5F302618
	dc.l	$2946012E
	dc.l	$2BD30FDA
	dc.l	$DD4C02ED
	dc.l	$29CD0C3F
	dc.l	$9C0D1AC2
	dc.l	$2800DA01
	dc.l	$F0DDF00C
	dc.l	$BE8F800
	dc.l	$A1F6E7E8
	dc.l	$1C9CF4EB
	dc.l	$1F8D9E4
	dc.l	$83F1EFE
	dc.l	$63032728
	dc.l	$DEE0DD18
	dc.l	$DEB704F1
	dc.l	$F3212320
	dc.l	$493A0018
	dc.l	$4E3D2646
	dc.l	$E0CC29FE
	dc.l	$E4C3EA05
	dc.l	$F40C18F2
	dc.l	$EC3AFBFB
	dc.l	$3DE72122
	dc.l	$C0C71CCF
	dc.l	$BDA0D50D
	dc.l	$D3FD630A
	dc.l	$CF41060B
	dc.l	$E3AB0BF4
	dc.l	$F9B8CA32
	dc.l	$EF61AD5
	dc.l	$DE4009E2
	dc.l	$627D312
	dc.l	$F2135130
	dc.l	$2E02909
	dc.l	$F014231
	dc.l	$D1F8CD18
	dc.l	$AE1E6FD
	dc.l	$22F8F940
	dc.l	$312BFA0B
	dc.l	$3DFF2D17
	dc.l	$EAEF07B8
	dc.l	$AEF3E1F3
	dc.l	$EB19F2D8
	dc.l	$DC093D24
	dc.l	$D235EFD2
	dc.l	$A0B8CF
	dc.l	$DBCFEF00
	dc.l	$FD0D0251
	dc.l	$2E024522
	dc.l	$211CE90E
	dc.l	$F4CCF310
	dc.l	$5EC1A1F
	dc.l	$FC1E3722
	dc.l	$3CF4EE15
	dc.l	$1B3CF82B
	dc.l	$ED9DECE9
	dc.l	$140FF91D
	dc.l	$F0F60A12
	dc.l	$F21629C0
	dc.l	$F7FECC15
	dc.l	$DAE61117
	dc.l	$D7FF1FE5
	dc.l	$E8AEE143
	dc.l	$F6DB29FD
	dc.l	$D40EEFFC
	dc.l	$14CF3009
	dc.l	$DC09C5FA
	dc.l	$24CBF3D7
	dc.l	$E82FEDF8
	dc.l	$16382014
	dc.l	$451A154E
	dc.l	$F1300EE6
	dc.l	$21ECDE7
	dc.l	$1A070025
	dc.l	$174B05FF
	dc.l	$3618150A
	dc.l	$F3E204E6
	dc.l	$CF9BFA10
	dc.l	$90EEEEDA
	dc.l	$1CBA013A
	dc.l	$E92AFC0A
	dc.l	$D9CFE8D1
	dc.l	$FE8FD2C
	dc.l	$E40909E6
	dc.l	$10210628
	dc.l	$BFEFD
	dc.l	$1F4F20C
	dc.l	$36060E0A
	dc.l	$384BDD0E
	dc.l	$2CEAF22D
	dc.l	$F5CC2014
	dc.l	$E541F4F3
	dc.l	$3D2327F1
	dc.l	$A30D51C8
	dc.l	$D2CCC608
	dc.l	$C704E798
	dc.l	$5220D06
	dc.l	$F6DA0D4F
	dc.l	$BC2C00BB
	dc.l	$C1DC18BB
	dc.l	$CAE64A16
	dc.l	$6460B22
	dc.l	$37FEF5D6
	dc.l	$1AF1D0C9
	dc.l	$E2110FF9
	dc.l	$FB3F1A3D
	dc.l	$162D4D15
	dc.l	$3A2C10EB
	dc.l	$E7FEE61D
	dc.l	$F4F00319
	dc.l	$D5EC1B03
	dc.l	$CA2D20EC
	dc.l	$15F525F3
	dc.l	$ACEBDDF6
	dc.l	$FDD4DBF7
	dc.l	$20E5FF2E
	dc.l	$F43008DE
	dc.l	$EFDA17E8
	dc.l	$E6421DD0
	dc.l	$31F1F5F0
	dc.l	$D10B0CAC
	dc.l	$FBFDC9FC
	dc.l	$C4043914
	dc.l	$10413230
	dc.l	$F4FE302B
	dc.l	$9F3D71F
	dc.l	$17D2EF20
	dc.l	$20EE0717
	dc.l	$1815E93A
	dc.l	$3F2B18D8
	dc.l	$F3DCA6C8
	dc.l	$C5F4EAC7
	dc.l	$C211225
	dc.l	$11F7FF17
	dc.l	$FD00FDA0
	dc.l	$4F7E0D2
	dc.l	$C1EDD3D1
	dc.l	$F4524617
	dc.l	$316FCFB
	dc.l	$280BE1E2
	dc.l	$236D200
	dc.l	$1E28410A
	dc.l	$401A001E
	dc.l	$ECF43A21
	dc.l	$EC192AE5
	dc.l	$F7CFF71F
	dc.l	$ED2F2DC6
	dc.l	$F40DE3F2
	dc.l	$B82B00D8
	dc.l	$CEFC3AE9
	dc.l	$18F8E7E2
	dc.l	$F0E1E1F7
	dc.l	$7DED5C1
	dc.l	$FCE3E5F2
	dc.l	$26522DF7
	dc.l	$F2D2939
	dc.l	$17C1DEF0
	dc.l	$BEFEE904
	dc.l	$EAD5091A
	dc.l	$3C13444E
	dc.l	$274027FC
	dc.l	$253226C4
	dc.l	$B5F8DAFC
	dc.l	$FB0AF004
	dc.l	$2FFF0C38
	dc.l	$4204D4EA
	dc.l	$B5B9F5DE
	dc.l	$E0ECF9FB
	dc.l	$D0E5D9EF
	dc.l	$D21F5E6
	dc.l	$10040C00
	dc.l	$FEDCD3FF
	dc.l	$33E1001E
	dc.l	$F71625EF
	dc.l	$14EAF2EC
	dc.l	$A150FF6
	dc.l	$2F0F1841
	dc.l	$FD04591E
	dc.l	$E2140D0A
	dc.l	$EBD7F4
	dc.l	$CE62815
	dc.l	$F1172C25
	dc.l	$42E708FB
	dc.l	$F1B3D1
	dc.l	$D0C9BFF7
	dc.l	$EAE21B17
	dc.l	$DF232807
	dc.l	$2AFBEBE9
	dc.l	$EAD9DEEB
	dc.l	$B7FCCDEC
	dc.l	$1B040D31
	dc.l	$42091EF2
	dc.l	$1B00FD20
	dc.l	$F4C1F8FC
	dc.l	$81DDF1D
	dc.l	$5C085019
	dc.l	$E73311FC
	dc.l	$171A14FB
	dc.l	$DFF7282C
	dc.l	$13BCF0F5
	dc.l	$E902F3FA
	dc.l	$FF11F9EF
	dc.l	$E01D06CA
	dc.l	$EFD40618
	dc.l	$D3E2FAF6
	dc.l	$14E5F3CF
	dc.l	$DCE8D40E
	dc.l	$1CF1032C
	dc.l	$F901F4F2
	dc.l	$4A16FE08
	dc.l	$BA2001C3
	dc.l	$B4DB4005
	dc.l	$12301C3C
	dc.l	$372A2821
	dc.l	$221220F7
	dc.l	$F417C2E7
	dc.l	$E8D4E82B
	dc.l	$FA0E3D17
	dc.l	$151E1D2B
	dc.l	$E30000C4
	dc.l	$C9E8C8EC
	dc.l	$FBB11
	dc.l	$49E7F108
	dc.l	$20E0F002
	dc.l	$CDFAF9C4
	dc.l	$DF00F215
	dc.l	$E8E92ABA
	dc.l	$617EBEE
	dc.l	$22A0F00
	dc.l	$13333313
	dc.l	$1006380F
	dc.l	$FF3BE5D6
	dc.l	$160EEFF6
	dc.l	$F10513FB
	dc.l	$1447F4F0
	dc.l	$1031FF25
	dc.l	$FEEFDDD1
	dc.l	$1CE9A5C5
	dc.l	$8E8F91D
	dc.l	$F3CF0032
	dc.l	$E02BF7D0
	dc.l	$EBC4BFE5
	dc.l	$FBEBAB06
	dc.l	$1FFA1C04
	dc.l	$53311D28
	dc.l	$2504DB46
	dc.l	$E9CBDED5
	dc.l	$F01A082A
	dc.l	$4113244
	dc.l	$F717340C
	dc.l	$120A07FA
	dc.l	$E6EFD8FF
	dc.l	$4AF9FE0C
	dc.l	$F1400F9
	dc.l	$FFEF507
	dc.l	$3103CAC2
	dc.l	$E71DD710
	dc.l	$E7C406ED
	dc.l	$F0FCD7F6
	dc.l	$EE00D11C
	dc.l	$E3EA0BF2
	dc.l	$FA1A210F
	dc.l	$ED131100
	dc.l	$F01F715
	dc.l	$FE01E1D5
	dc.l	$FD1E0F15
	dc.l	$37F21E17
	dc.l	$2A4B0E03
	dc.l	$AD6E133
	dc.l	$EFC6F20D
	dc.l	$FF1A102C
	dc.l	$3DF73122
	dc.l	$D1F7FB00
	dc.l	$B9D40AC8
	dc.l	$C9E5EFE2
	dc.l	$FA0600F8
	dc.l	$32DEF439
	dc.l	$1AE6DCFC
	dc.l	$EEEDDEDD
	dc.l	$E8E5F30D
	dc.l	$F7270707
	dc.l	$2023081F
	dc.l	$2110DDFC
	dc.l	$36FADBFD
	dc.l	$9112321
	dc.l	$11051C11
	dc.l	$D2D3911
	dc.l	$F6152AE3
	dc.l	$16F4B1DB
	dc.l	$C3B4CE22
	dc.l	$3B12537
	dc.l	$C403306
	dc.l	$2C0D0202
	dc.l	$E4F3BDDD
	dc.l	$B5C22487
	dc.l	$88251917
	dc.l	$2522391F
	dc.l	$2CF5D11F
	dc.l	$FDBDFA26
	dc.l	$1F30ADD
	dc.l	$FC40FEF3
	dc.l	$1B4925D6
	dc.l	$D8190EC9
	dc.l	$CD181E24
	dc.l	$20081F4F
	dc.l	$15BFEC41
	dc.l	$22CCB5E2
	dc.l	$11E9C91E
	dc.l	$1B034E06
	dc.l	$E42924F4
	dc.l	$EC09F7D2
	dc.l	$B8B7C705
	dc.l	$EDA0E7F8
	dc.l	$2A031551
	dc.l	$1E2106FE
	dc.l	$23B9BDE9
	dc.l	$ECFC04DD
	dc.l	$D71E12DA
	dc.l	$D5403F34
	dc.l	$1F15300A
	dc.l	$2FDE000D
	dc.l	$C6E017FB
	dc.l	$FF1B0CFA
	dc.l	$5318440D
	dc.l	$DA4615C6
	dc.l	$E825EDFF
	dc.l	$21F106F1
	dc.l	$D2E51DF5
	dc.l	$9EFB9D9
	dc.l	$20EABBF8
	dc.l	$2F3F1C3
	dc.l	$FE3F00E8
	dc.l	$D9EC2003
	dc.l	$E5EBCF04
	dc.l	$D0E932
	dc.l	$4A05210E
	dc.l	$2056F3FB
	dc.l	$18E51C1B
	dc.l	$E7E7F2E7
	dc.l	$22ED6F5
	dc.l	$40355125
	dc.l	$EF1F3E1A
	dc.l	$D9BCE5C1
	dc.l	$C1D2F503
	dc.l	$FC0912
	dc.l	$2A3DDCF5
	dc.l	$3717F3D0
	dc.l	$D006D5C8
	dc.l	$CE94F007
	dc.l	$C9F02A3A
	dc.l	$11302FFD
	dc.l	$F0205F8
	dc.l	$ECEEF4EC
	dc.l	$E7FFF907
	dc.l	$26082224
	dc.l	$1823002A
	dc.l	$2B0809F9
	dc.l	$FD090F25
	dc.l	$51804
	dc.l	$4080005
	dc.l	$F2E2F3
	dc.l	$9F91104
	dc.l	$110FB15
	dc.l	$B08E8
	dc.l	$E4DDDBF4
	dc.l	$AEB4DEDE
	dc.l	$EFFC200F
	dc.l	$B0B0713
	dc.l	$FF22F3DF
	dc.l	$D8D5E0
	dc.l	$D7EAD5F8
	dc.l	$140B5034
	dc.l	$1E324241
	dc.l	$1E000700
	dc.l	$EEDCEB0C
	dc.l	$F3DCEB07
	dc.l	$313030
	dc.l	$2B2624FF
	dc.l	$B0A07C7
	dc.l	$C7EAD6D2
	dc.l	$E000171A
	dc.l	$1712F1EE
UbL0049FC	dc.l	$980
	dc.l	$1AD003F
	dc.l	UbL0036FC
UbL004A08	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL0049FC
UbL004A16	dc.l	UbL004A08
	dc.l	0
	dc.l	0
	dc.l	$240A00
	dc.w	$FFFE

;	SECTION	UltSnd004A28,DATA,CHIP
UbL004A28	dc.l	$2121B12
	dc.l	$F04FFF8
	dc.l	$F4F2EEEB
	dc.l	$E8E5E6E5
	dc.l	$E2E1E7EB
	dc.l	$E9E4E3E6
	dc.l	$E9F0F4E7
	dc.l	$D7DAE1E5
	dc.l	$ECF1F0F0
	dc.l	$EFF3F5F3
	dc.l	$EEF2F7F6
	dc.l	$FAFEFBFB
	dc.l	$50504
	dc.l	$A141001
	dc.l	$FEF9F8
	dc.l	$FBFDFBFA
	dc.l	$F8F3F4FA
	dc.l	$FE00040F
	dc.l	$2E4E361A
	dc.l	$3F5E9D9
	dc.l	$D2D6DEEB
	dc.l	$FB0B1E40
	dc.l	$7F56EC3B
	dc.l	$42313A28
	dc.l	$2B271C18
	dc.l	$171409ED
	dc.l	$CA8BA4EF
	dc.l	$EDFD00FD
	dc.l	$FE000D
	dc.l	$18242F03
	dc.l	$D35645F7
	dc.l	$1E07FDF3
	dc.l	$E0D5CFCF
	dc.l	$D9ECFEF0
	dc.l	$DD0BE5EE
	dc.l	$2FA03FE
	dc.l	$F9F2E0D2
	dc.l	$CEC9AC83
	dc.l	$BE2ACFDD
	dc.l	$ECE5F6F8
	dc.l	$4050002
	dc.l	$FBE9EB1E
	dc.l	$F08321C
	dc.l	$2627252B
	dc.l	$2B2E343D
	dc.l	$47616F29
	dc.l	$2FA0F23
	dc.l	$1B1F2528
	dc.l	$251D1406
	dc.l	$F1E3024B
	dc.l	$3C1400E8
	dc.l	$DCCBBFB3
	dc.l	$AAAFB1B3
	dc.l	$B08583EA
	dc.l	$35DFDEE7
	dc.l	$E4E6D7D5
	dc.l	$D4DCEA00
	dc.l	$1B211954
	dc.l	$361D382B
	dc.l	$2B241E17
	dc.l	$9FEFBFF
	dc.l	$5D8BB13
	dc.l	$DBE000FC
	dc.l	$11181F1A
	dc.l	$15181313
	dc.l	$2B567F76
	dc.l	$211C1310
	dc.l	$6FEF9F1
	dc.l	$E9DECEB7
	dc.l	$C221EEAD
	dc.l	$2E9EFFA
	dc.l	$FB102233
	dc.l	$30221604
	dc.l	$C473B16
	dc.l	$18F2E1D0
	dc.l	$C7CACBCC
	dc.l	$D4E3F620
	dc.l	$5E30B7CA
	dc.l	$EEDEEFF3
	dc.l	$F9FDFC00
	dc.l	$B131A25
	dc.l	$12271FD6
	dc.l	$E3D9D5D9
	dc.l	$D6D7D2CB
	dc.l	$C4BA9D83
	dc.l	$ADEEF0F6
	dc.l	$E5ECE7E0
	dc.l	$E0E0E7F7
	dc.l	$B26423E
	dc.l	$5FA110C
	dc.l	$212F3E4D
	dc.l	$555A5B5A
	dc.l	$554B557F
	dc.l	$7FFE1039
	dc.l	$202E282E
	dc.l	$3839362A
	dc.l	$17FDE90C
	dc.l	$30E7F3E4
	dc.l	$C4BDA5A6
	dc.l	$A8ABB9C4
	dc.l	$CDD5FB48
	dc.l	$B308E6
	dc.l	$DEEADEE5
	dc.l	$DDE0E2E9
	dc.l	$F1E5CEA4
	dc.l	$839CE6E6
	dc.l	$FD040E1A
	dc.l	$24333032
	dc.l	$3826D8BF
	dc.l	$5B500719
	dc.l	$F9EADBC8
	dc.l	$CACEDEF3
	dc.l	$31F397F
	dc.l	$7F4E3815
	dc.l	$FDF6F3E4
	dc.l	$DADEEAF9
	dc.l	$1340782D
	dc.l	$D638493F
	dc.l	$3C2D2B23
	dc.l	$17120B09
	dc.l	$5EABE83
	dc.l	$83CFCDDE
	dc.l	$DCDBE5EB
	dc.l	$F6001021
	dc.l	$24F5B42F
	dc.l	$4DFE14FD
	dc.l	$10BFCE5
	dc.l	$D6DDEC00
	dc.l	$130DEC1C
	dc.l	$7011F17
	dc.l	$24201812
	dc.l	$300FAF1
	dc.l	$E3A7BB43
	dc.l	$BE1F4F3
	dc.l	$5FE05FB
	dc.l	$F5F7F2DC
	dc.l	$C6E2E8BE
	dc.l	$D0E00F9
	dc.l	$F00B07FC
	dc.l	$20F2346
	dc.l	$736008D9
	dc.l	$EA1F00F2
	dc.l	$FA041129
	dc.l	$290D00EC
	dc.l	$F9345E2B
	dc.l	$1206F0DC
	dc.l	$CFC5C1BE
	dc.l	$BDC3C0AE
	dc.l	$83860F27
	dc.l	$EFFAEFE4
	dc.l	$F0F5E7DE
	dc.l	$EF0B2240
	dc.l	$3A2C6942
	dc.l	$38473A3E
	dc.l	$322B2012
	dc.l	$E050D15
	dc.l	$EBD526E4
	dc.l	$E003F800
	dc.l	$20501
	dc.l	$3040611
	dc.l	$2567601E
	dc.l	$1507FAFA
	dc.l	$809EFE1
	dc.l	$DDD7C4CD
	dc.l	$3024C7FF
	dc.l	$EBEF00FC
	dc.l	$FF010400
	dc.l	$FCEEE5F4
	dc.l	$363DFAEF
	dc.l	$C6B3B3A5
	dc.l	$9DA5AAB3
	dc.l	$C4DD0141
	dc.l	$27B9B9F6
	dc.l	$E9F3FF0C
	dc.l	$28201516
	dc.l	$1F303F3E
	dc.l	$57772B1B
	dc.l	$160602FE
	dc.l	$FDFEFAEA
	dc.l	$DFC5A4BE
	dc.l	$15242102
	dc.l	$FEF9EDEF
	dc.l	$E8E7EEFD
	dc.l	$102325F0
	dc.l	$BCD7DEFB
	dc.l	$D091E38
	dc.l	$453B2F2C
	dc.l	$2A36617F
	dc.l	$25DF3219
	dc.l	$B0C0312
	dc.l	$191E1C13
	dc.l	$6F6FB36
	dc.l	$13EAFACD
	dc.l	$C6BEB0AF
	dc.l	$B5B8C3D0
	dc.l	$DBE6233D
	dc.l	$B9DB15EE
	dc.l	$FCEBF203
	dc.l	$2F4F2FF
	dc.l	$F3DB9F
	dc.l	$83DF0008
	dc.l	$161B2833
	dc.l	$3232393C
	dc.l	$3D1FCE00
	dc.l	$7F2D1010
	dc.l	$F4E9C9BE
	dc.l	$BFC2D3E7
	dc.l	$142971
	dc.l	$5F2220FC
	dc.l	$E5D7DEF2
	dc.l	$F8F9F809
	dc.l	$205372FE
	dc.l	$D8393A3C
	dc.l	$36252018
	dc.l	$1409FFFE
	dc.l	$ECCE9383
	dc.l	$9ABDBFCE
	dc.l	$CBD2D5D7
	dc.l	$E3F20D18
	dc.l	$13C2BB48
	dc.l	$4F507F0
	dc.l	$F5F500FF
	dc.l	$EFF00020
	dc.l	$2C000327
	dc.l	$9253036
	dc.l	$39343227
	dc.l	$1B150D04
	dc.l	$CBB14651
	dc.l	$FD0E0400
	dc.l	$FEFCFBF2
	dc.l	$EFE9DDC8
	dc.l	$D3E9B4F8
	dc.l	$13FA01F3
	dc.l	$FD0E1B19
	dc.l	$121A2E53
	dc.l	$5309E2D4
	dc.l	$F3F7FD09
	dc.l	$B101710
	dc.l	$3FBF0F8
	dc.l	$396E4125
	dc.l	$EFAF5E0
	dc.l	$CAC5BDB5
	dc.l	$B6B7A083
	dc.l	$860F25F1
	dc.l	$F3E4D6D4
	dc.l	$DEEFF0F2
	dc.l	$FE152D24
	dc.l	$29673034
	dc.l	$483B3E33
	dc.l	$2E282216
	dc.l	$D130DD1
	dc.l	$EC3CE4FC
	dc.l	$9F80504
	dc.l	$60503FE
	dc.l	$FB00143D
	dc.l	$7F551914
	dc.l	$FFF5EAEF
	dc.l	$FBFDEDD9
	dc.l	$CDC1D92C
	dc.l	$E4D413EE
	dc.l	$FDFDF1F5
	dc.l	$FD000006
	dc.l	$FEEFFF2E
	dc.l	$1BFEF1CB
	dc.l	$BDAB9E9E
	dc.l	$A0A5B0C4
	dc.l	$DC0826EB
	dc.l	$BBE0F5F3
	dc.l	$FCFE0D23
	dc.l	$2A28201F
	dc.l	$2A343864
	dc.l	$4E1B2318
	dc.l	$130F0501
	dc.l	$FFF6F0E3
	dc.l	$CBB3EA24
	dc.l	$301C0B0E
	dc.l	$AFFF5F8
	dc.l	$FEFA0012
	dc.l	$15FBC7D6
	dc.l	$E1E90102
	dc.l	$A152537
	dc.l	$3A312527
	dc.l	$416F2ED6
	dc.l	$141E161F
	dc.l	$1A21242B
	dc.l	$2B1F0BF2
	dc.l	$F319FEEB
	dc.l	$FED6CDC2
	dc.l	$B9BCC0C5
	dc.l	$CAD0D9EC
	dc.l	$231CB1E3
	dc.l	$1E9F7E6
	dc.l	$E2F0FF06
	dc.l	$3050E06
	dc.l	$DD9FABED
	dc.l	$FB0B1715
	dc.l	$1A212327
	dc.l	$30332F03
	dc.l	$CC29661A
	dc.l	$190AF4E9
	dc.l	$D8D2D0D3
	dc.l	$DEED0010
	dc.l	$32653E27
	dc.l	$16FCECDB
	dc.l	$D9E8FA00
	dc.l	$10E2855
	dc.l	$39D7FF30
	dc.l	$232F2B26
	dc.l	$201C180F
	dc.l	$E09F4CF
	dc.l	$949DD5CD
	dc.l	$D4DEDED8
	dc.l	$D6DAD9E0
	dc.l	$EEFDEBA6
	dc.l	$E22EE2F5
	dc.l	$F9EBECE4
	dc.l	$F1FD060A
	dc.l	$A1512EC
	dc.l	$81D0E38
	dc.l	$3C393D39
	dc.l	$38393220
	dc.l	$12FDC9D6
	dc.l	$4A480D13
	dc.l	$601FDFA
	dc.l	$F5F1ECE4
	dc.l	$DFD3DBD7
	dc.l	$B1F1FDF6
	dc.l	$600F7FA
	dc.l	$C191A1D
	dc.l	$2D4B3BFB
	dc.l	$E1DAF5F9
	dc.l	$FF020207
	dc.l	$904FEFB
	dc.l	$F0F92956
	dc.l	$352311FA
	dc.l	$F1E8E5DF
	dc.l	$D2CAC7C2
	dc.l	$B18A9D00
	dc.l	$2403F9F1
	dc.l	$E7DCD6E1
	dc.l	$F7030913
	dc.l	$2115134C
	dc.l	$28223630
	dc.l	$342D2829
	dc.l	$281A1215
	dc.l	$6D3EF3E
	dc.l	$FCFD0B06
	dc.l	$140D0907
	dc.l	$FCF7F9
	dc.l	$6265E3E
	dc.l	$131000F5
	dc.l	$ECE2E7F4
	dc.l	$EACE
	dc.l	$DE19E3DE
	dc.l	$12FB0402
	dc.l	$30A15
	dc.l	$1104FBF5
	dc.l	$2B1F05
	dc.l	$F8D5C5B3
	dc.l	$A19E9E9D
	dc.l	$A3B2D303
	dc.l	$10D3ACB9
	dc.l	$D9E1F1F4
	dc.l	$F4011223
	dc.l	$2C2E3741
	dc.l	$416C5A22
	dc.l	$2A1E1915
	dc.l	$C0701FC
	dc.l	$F7EDD3B5
	dc.l	$DE253E30
	dc.l	$191005FF
	dc.l	$F7F3F4F4
	dc.l	$F7030D00
	dc.l	$CDC4EEF4
	dc.l	$3091012
	dc.l	$101B2E34
	dc.l	$302F3A64
	dc.l	$4FE2F51E
	dc.l	$B1B1A1C
	dc.l	$20202116
	dc.l	$AFDEC01
	dc.l	$19E700FB
	dc.l	$E0E2DAD4
	dc.l	$CFCFD0D3
	dc.l	$D5D9F52B
	dc.l	$FEA8F501
	dc.l	$FCFEE9E4
	dc.l	$DFE7F806
	dc.l	$6FFEECA
	dc.l	$8FA6E5F2
	dc.l	$7141A21
	dc.l	$27303C39
	dc.l	$383502CB
	dc.l	$2C621C1D
	dc.l	$1005FBE2
	dc.l	$DDD9D8DD
	dc.l	$E9FC0721
	dc.l	$552E1914
	dc.l	$FEF3DED3
	dc.l	$D9EAFB0B
	dc.l	$1B385E34
	dc.l	$D3EC201C
	dc.l	$292A2B2F
	dc.l	$27201B1A
	dc.l	$12FDD591
	dc.l	$94C8CED5
	dc.l	$D8D9D7D5
	dc.l	$DADFE2EE
	dc.l	$FCEEA5D7
	dc.l	$31EEF6F6
	dc.l	$ECECDCDC
	dc.l	$ED000811
	dc.l	$1C0FE4FE
	dc.l	$13FC212E
	dc.l	$363E3E38
	dc.l	$332B2218
	dc.l	$FFCBE856
	dc.l	$491C2516
	dc.l	$18170700
	dc.l	$FDF9F4E2
	dc.l	$D3E3CDAE
	dc.l	$F7FAF7FD
	dc.l	$F5F3EEF9
	dc.l	$A1A2432
	dc.l	$4A28EFD7
	dc.l	$D5EFF300
	dc.l	$5040910
	dc.l	$1409FDFF
	dc.l	$9304C28
	dc.l	$1E06F5ED
	dc.l	$DCD4D2CC
	dc.l	$C7C4BBA4
	dc.l	$83A90214
	dc.l	$F7F6F2EA
	dc.l	$E2D9D9E7
	dc.l	$FE122637
	dc.l	$2B2C5622
	dc.l	$20322A30
	dc.l	$2B231B16
	dc.l	$12101403
	dc.l	$CCF13DFE
	dc.l	$8120512
	dc.l	$D0A0E0A
	dc.l	$F9FC08
	dc.l	$2A624F35
	dc.l	$270901F5
	dc.l	$E5DCDCE4
	dc.l	$E6E0CED3
	dc.l	$11ECD507
	dc.l	$F2FF0303
	dc.l	$90A0D0F
	dc.l	$C050007
	dc.l	$2F340F0D
	dc.l	$F8E3CEC0
	dc.l	$B2A2A0A2
	dc.l	$A7B5DBF0
	dc.l	$BEA0A0C2
	dc.l	$D1DEEDF2
	dc.l	$F6FC0A1D
	dc.l	$29333F41
	dc.l	$67652C29
	dc.l	$23272D20
	dc.l	$1514140C
	dc.l	$FAE2BBC5
	dc.l	$C382E1B
	dc.l	$170B04FC
	dc.l	$FCFCF9FA
	dc.l	$1F6C8
	dc.l	$B7DAE2F8
	dc.l	$40E191D
	dc.l	$201F2831
	dc.l	$38537F53
	dc.l	$DBDB04FC
	dc.l	$C151819
	dc.l	$1B1D150B
	dc.l	$FDF1080D
	dc.l	$D904FFE7
	dc.l	$E5D6D5D5
	dc.l	$DEEDE8E0
	dc.l	$E70432EC
	dc.l	$B00601F5
	dc.l	$F9ECE9DE
	dc.l	$DCE8F702
	dc.l	$2F0C288
	dc.l	$B4DFEB05
	dc.l	$D192226
	dc.l	$2B354045
	dc.l	$31EED447
	dc.l	$4E18250E
	dc.l	$500EFE4
	dc.l	$E5E7EAF3
	dc.l	$62E54
	dc.l	$201A1000
	dc.l	$F8E4D7D3
	dc.l	$DCEE031F
	dc.l	$4B611CCD
	dc.l	$EC100F23
	dc.l	$27262625
	dc.l	$2B302B21
	dc.l	$9D28FA8
	dc.l	$C6CEDBD9
	dc.l	$DBDADCDE
	dc.l	$E0E4EBF8
	dc.l	$E6A6E920
	dc.l	$D4E8E5DF
	dc.l	$E4D7D1CE
	dc.l	$D9EE0218
	dc.l	$1BF2FF10
	dc.l	$F91A2734
	dc.l	$3F403F3B
	dc.l	$372D2207
	dc.l	$E0F54152
	dc.l	$3532251B
	dc.l	$140A0703
	dc.l	$FEF3E7DD
	dc.l	$E2D9BBF6
	dc.l	$6FA0000
	dc.l	$FCF6F1F4
	dc.l	$102B41
	dc.l	$3103E5CB
	dc.l	$E2EBF5FE
	dc.l	$50204
	dc.l	$FDFAFC
	dc.l	$13372015
	dc.l	$1200FBF1
	dc.l	$F1ECDFDD
	dc.l	$D6CFC0A1
	dc.l	$AFF21000
	dc.l	$F4F6F0EE
	dc.l	$E9E0E3E7
	dc.l	$FA0D1F23
	dc.l	$10382506
	dc.l	$181B2524
	dc.l	$1F201B1A
	dc.l	$1F1609E6
	dc.l	$E2342203
	dc.l	$150C1210
	dc.l	$D100B0A
	dc.l	$1FA0011
	dc.l	$433A1E19
	dc.l	$200FAF5
	dc.l	$EDE1DFE2
	dc.l	$E1E0DF11
	dc.l	$14E20A02
	dc.l	$405060A
	dc.l	$B10130D
	dc.l	$C0B071C
	dc.l	$2F0F07F9
	dc.l	$E6D5C7BA
	dc.l	$B1B3B0B1
	dc.l	$BBCBDABB
	dc.l	$AEA2ADD6
	dc.l	$D9EDF5F1
	dc.l	$F7F5FF0B
	dc.l	$1A2C3959
	dc.l	$67302420
	dc.l	$1D221E1B
	dc.l	$1612130A
	dc.l	$FDE5D3FE
	dc.l	$2C33221C
	dc.l	$19151921
	dc.l	$B090803
	dc.l	$2FFE4BA
	dc.l	$D9EBEBFE
	dc.l	$30C120E
	dc.l	$C090E19
	dc.l	$2B505D17
	dc.l	$DFF4F5F7
	dc.l	$4071014
	dc.l	$201D170E
	dc.l	$F5FA0CE7
	dc.l	$E204F1ED
	dc.l	$E7E2DFE3
	dc.l	$E6EAEBE8
	dc.l	$F21514BB
	dc.l	$DB04EEF8
	dc.l	$F5F1EBE4
	dc.l	$E4EDFEFF
	dc.l	$FDF4CABE
	dc.l	$E6E9F505
	dc.l	$10171B20
	dc.l	$272B2E2F
	dc.l	$BD9064E
	dc.l	$1F191D08
	dc.l	$4F9EDED
	dc.l	$EEF5FC0B
	dc.l	$E0B3E30
	dc.l	$12200FFE
	dc.l	$F8EDE3DA
	dc.l	$E1EC0020
	dc.l	$4430F5E7
	dc.l	$2090E1D
	dc.l	$22252323
	dc.l	$2824221C
	dc.l	$FEC6BDD4
	dc.l	$D7E1E3E6
	dc.l	$EBEEE7E8
	dc.l	$E9EAF0E9
	dc.l	$B6BC01D0
	dc.l	$D2DAD8DF
	dc.l	$D9D8D3D0
	dc.l	$D9EEFD04
	dc.l	$F2FB240A
	dc.l	$D1D2836
	dc.l	$393B3B39
	dc.l	$3E3110F5
	dc.l	$3D4C30
	dc.l	$2C251E18
	dc.l	$10090502
	dc.l	$FCEBE1E1
	dc.l	$CDADE7FD
	dc.l	$FE02
	dc.l	$FBF7F5F7
	dc.l	$18393B
	dc.l	$17F3DDE7
	dc.l	$E2ECF700
	dc.l	$205FF
	dc.l	$FBFEFF1C
	dc.l	$3B1B110B
	dc.l	$303FBEE
	dc.l	$E8E4E2DD
	dc.l	$D4BC9BB5
	dc.l	$F30AFE00
	dc.l	$FDEEF1EA
	dc.l	$E4E0E5F4
	dc.l	$1151217
	dc.l	$46200F15
	dc.l	$1020272C
	dc.l	$201B1D15
	dc.l	$1200D800
	dc.l	$38010710
	dc.l	$D161A1C
	dc.l	$15120E03
	dc.l	$1092A47
	dc.l	$271C0F01
	dc.l	$FFF5F4F1
	dc.l	$EFE3D7D8
	dc.l	$CDE617F4
	dc.l	$FE10F2F6
	dc.l	$F8FF0206
	dc.l	$C151C16
	dc.l	$F19361E
	dc.l	$403ECDD
	dc.l	$D2C4B7B5
	dc.l	$B8C0C8C9
	dc.l	$DAC1ADA7
	dc.l	$9CBDC1D0
	dc.l	$E1E6EEF0
	dc.l	$F1F60015
	dc.l	$2B53784E
	dc.l	$36241518
	dc.l	$191B1917
	dc.l	$16110CFE
	dc.l	$DEFE2E39
	dc.l	$26242417
	dc.l	$15110B0E
	dc.l	$110C0800
	dc.l	$EDBECAE8
	dc.l	$F30D080B
	dc.l	$15120E0A
	dc.l	$A0C1D47
	dc.l	$5E25EEFF
	dc.l	$FFF1F903
	dc.l	$B0F100E
	dc.l	$900F5F8
	dc.l	$DE6D8FC
	dc.l	$ECECEDEB
	dc.l	$F2F6F1F3
	dc.l	$F5F3F716
	dc.l	$1ECBE00A
	dc.l	$EDF9F5EF
	dc.l	$F1EAE4E1
	dc.l	$E7EBE8E1
	dc.l	$B8B9EEF0
	dc.l	$F1F7000F
	dc.l	$181A2431
	dc.l	$393010DE
	dc.l	$1471D1E
	dc.l	$210705FA
	dc.l	$EFF0F5FA
	dc.l	$FF050709
	dc.l	$3D2F1018
	dc.l	$5FFF9F1
	dc.l	$E9E5E3E7
	dc.l	$F816372E
	dc.l	$1F31212
	dc.l	$12222426
	dc.l	$28282C2E
	dc.l	$2E24FEC5
	dc.l	$C7D2D6E7
	dc.l	$E9E8EBE9
	dc.l	$E7EAF1F5
	dc.l	$F8EEB5C5
	dc.l	$F7C5D7E0
	dc.l	$D2DCE2DD
	dc.l	$D0CBCFD6
	dc.l	$E4ECD5E8
	dc.l	$13061215
	dc.l	$1E30373D
	dc.l	$3E3C392B
	dc.l	$15FF1745
	dc.l	$50332F2B
	dc.l	$23232524
	dc.l	$1E1408FB
	dc.l	$E9E5C8AD
	dc.l	$E3EEF1F7
	dc.l	$F7FEFDFC
	dc.l	$F7F6FB0D
	dc.l	$272903E9
	dc.l	$E1F4E7DF
	dc.l	$EAF3FAFD
	dc.l	$4111606
	dc.l	$41F3A18
	dc.l	$906F8F4
	dc.l	$EFE7E3E3
	dc.l	$E3E1DBC5
	dc.l	$A6B5EB0C
	dc.l	$1F6F9F2
	dc.l	$F0EEEBEA
	dc.l	$E9EF0016
	dc.l	$14114B36
	dc.l	$1E26130F
	dc.l	$11181812
	dc.l	$16141006
	dc.l	$E1E63519
	dc.l	$FA110B0F
	dc.l	$14141212
	dc.l	$120D070B
	dc.l	$23534024
	dc.l	$27190D00
	dc.l	$FDF5EDE6
	dc.l	$DACFC6C4
	dc.l	$F100E91D
	dc.l	$1300F9F5
	dc.l	$FD030B0D
	dc.l	$10131314
	dc.l	$27442513
	dc.l	$2EFE5D6
	dc.l	$D1CEC2BD
	dc.l	$C2C8D0CE
	dc.l	$ACB0A097
	dc.l	$B8B9CBD5
	dc.l	$DEE8E7EA
	dc.l	$ECF10013
	dc.l	$415C3B43
	dc.l	$3C201214
	dc.l	$1614151B
	dc.l	$1E11F3E0
	dc.l	$5343922
	dc.l	$24201B1B
	dc.l	$14101414
	dc.l	$141408EE
	dc.l	$C2D7EAEC
	dc.l	$40D191A
	dc.l	$1817110B
	dc.l	$B193A4E
	dc.l	$21EDFB0F
	dc.l	$F7F4FF
	dc.l	$8090906
	dc.l	$F4FA12
	dc.l	$F2D5F9EA
	dc.l	$ECEDECF7
	dc.l	$F8F1F4F9
	dc.l	$F7F90D17
	dc.l	$D8C8FAF7
	dc.l	$FEFAF0F3
	dc.l	$EFECE9E8
	dc.l	$E5E3DFC6
	dc.l	$B7E5FC02
	dc.l	$5FC000D
	dc.l	$1B292A2A
	dc.l	$2B1CF5ED
	dc.l	$2C290912
	dc.l	$B060607
	dc.l	$5FE0008
	dc.l	$D0D0218
	dc.l	$320D0C0B
	dc.l	$FDFCF7F0
	dc.l	$EEEAF0FA
	dc.l	$111B04
	dc.l	$EAF11516
	dc.l	$E0B0F17
	dc.l	$1F242B36
	dc.l	$362502DC
	dc.l	$E6DDDEEA
	dc.l	$EAEDEDEC
	dc.l	$ECEEF3FC
	dc.l	$7FDC8D3
	dc.l	$F0C2CACE
	dc.l	$CACDCED1
	dc.l	$CECDD0D2
	dc.l	$D9DFD8ED
	dc.l	$E082027
	dc.l	$1B1E2B31
	dc.l	$3336352B
	dc.l	$1C183346
	dc.l	$4B392F2C
	dc.l	$25211B14
	dc.l	$12120E07
	dc.l	$FDF6D5C0
	dc.l	$ECF70105
	dc.l	$FEFFFDFD
	dc.l	$FBF5F6FE
	dc.l	$1010F5E9
	dc.l	$EDFDFAF4
	dc.l	$E5E4F0F7
	dc.l	$F6F7FBFB
	dc.l	$FF19300F
	dc.l	$300F7F7
	dc.l	$F5F6FBF4
	dc.l	$EDEFE9D5
	dc.l	$BCCBF003
	dc.l	$100FFF2
	dc.l	$F4F1EDEC
	dc.l	$ECEFF3FC
	dc.l	$F7F8281B
	dc.l	$14271B11
	dc.l	$B0E1014
	dc.l	$181C1E0A
	dc.l	$E9003811
	dc.l	$E101B
	dc.l	$16121312
	dc.l	$100D0D0D
	dc.l	$223D241C
	dc.l	$170902FB
	dc.l	$FAF9F4EE
	dc.l	$E4D9CBD5
	dc.l	$2FC001C
	dc.l	$151403FB
	dc.l	$60B080D
	dc.l	$12111219
	dc.l	$31290701
	dc.l	$F3EDE6D7
	dc.l	$D2CBC8CD
	dc.l	$D3D9DCC0
	dc.l	$B5C2A3B5
	dc.l	$C8CED2D5
	dc.l	$E0EBF0E8
	dc.l	$E6ECF514
	dc.l	$36232329
	dc.l	$241A090B
	dc.l	$12151517
	dc.l	$1505F207
	dc.l	$353C2826
	dc.l	$231E2225
	dc.l	$2721222C
	dc.l	$261500D9
	dc.l	$D9EDEFFC
	dc.l	$FC010808
	dc.l	$B0B0604
	dc.l	$B242801
	dc.l	$E0EF0207
	dc.l	$6FAF4FB
	dc.l	$105080E
	dc.l	$F1017EF
	dc.l	$E5F8E6EB
	dc.l	$EAE7ECEE
	dc.l	$F0F70003
	dc.l	$C2210CE
	dc.l	$E6FDF2F9
	dc.l	$F1F2F6F1
	dc.l	$EEF0EDE7
	dc.l	$E2DCCAD9
	dc.l	$F0FB0E0E
	dc.l	$3FE0209
	dc.l	$121B2020
	dc.l	$5E51237
	dc.l	$E0D0B04
	dc.l	$4FCFBFB
	dc.l	$FC020C13
	dc.l	$C143B27
	dc.l	$1C1B140C
	dc.l	$FBF5F2EF
	dc.l	$EBEDF703
	dc.l	$AFAE5E4
	dc.l	$3141F1B
	dc.l	$E121C21
	dc.l	$272E2F22
	dc.l	$1E8F5E6
	dc.l	$F100F8F7
	dc.l	$FB00FEF8
	dc.l	$FB0003EF
	dc.l	$C1DEE1BB
	dc.l	$C9C4C2C8
	dc.l	$C9C9C7C9
	dc.l	$C8CAD0C9
	dc.l	$B7D5ECF3
	dc.l	$1427322F
	dc.l	$262C3234
	dc.l	$37352221
	dc.l	$3A454B35
	dc.l	$312D2422
	dc.l	$1F1B1918
	dc.l	$170F05F7
	dc.l	$CEDEF7F3
	dc.l	$FDFCFE09
UbL005828	dc.l	$700
	dc.l	$1AD003F
	dc.l	UbL004A28
UbL005834	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL005828
UbL005842	dc.l	UbL005834
	dc.l	0
	dc.l	0
	dc.l	$1A0A00
	dc.w	$FFFE

;	SECTION	UltSnd005854,DATA,CHIP
UbL005854	dc.l	$4026EBE0
	dc.l	$EDECDDA6
	dc.l	$9CE306E8
	dc.l	$D6D6F00E
	dc.l	$ECACC0F4
	dc.l	$FCEEDADF
	dc.l	$223F0EF6
	dc.l	$11232807
	dc.l	$D3F43735
	dc.l	$1D203259
	dc.l	$5F220520
	dc.l	$2611FAE8
	dc.l	$E441BE0
	dc.l	$E1F5FBED
	dc.l	$B7B80012
	dc.l	$F0E8F215
	dc.l	$2EF5BFE4
	dc.l	$1F9E8D8
	dc.l	$FC4137FD
	dc.l	$F80D1309
	dc.l	$D3C81035
	dc.l	$1504071E
	dc.l	$32FEBEDE
	dc.l	$806F8E5
	dc.l	$FA4134F5
	dc.l	$F0070D03
	dc.l	$C9B9FF1A
	dc.l	$F8F1FC1F
	dc.l	$33F3BAE2
	dc.l	$FEF4E8DC
	dc.l	$54828EF
	dc.l	$F4070F00
	dc.l	$CFDA252E
	dc.l	$8061745
	dc.l	$571BFB14
	dc.l	$15FCDFD2
	dc.l	$C3D07DF
	dc.l	$F2F9F7CE
	dc.l	$A3D51609
	dc.l	$F6F90C2D
	dc.l	$AC4D606
	dc.l	$BFEEBFA
	dc.l	$3E3D0000
	dc.l	$1B2419DE
	dc.l	$D81F3110
	dc.l	$9143434
	dc.l	$E5BCEAFF
	dc.l	$F3E3DE1A
	dc.l	$3EFCD3E7
	dc.l	$F2F4C8A3
	dc.l	$DE0DE9D6
	dc.l	$DF0022F1
	dc.l	$B2D9F8EC
	dc.l	$DFD30148
	dc.l	$25F0FF0F
	dc.l	$12FACBF7
	dc.l	$3F2E1113
	dc.l	$294E3A02
	dc.l	$20462A08
	dc.l	$EBFB432F
	dc.l	$EEF60604
	dc.l	$E8AAC114
	dc.l	$14F4F807
	dc.l	$2B11BECE
	dc.l	$D1808F0
	dc.l	$2452DE6
	dc.l	$E6FBFFDF
	dc.l	$99B00301
	dc.l	$EBF60B34
	dc.l	$19C5D207
	dc.l	$8FFF612
	dc.l	$5A400009
	dc.l	$20260CD2
	dc.l	$EF352000
	dc.l	$2234A0F
	dc.l	$BDE206FF
	dc.l	$F9E21A4D
	dc.l	$8E4F7FF
	dc.l	$F5C7ABF2
	dc.l	$1CEEE0E9
	dc.l	$D20D6B4
	dc.l	$F304FDFD
	dc.l	$34649FB
	dc.l	$E5F5F3E8
	dc.l	$ADB00F0F
	dc.l	$E1EDFF22
	dc.l	$BC4E51B
	dc.l	$170C012D
	dc.l	$7F55193E
	dc.l	$3827F7C0
	dc.l	$22FFBF3
	dc.l	$FD191BC1
	dc.l	$B4F9FAE6
	dc.l	$D6E43634
	dc.l	$F0FE1917
	dc.l	$F9BBE937
	dc.l	$1F080F29
	dc.l	$37EBB9F4
	dc.l	$9F2DEE3
	dc.l	$2E3CECE8
	dc.l	$1018F2A8
	dc.l	$C11508F0
	dc.l	$FB0E28E2
	dc.l	$9DD7F3E2
	dc.l	$D4D11E37
	dc.l	$EFF42532
	dc.l	$15C5CF1E
	dc.l	$3E7F40D
	dc.l	$2CE7A0D6
	dc.l	$F1E6E0EA
	dc.l	$3640F809
	dc.l	$333B28EB
	dc.l	$F614328
	dc.l	$374D54FB
	dc.l	$C5FD00E6
	dc.l	$D7EC3E21
	dc.l	$CFE60106
	dc.l	$DEB3FC2B
	dc.l	$FFFE163D
	dc.l	$3CDED612
	dc.l	$5F1E815
	dc.l	$5310DE01
	dc.l	$D06CABE
	dc.l	$1A19EFFE
	dc.l	$113401AD
	dc.l	$D7F6DCD0
	dc.l	$D62A39DA
	dc.l	$D7F3F4DB
	dc.l	$A2D72200
	dc.l	$F61B4950
	dc.l	$F0C4F6F1
	dc.l	$DACBEB3D
	dc.l	$16CBE4F8
	dc.l	$F9D5B000
	dc.l	$32090C28
	dc.l	$5756FE08
	dc.l	$422D160A
	dc.l	$3B7830F5
	dc.l	$506FCBE
	dc.l	$B50E13E2
	dc.l	$EA003007
	dc.l	$B1DB01F0
	dc.l	$E6E12561
	dc.l	$18F41112
	dc.l	$5C3C82E
	dc.l	$25F5FB0C
	dc.l	$3606B7EA
	dc.l	$9F3DDD9
	dc.l	$265203E5
	dc.l	$FDFCECA8
	dc.l	$AC1113E6
	dc.l	$F40423FD
	dc.l	$A6DA09FE
	dc.l	$FBF33159
	dc.l	$6E5FCFB
	dc.l	$F0A7A105
	dc.l	$1BEDF103
	dc.l	$2611B5D5
	dc.l	$161206FB
	dc.l	$28774820
	dc.l	$4C494205
	dc.l	$CE1D4A0E
	dc.l	$50D3025
	dc.l	$B5ACEEF1
	dc.l	$DECEDD36
	dc.l	$30D8EF13
	dc.l	$1504BFD9
	dc.l	$3520000C
	dc.l	$1F4D18C6
	dc.l	$E704FBE9
	dc.l	$D614580D
	dc.l	$D7000803
	dc.l	$D5A1EE24
	dc.l	$EDE6F51D
	dc.l	$31D0A8E5
	dc.l	$E8D7C9CB
	dc.l	$2F4AFB00
	dc.l	$221C11C8
	dc.l	$B41817E0
	dc.l	$E3F32212
	dc.l	$ADBBF4E9
	dc.l	$E2D2EB50
	dc.l	$4E081B38
	dc.l	$352AE900
	dc.l	$67592927
	dc.l	$315225BB
	dc.l	$D31108E8
	dc.l	$C7E53E1E
	dc.l	$D2ED0300
	dc.l	$F0A9C629
	dc.l	$24020E1E
	dc.l	$4B27C1D1
	dc.l	$B05F1DC
	dc.l	$F94F2BDE
	dc.l	$EF0A0FFC
	dc.l	$B4D33116
	dc.l	$F2FA0331
	dc.l	$24B8B1EF
	dc.l	$E8D8C6D8
	dc.l	$3C34D9DE
	dc.l	$1131FD5
	dc.l	$C92220EA
	dc.l	$E7E81424
	dc.l	$C4A4E8F0
	dc.l	$E5DCCB1E
	dc.l	$560FF51D
	dc.l	$292CFCD5
	dc.l	$25674133
	dc.l	$3443611E
	dc.l	$C3F014FF
	dc.l	$ECC9E844
	dc.l	$16C9E1FB
	dc.l	$FFF4ABB6
	dc.l	$212CFA07
	dc.l	$144050F2
	dc.l	$C0F708F6
	dc.l	$E9DC1F63
	dc.l	$19DCF906
	dc.l	$12F3ADE3
	dc.l	$3717F1F2
	dc.l	$FA2B29C2
	dc.l	$B6F8F8EA
	dc.l	$D9C21647
	dc.l	$FBDF050F
	dc.l	$1AF7B6E5
	dc.l	$2D0BF0EF
	dc.l	$F51D0AA5
	dc.l	$A5E6E8DF
	dc.l	$D2CB1E4B
	dc.l	$3E91627
	dc.l	$3713D916
	dc.l	$643F2429
	dc.l	$376C61EC
	dc.l	$C4F9FDEC
	dc.l	$DFD01556
	dc.l	$9D0EEFA
	dc.l	$7F6B4D4
	dc.l	$2D290003
	dc.l	$9354DF9
	dc.l	$C0FE1D0A
	dc.l	$F0CCFB56
	dc.l	$33F1000D
	dc.l	$6FFB1A0
	dc.l	$24F5EE
	dc.l	$F00A32F6
	dc.l	$96B9EEE6
	dc.l	$DECECB27
	dc.l	$3FF0DE0C
	dc.l	$212E03B6
	dc.l	$CD16FCDA
	dc.l	$EAFB252A
	dc.l	$CBA4E5F7
	dc.l	$ECE6D707
	dc.l	$62521724
	dc.l	$3F3F4101
	dc.l	$E02D6738
	dc.l	$25222548
	dc.l	$2CCCC9FD
	dc.l	$F8E8DAC5
	dc.l	$FE480DCC
	dc.l	$E3FA0006
	dc.l	$C6B10030
	dc.l	$4FC0212
	dc.l	$402CCCC7
	dc.l	$100EEE1
	dc.l	$CC0A5B2B
	dc.l	$F0041403
	dc.l	$BD99ED
	dc.l	$3D17FBF6
	dc.l	$F72224C7
	dc.l	$A9E900F4
	dc.l	$EAD9FD66
	dc.l	$5F0CF404
	dc.l	$FE02D793
	dc.l	$B70C03E2
	dc.l	$EEF8213C
	dc.l	$B4DA0E
	dc.l	$110B00F5
	dc.l	$3E7B4514
	dc.l	$252C2822
	dc.l	$DEB9FD35
	dc.l	$9F6F5F6
	dc.l	$1B20C69D
	dc.l	$D8F3EAE6
	dc.l	$D7DE3347
	dc.l	$6E6081D
	dc.l	$2D25DAC5
	dc.l	$F340803
	dc.l	$716423C
	dc.l	$DEC7000C
	dc.l	$FFF7DFE6
	dc.l	$3C4A02E5
	dc.l	$FCFB00F1
	dc.l	$ACB40A22
	dc.l	$F8F3F301
	dc.l	$2F2DDCCE
	dc.l	$B10F7E5
	dc.l	$C5C61926
	dc.l	$DFBFDDE4
	dc.l	$F1EEB1AB
	dc.l	$F7260400
	dc.l	$C1A4D63
	dc.l	$1EF82637
	dc.l	$261808FD
	dc.l	$46732EF6
	dc.l	$50002
	dc.l	$C59AD318
	dc.l	$4F2FBFD
	dc.l	$1F4414C0
	dc.l	$D6050B03
	dc.l	$EB0F5E
	dc.l	$4807F807
	dc.l	$212FEC0
	dc.l	$CC1B2DFE
	dc.l	$FAF4FE26
	dc.l	$23CCACE3
	dc.l	$F4E7DFC9
	dc.l	$C11333F5
	dc.l	$D3ECF6FD
	dc.l	$1C9B80A
	dc.l	$4C1CFFF7
	dc.l	$F3182EEA
	dc.l	$ADD6F0E3
	dc.l	$DDD6CC12
	dc.l	$5121F409
	dc.l	$1E253508
	dc.l	$E61C6249
	dc.l	$2F302740
	dc.l	$510EC3E6
	dc.l	$E05F7E0
	dc.l	$C8043D08
	dc.l	$D6E8FBFB
	dc.l	$7D4AAE1
	dc.l	$240DFB04
	dc.l	$A334800
	dc.l	$BEEA06FB
	dc.l	$F3DFDA2B
	dc.l	$5010EA04
	dc.l	$C1103B9
	dc.l	$AC002B08
	dc.l	$4020528
	dc.l	$18BAB2ED
	dc.l	$F5EBE6CE
	dc.l	$EE49440E
	dc.l	$D16060B
	dc.l	$E0A5C709
	dc.l	$F8DEEAF0
	dc.l	$1936F6BB
	dc.l	$E9070407
	dc.l	$15068
	dc.l	$2B142A26
	dc.l	$2A15D2D8
	dc.l	$2223F9F9
	dc.l	$F30524F6
	dc.l	$A9C8F1EA
	dc.l	$E6DCD113
	dc.l	$430FE704
	dc.l	$F1E1EE5
	dc.l	$E52C3208
	dc.l	$A0A2347
	dc.l	$13C8E505
	dc.l	$FCF6EDE8
	dc.l	$304D0EEA
	dc.l	$FEFC00F1
	dc.l	$B4C5161D
	dc.l	$F8F9F108
	dc.l	$27F3B2E1
	dc.l	$9090EFD
	dc.l	$E51F2BE4
	dc.l	$CCE6E7F1
	dc.l	$D79ABB05
	dc.l	$FBE5F5FE
	dc.l	$293FFCDA
	dc.l	$F271D22
	dc.l	$13236A52
	dc.l	$1318261E
	dc.l	$1DE3BFFF
	dc.l	$22F5ECEC
	dc.l	$F71F06BB
	dc.l	$D000F9F1
	dc.l	$E8DF254C
	dc.l	$16FE1A1B
	dc.l	$1E02C9EF
	dc.l	$341F040C
UbL005E54	dc.l	$300
	dc.l	$1AD003F
	dc.l	UbL005854
UbL005E60	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL005E54
UbL005E6E	dc.l	UbL005E60
	dc.l	0
	dc.l	0
	dc.l	$2580A00
	dc.w	$FFFE

;	SECTION	UltSnd005E80,DATA,CHIP
UbW005E80	dc.w	$C133
	dc.b	$80
UbB005E83	dc.b	$C5
	dc.l	$7F7F297F
	dc.l	$C52C7F2E
	dc.l	$7FC5327F
	dc.l	$357FC538
	dc.l	$7F3A7FC5
	dc.l	$3E7F417F
	dc.l	$C5447F46
	dc.l	$7FC54A7F
	dc.l	$4D7FC550
	dc.l	$7F527FC5
	dc.l	$567F547F
	dc.l	$C5597F54
	dc.l	$7FC5597F
	dc.l	$547FC559
	dc.l	$7F547FC5
	dc.l	$597F547F
	dc.l	$C5597F54
	dc.l	$7FC5597F
	dc.w	$547F
	dc.b	$80
UbB005ECF	dc.b	$C1
	dc.w	$3180
UbL005ED2	dc.l	$C57F2429
	dc.l	$2EC52C29
	dc.l	$2E33C532
	dc.l	$30353AC5
	dc.l	$38353A3F
	dc.l	$C53E3C41
	dc.l	$46C54441
	dc.l	$464BC54A
	dc.l	$484D52C5
	dc.l	$504D5257
	dc.l	$C5565254
	dc.w	$5780
UbW005F00	dc.w	$C17F
	dc.b	$80
UbB005F03	dc.b	$C1
	dc.w	$2F80
UbL005F06	dc.l	$C17F7FC3
	dc.w	$7F80
UbL005F0C	dc.l	$C12D8000
UbW005F10	dc.w	1
	dc.l	UbW005F00
	dc.w	10
	dc.l	UbB005E83
	dc.l	0
	dc.w	0
UbW005F22	dc.w	1
	dc.l	UbW005F00
	dc.w	10
	dc.l	UbL005ED2
	dc.l	0
	dc.w	0
UbW005F34	dc.w	1
	dc.l	UbB005F03
	dc.w	10
	dc.l	UbL005F06
	dc.l	0
	dc.w	0
UbW005F46	dc.w	1
	dc.l	UbL005F0C
	dc.w	10
	dc.l	UbL005F06
	dc.l	0
	dc.w	0
UbW005F58	dc.w	1
	dc.l	UbW005E80
	dc.w	10
	dc.l	UbL005F06
	dc.l	0
	dc.w	0
UbW005F6A	dc.w	1
	dc.l	UbB005ECF
	dc.w	10
	dc.l	UbL005F06
	dc.l	0
	dc.w	0
UbL005F7C	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFC000
	dc.l	0
	dc.l	$666
	dc.l	$10000
	dc.l	0
UbL005F98	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.l	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL005F7C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL005F7C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL005F7C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL005F7C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
	dc.l	UbL000248
	dc.l	UbL000260
	dc.l	UbL00045C
	dc.w	0
	dc.w	0
	dc.w	$2F00
UbL006028	dc.l	UbW005F10
	dc.l	$FFFF0000
	dc.l	UbW005F58
	dc.l	$FFFE0000
	dc.l	UbW005F34
	dc.l	$FFFF0000
	dc.l	UbW005F46
	dc.l	$FFFE0000
	dc.l	UbW005F22
	dc.l	$FFFF0000
	dc.l	UbW005F6A
	dc.l	$FFFE0000
	dc.l	0
	dc.l	0
	dc.l	0
	dc.l	0
UbL006068	dc.l	UbL006028
	dc.l	UbL005F98
	dc.l	$B40000

;	SECTION	UltSnd006074,DATA,CHIP
UbL006074	dc.l	$10000
	dc.l	$10000
	dc.l	0
UbL006080	dc.l	$1AC0A
	dc.l	$1AC0A
	dc.l	$FFFFFFFF
	dc.l	$1AC00
	dc.l	$FFFFD10A
	dc.l	$17D0A
	dc.l	$FFFFFFFF
	dc.l	$17D00
	dc.l	$FFFFD60A
	dc.l	$1530A
	dc.l	$FFFFFFFF
	dc.l	$15300
	dc.l	$FFFFDB0A
	dc.l	$12E0A
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$FFFFDF0A
	dc.l	$10D0A
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$FFFFE30A
	dc.l	$F00A
	dc.l	$FFFFFFFF
	dc.l	$F000
	dc.l	$FFFFE60A
	dc.l	$D60A
	dc.l	$FFFFFFFF
	dc.l	$D600
	dc.l	$FFFFE80A
	dc.l	$BE0A
	dc.l	$FFFFFFFF
	dc.l	$BE00
	dc.l	$700A
	dc.l	$12E0A
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$FFFFDF0A
	dc.l	$10D0A
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$FFFFE30A
	dc.l	$F00A
	dc.l	$FFFFFFFF
	dc.l	$F000
	dc.l	$FFFFE60A
	dc.l	$D60A
	dc.l	$FFFFFFFF
	dc.l	$D600
	dc.l	$FFFFE80A
	dc.l	$BE0A
	dc.l	$FFFFFFFF
	dc.l	$BE00
	dc.l	$FFFFEC0A
	dc.l	$AA0A
	dc.l	$FFFFFFFF
	dc.l	$AA00
	dc.l	$FFFFED0A
	dc.l	$970A
	dc.l	$FFFFFFFF
	dc.l	$9700
	dc.l	$FFFFF00A
	dc.l	$870A
	dc.l	$FFFFFFFF
	dc.l	$8700
	dc.l	0
UbL006184	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL006190	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL006184
UbL00619E	dc.l	UbL006190
	dc.l	UbL006074
	dc.l	UbL006080
	dc.l	$500A00
	dc.w	$FFFE

;	SECTION	UltSnd0061B0,DATA,CHIP
UbL0061B0	dc.l	$10000
	dc.l	$10000
	dc.l	$FFFFFBBC
	dc.l	$4000
	dc.l	$FFFFFFFF
	dc.l	$3FF1
	dc.l	$C00F
	dc.l	$10000
	dc.l	$FFFFE000
	dc.l	$4000
	dc.l	$FFFFFFFF
	dc.l	$3FFE
	dc.l	$C002
	dc.l	$10000
	dc.l	$FFFFE000
	dc.l	$4000
	dc.l	$FFFFFFFF
	dc.l	$3FFE
	dc.l	$C002
	dc.l	$10000
	dc.l	$FFFFE000
	dc.l	$4000
	dc.l	$FFFFFFFF
	dc.l	$3FFE
	dc.l	$C002
	dc.l	$10000
	dc.l	$FFFFFDDE
	dc.l	0
	dc.l	0
UbL006224	dc.l	$1943C
	dc.l	$1943C
	dc.l	$FFFFFFFF
	dc.l	$19400
	dc.l	$FFFF9A08
	dc.l	$12E08
	dc.l	$FFFFFFFF
	dc.l	$12E00
	dc.l	$FFFFDF08
	dc.l	$10D08
	dc.l	$FFFFFFFF
	dc.l	$10D00
	dc.l	$FFFFE308
	dc.l	$F008
	dc.l	$FFFFFFFF
	dc.l	$F000
	dc.l	$FFFFDA78
	dc.l	$CA78
	dc.l	$FFFFFFFF
	dc.l	$CA00
	dc.l	0
UbL006278	dc.l	8
	dc.l	$100003F
	dc.l	UbL00026C
UbL006284	dc.l	0
	dc.l	$FFFE0000
	dc.w	0
	dc.l	UbL006278
UbL006292	dc.l	UbL006284
	dc.l	UbL0061B0
	dc.l	UbL006224
	dc.l	$D20A00
	dc.w	$FFFE
MM_End
