...
static struct cfg_lines opttable[] =
{
    ...
    {"sound_bits", "" },
    {"sound_channels", "" },
//    {"sound_min_buff", "" },	EMSCRIPTEN cleanup
//    {"sound_max_buff", "" },
    {"parallel_on_demand", "" },
    {"serial_on_demand", "" },
    {"joyport0", "" },
...
//    fprintf (f, "sound_min_buff=%d\n", p->sound_minbsiz); EMSCRIPTEN cleanup
...
//	|| cfgfile_intval (option, value, "sound_min_buff", &p->sound_minbsiz, 1) EMSCRIPTEN cleanup
...
int cfgfile_load_file (struct uae_prefs *p, FILE *fh)
{
    ...
    if (! fh) {
      fprintf(stderr, "cfgfile: no FILE*\n");
	return 0;
	}
    while (1) {
