...
static int decrunch_sqsh (uint8_t *src, size_t s, uint8_t **unpacked, size_t *unpackedlen)
{
  ...
  if ((dl = unpack(src + 16, dst, dstlen)) != dstlen) {
	fprintf(stderr,"fail XPK SQSH %d %d\n", dl, dstlen);	// XXX
...
//    goto error;
	...
	*unpackedlen = dstlen;
	...
	fprintf(stderr,"unpacked XPK SQSH file\n");
