...
void Support_Types_FileDefault ( void )
{
  ...
  strcpy ( Extensions[168], "DM2" );
  strcpy ( Extensions[169], "HEADPACK" );
  strcpy ( Extensions[170], "LOBDataCruncher" );
  strcpy ( Extensions[171], "DevilsReplay" );
  strcpy ( Extensions[172], "PerfSong2" );
  strcpy ( Extensions[173], "PRT" );
  strcpy ( Extensions[174], "SFX2" );
  strcpy ( Extensions[175], "Promizer10b" );
  strcpy ( Extensions[176], "Crunch13data" );
  strcpy ( Extensions[177], "SVDMF" );
  strcpy ( Extensions[178], "---" );
}

extern int DetectedFormatId;

int Get_Detected_Format_Id() {
	// when converted version is used then the original type is irrelevant
	return UseConvertedResult ? -1 : DetectedFormatId;
}
...
{
  LastFormatName= format_to_save;	// UADE
  Save_Status = BAD;
  printf ( "%s found at %d !. its size is : %u\n", format_to_save , PW_Start_Address , OutputSize );
  if ( (PW_Start_Address + (int32_t)OutputSize) > PW_in_size+3 )	// HACK: Voxpop - Twn And Xeno.gnoi (corrupt rip still plays)
...
/* writing craps in converted MODs */
void Crap15 ( char *Format , FILE *out )
{
 ...
 * only one func and not hundreds ...
 * TODO : handle case where sizes are on 3 bytes only
*/
int16_t testSpecialCruncherData ( int32_t Pack_addy , int32_t Unpack_addy )
{
  ...
  if ( ( (uint32_t)PW_i + Pack_addy ) > PW_in_size )
  {
/*printf ( "#0\n" );*/
    return BAD;
  }
...
/*  PW_l = ( (in_data[PW_Start_Address+Pack_addy+1]*256*256) +*/
  PW_l = ( (in_data[PW_Start_Address+Pack_addy+0]*256*256*256) +
           (in_data[PW_Start_Address+Pack_addy+1]*256*256) +
           (in_data[PW_Start_Address+Pack_addy+2]*256) +
            in_data[PW_Start_Address+Pack_addy+3] );
  /* unpacked size */
  /* first byte is sometime used ... "LOB is an ex" */
/*  PW_k = ( (in_data[PW_Start_Address+Unpack_addy+1]*256*256) +*/
  PW_k = ( (in_data[PW_Start_Address+Unpack_addy+0]*256*256*256) +
