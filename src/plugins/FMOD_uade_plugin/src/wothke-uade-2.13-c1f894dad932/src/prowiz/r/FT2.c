...
int16_t	 testXM ( void )
{
  ...
  if ( in_data[PW_Start_Address + 37] != 0x1A )
  {
    /*printf ( "#2 Start:%ld\n" , PW_Start_Address );*/
    return BAD;
  }
  if ( in_data[PW_Start_Address+58] != 0x04 )
  {
    /*printf ( "#2 Start:%ld\n" , PW_Start_Address );*/
    return BAD;
  }
...
void Rip_XM ( void )
{
  ...
  for ( PW_o=0 ; PW_o<PW_j ; PW_o++ )
  {
	...
	printf ("[XMrip] @instruments start : %d\n",PW_l);
  ...
  for ( PW_o=0 ; PW_o<PW_k ; PW_o++ )
  {
    ...
    printf ("[XMrip][inst %03d] - @ : %d - header size : %d",PW_o, PW_l, PW_m);
    ...
    if (PW_j>64)break; /* should be 16 - technical limit of FT2, but some later tracker broke this limit */
