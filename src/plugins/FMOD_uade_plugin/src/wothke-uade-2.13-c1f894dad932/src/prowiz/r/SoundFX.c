...
/* first line on 2022-06-067*/
int16_t	 testSoundFX2 ( void )
{
  /* test 1 - header size */
  if ( PW_i < 0x3C )
  {
/*printf ( "#1 (PW_i:%ld)\n" , PW_i );*/
  ...
  PW_Start_Address = PW_i-124;

  /* test 2 - samples tests */
  PW_WholeSampleSize = 0;
  for ( PW_k=0 ; PW_k<31 ; PW_k++ )
  {
    /* size */
    PW_j = ((in_data[PW_Start_Address+PW_k*4+1]*256*256)+(in_data[PW_Start_Address+PW_k*4+2]*256)+in_data[PW_Start_Address+PW_k*4+3]);
    /* loop start */
    PW_m = ((in_data[PW_Start_Address+170+PW_k*30]*256)+in_data[PW_Start_Address+171+PW_k*30]);
    /* loop size */
    PW_n = ((in_data[PW_Start_Address+172+PW_k*30]*256)+in_data[PW_Start_Address+173+PW_k*30]);
	/* sum of sample sizes */
    PW_WholeSampleSize += PW_j;

    /* loopstart,replen > 64k ? */
    if ( (PW_m > 0xFFFF) || (PW_n > 0xFFFF) )
    {
/* printf ( "#2,0 (Start:%d) (PW_m:%d) (PW_n:%d) (PW_k:%d)\n" , PW_Start_Address, PW_m, PW_n, PW_k );*/
      return BAD;
    }
    /* loop size =0 & loop start != 0 ?*/
    if ( (PW_m != 0) && (PW_n==0) )
...
/*printf ( "#2,2 (start:%d) (size:%d)\n" , PW_Start_Address,PW_j );*/
  ...
  for ( PW_k=0 ; PW_k<31 ; PW_k++ )
  {
    if ( in_data[PW_Start_Address+169+PW_k*30]>0x40 )
    {
/*printf ( "#3 (Start:%d)\n" , PW_Start_Address );*/
      return BAD;
    }
  ...
  /* test #4  pattern list size */
  PW_l = in_data[PW_Start_Address+1074];
  if ( (PW_l>127) || (PW_l==0) )
  {
/*printf ( "#4,0 (Start:%d) (patternlist size:%d)\n" , PW_Start_Address, PW_l );*/
    return BAD;
  }
  /* PW_l holds the size of the pattern list */
  PW_k=0;
  for ( PW_j=0 ; PW_j<PW_l ; PW_j++ )
  {
    if ( in_data[PW_Start_Address+1076+PW_j] > PW_k )
      PW_k = in_data[PW_Start_Address+1076+PW_j];
    if ( in_data[PW_Start_Address+1076+PW_j] > 127 )
    {
/*printf ( "#4,1 (Start:%d)\n" , PW_Start_Address );*/
      return BAD;
    }
  }
  /* PW_k is the number of pattern -1 in the file */
  PW_k += 1;
...
void Rip_SoundFX2 ( void )
{
  /* PW_k is still the nbr of pattern */
  /* PW_WholeSampleSize is the WholeSampleSize :) */

  OutputSize = PW_WholeSampleSize + (PW_k*1024) + 1204 +4; /*+4 as in 4 empty bytes before sample data*/
  printf ("SFX2: PW_WholeSampleSize : %d\n"
  "SFX2: nbr pats (PW_k): %d\n"
  "SFX2: pos of 1st sample : %d\n"
  ,PW_WholeSampleSize, PW_k, 1208+(PW_k*1024));

  CONVERT = BAD;

  Save_Rip ( "SoundFX 2 module", SoundFX2 );
  
  if ( Save_Status == GOOD )
    PW_i += 1;  /* overall point in input file */
}
