...
int16_t	 testPMZ ( void )
{
  ...
  /* test 1 */
  if ( (PW_Start_Address + 4713) > PW_in_size )
  {
    return BAD;
  }

/*20210727 - test added after XMP guys created crazy examples */
  /* test 2 - test Whole Pattern Size */
  PW_j = (in_data[PW_Start_Address+4460]*256*256*256)+(in_data[PW_Start_Address+4461]*256*256)+(in_data[PW_Start_Address+4462]*256)+in_data[PW_Start_Address+4463];
  if ( PW_j > 0x10000 )
  {
    return BAD;
  }
