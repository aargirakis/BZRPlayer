...
extern int16_t testTetrapack22pro ( void );
...
extern int16_t testSoundFX2 ( void );
...
extern void Depack_Devils ( void );
...
extern void Depack_PERFSNG2 ( void );
extern void Depack_PHA ( void );
extern void Depack_PM01 ( void );
extern void Depack_PM10b ( void );
...
extern uint32_t PW_Start_Address;		/* xigh: Should be 64 bits */
extern uint32_t OutputSize;
extern uint32_t PW_in_size;
...
extern const char *LastFormatName;

// disable all printf output
#include <stdarg.h>
extern void disabledprint(const char*f, ...);
#define printf disabledprint

extern unsigned int xcrc32 (const unsigned char *buf, int len);
extern int checkcrc(unsigned int *refs);
extern void calccrc(const unsigned char *buf, int len);

extern int UseConvertedResult;