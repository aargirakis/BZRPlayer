...
#ifdef EMSCRIPTEN
#include "../../prowiz/include/globals.h"	// Prowiz
#endif
...
{
#ifdef EMSCRIPTEN
    // use existing Prowiz file content analysis result (todo? might similarly
	// replace some of the below..)
	switch (Get_Detected_Format_Id()) {
		case Star_pack:
			return	MOD_STAR_PACK;
  ...
  // EMSCRIPTEN FIXME: numbering changed with added entries
  ...
  // EMSCRIPTEN: why is there no "!PM!" (Power Music) detection here?
  // should be the same simple file magic based impl!
		    ...
		    size_t realfilesize, const char *path, int verbose)
{
  ...
  struct modtype mod32types[] = {
    ...
    {.e = MOD_STAR_PACK, .str = "MOD_STPK"},			// EMSCRIPTEN
    {.e = MOD_UST, .str = "MOD15"},						// EMSCRIPTEN hack: none of the available players handles this 100% correctly
    {.str = NULL}
  };
