...
{
//fprintf(stdout, "process_module [%s][%d][%s]\n", credits, credits_len, filename);
	FILE *modfile;
	struct stat st;
	size_t modfilelen;
	unsigned char *buf;
...
#else
	struct AFILE *uo= uade_fopen(filename, "rb");

	if ((modfile = uo->file) == NULL)	{
		if (uo->async_status<0)
			return -1;
		return 0;
	}
	modfilelen= uade_get_file_size(filename);
#endif
	if ((buf = malloc(modfilelen)) == NULL) {
		fprintf(stdout, "uade: can't allocate mem in process_module()");
		fclose(modfile);
		return 0;
	}
	...
	if (rb < modfilelen) {
		fprintf(stdout, "uade: song info could not read %s fully\n",
			filename);
		free(buf);
		return 0;
	}
			     ...
			     struct uade_state *state)
{
	...
	if ((srclen = strlen(format)) == 0) {
		fprintf(stdout, "Warning: empty song_title format string.\n");
		return 1;
	}
	...
	while (dstoffs < dstlen) {
		...
		if (c != '%') {
			...
			if ((srcoffs + 1) >= srclen) {
				fprintf(stdout, "Error: no identifier given in song title format: %s\n", format);
				title[dstoffs] = 0;
				return 1;
			}
...
#endif
	default:
		fprintf(stdout, "Illegal info requested.\n");
		exit(-1);
	}
	return 0;
}
