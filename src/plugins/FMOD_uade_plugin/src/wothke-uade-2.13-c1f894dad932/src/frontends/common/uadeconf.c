			      ...
			      double high, const char *type)
{
	...
	if (*endptr != 0 || v < low || v > high) {
		fprintf(stdout, "Invalid %s value: %s\n", type, value);
		v = def;
	}
			     ...
			     int flags, struct uade_attribute *attributelist)
{
	...
	if (flags & ES_NEVER_ENDS)
		fprintf(stdout, "uade: ES_NEVER_ENDS is not implemented. What should it do?\n");
	...
	while (a != NULL) {
		...
		case ES_EP_OPTION:
			if (uc->verbose)
				fprintf(stdout, "Using eagleplayer option %s\n", a->s);
		...
		case ES_PLAYER:
			if (playername) {
				...
				fprintf(stdout, "Error: attribute handling was given playername == NULL.\n");
			}
			break;
		...
		case ES_SUBSONGS:
			fprintf(stdout, "Subsongs not implemented.\n");
			break;
		...
		default:
			fprintf(stdout,	"Unknown song attribute integer: 0x%x\n", a->type);
			break;
		}
...
#ifdef EMSCRIPTEN
extern int ems_request_file(const char *filename); // must be implemented on JavaScript side (also see mycallback.js)
#endif
...
//			fprintf(stdout, "some file is not ready yet: %s\n", modulename);
	...
	while (xfgets(line, sizeof(line), f) != NULL) {
		...
		if (opt) {
			...
			fprintf(stdout,	"Unknown config key in %s on line %d: %s\n", filename, linenumber, key);
		}
	}
...
int uade_parse_subsongs(int **subsongs, char *option)
{
	...
	*subsongs = NULL;
	...
	if (strlcpy(substr, option, sizeof subsongs) >= sizeof subsongs) {
		fprintf(stdout, "Too long a subsong option: %s\n", option);
		return -1;
	}
	...
	*subsongs = malloc((nsubsongs + 1) * sizeof((*subsongs)[0]));
	if (*subsongs == NULL) {
		fprintf(stdout, "No memory for subsongs.\n");
		return -1;
	}
...
#define SET_OPTION(opt, value) do { uc->opt = (value); uc->opt##_set = 1; } while (0)
	...
	case UC_BASE_DIR:
		if (value != NULL) {
			...
			fprintf(stdout, "uade: Passed NULL to UC_BASE_DIR.\n");
		}
		break;
	...
	case UC_BUFFER_TIME:
		if (value != NULL) {
			...
			if (uc->buffer_time <= 0 || *endptr != 0) {
				fprintf(stdout, "Invalid buffer_time: %s\n",
					value);
				uc->buffer_time = 0;
			}
		} else {
	...
	case UC_FORCE_LED:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_FORCE_LED value is NULL\n");
			break;
		}
		if (strcasecmp(value, "off") == 0 || strcmp(value, "0") == 0) {
			...
			fprintf(stdout, "Unknown force led argument: %s\n",
				value);
			break;
		}
		uc->led_state_set = 1;
	...
	case UC_FREQUENCY:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_FREQUENCY value is NULL\n");
			break;
		}
		x = strtol(value, &endptr, 10);
		if (*endptr != 0) {
			fprintf(stdout, "Invalid frequency number: %s\n",
				value);
			break;
		}
		/* The upper bound is NTSC Amigas bus freq */
		if (x < 1 || x > 3579545) {
			fprintf(stdout, "Frequency out of bounds: %ld\n", x);
	...
	case UC_GAIN:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_GAIN value is NULL\n");
			break;
		}
	...
	case UC_RESAMPLER:
		if (value == NULL) {
			fprintf(stdout, "uade.conf: No resampler given.\n");
			break;
		}
		...
		if (uc->resampler != NULL) {
			...
			fprintf(stdout,	"uade.conf: no memory for resampler.\n");
		}
		break;
	...
	case UC_NORMALISE:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_NORMALISE is NULL\n");
			break;
		}
	...
	case UC_PANNING_VALUE:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_PANNING_VALUE is NULL\n");
			break;
		}
	...
	case UC_SONG_TITLE:
		if (value == NULL) {
			fprintf(stdout, "uade: No song_title format given.\n");
			break;
		}
		if ((uc->song_title = strdup(value)) == NULL) {
			fprintf(stdout, "No memory for song title format\n");
		} else {
			uc->song_title_set = 1;
		}
		break;
	...
	case UC_TIMEOUT_VALUE:
		if (value == NULL) {
			fprintf(stdout, "uade: UC_TIMEOUT_VALUE is NULL\n");
			break;
		}
	...
	default:
		fprintf(stdout, "uade_set_config_option(): unknown enum: %d\n",
			opt);
		exit(1);
	}
}
...
void uade_set_filter_type(struct uade_config *uc, const char *model)
{
	...
	if (strncasecmp(model, "a500", 4) == 0) {
		...
		fprintf(stdout, "Unknown filter model: %s\n", model);
	}
}
...
int uade_set_silence_timeout(struct uade_config *uc, const char *value)
{
	...
	if (*endptr != 0 || t < -1) {
		fprintf(stdout, "Invalid silence timeout value: %s\n", value);
		return -1;
	}
...
static int uade_set_subsong_timeout(struct uade_config *uc, const char *value)
{
	...
	t = strtol(value, &endptr, 10);
	if (*endptr != 0 || t < -1) {
		fprintf(stdout, "Invalid subsong timeout value: %s\n", value);
		return -1;
	}
...
static int uade_set_timeout(struct uade_config *uc, const char *value)
{
	...
	t = strtol(value, &endptr, 10);
	if (*endptr != 0 || t < -1) {
		fprintf(stdout, "Invalid timeout value: %s\n", value);
		return -1;
	}
	uc->timeout = t;
	uc->timeout_set = 1;
