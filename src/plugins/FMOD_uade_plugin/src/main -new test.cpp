...
FMOD_RESULT F_CALLBACK open(FMOD_CODEC_STATE *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo)
{
    ...
    {
        char* testBuffer;
        smallBuffer = new char[62];
        result = FMOD_CODEC_FILE_SEEK(codec,0,0);
        result = FMOD_CODEC_FILE_READ(codec,testBuffer,62,&bytesread);
        char majorVersion = testBuffer[0];
        char minorVersion = testBuffer[1];
        int tuneId = testBuffer[2]+testBuffer[3]+testBuffer[4]+testBuffer[5];
        char tickBeat = testBuffer[36];
        char beatMeasure = testBuffer[37];
        int totalTick = testBuffer[38]+testBuffer[39]+testBuffer[40]+testBuffer[41];
        int dataSize = testBuffer[42]+testBuffer[43]+testBuffer[44]+testBuffer[45];
        int nrCommand = testBuffer[46]+testBuffer[47]+testBuffer[48]+testBuffer[49];
        char soundMode = testBuffer[58];
        char pitchBRange = testBuffer[59];
        int basicTempo = testBuffer[60] + testBuffer[61];
...
//		fp.filesize(f) < (unsigned)(HEADER_LEN + dataSize))
    ...
    snprintf(uade_basedir, 1024, "%s/%s", gp->info->applicationPath.c_str(),"plugin/uade");
    #ifdef UNICODEHACK
    ...
    string filename = gp->info->applicationPath + "/plugin/config/uade.cfg";
    ...
    {
        gp->songlengthpath=gp->info->applicationPath + "/plugin/uade/uade.md5";
    }
