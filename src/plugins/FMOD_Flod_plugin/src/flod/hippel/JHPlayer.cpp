...
void JHPlayer::process()
{
        ...
        do {
            ...
            if (coso) {
                    ...
                    do {
                        ...
                        do {
                            ...
                            if (value == -1) {
                                ...
                                voice->cosoCtr = voice->cosoSpeed = stream[position];position++;
                                loop = 3;
                            } else if (value == -3) {
                                voice->cosoCtr = voice->cosoSpeed = stream[position];position++;
                                ...
                                voice->note = value;
                                voice->info = (signed char)stream[position];position++;
                                ...
                                if (value >= 0) {
                                    ...
                                    voice->volCtr = voice->volSpeed = stream[position];position++;
                                    voice->volSustain = 0;
                                    value = (signed char)stream[position];position++;

                                    voice->vibSpeed = (signed char)stream[position];position++;
                                    voice->vibrato  = 64;
                                    voice->vibDepth = voice->vibDelta = (signed char)stream[position];position++;
                                    voice->vibDelay = stream[position];position++;
                                    ...
                                    if (value != -128) {
                                        ...
                                        voice->freqsPtr = readEndian(stream[position],stream[position+1]);position+=2;
                                        voice->freqsPos = 0;
                ...
                if ((voice->patternPos == patternLen) || ((value & 127) == 1)) {
                    ...
                    voice->trackTrans = (signed char)stream[position];position++;
                    voice->volTrans = (signed char)stream[position];position++;
                ...
                if (value & 127) {
                    ...
                    voice->infoPrev = (signed char)stream[position];position++;
                    position = pos1;
                    voice->info = (signed char)stream[position];position++;
    ...
    do {
        ...
        do {
            ...
            if (voice->tick) {
                ...
                do {
                    ...
                    case -29:
                        voice->vibSpeed = (signed char)stream[position];position++;
                        voice->vibDepth = (signed char)stream[position];position++;
                        voice->freqsPos += 3;
                        break;
                    case -28:
                        sample = samples[stream[position]];
                    ...
                    case -27:
                        ...
                        if (m_variant == 2) {
                            pos1 = stream[position] * sample->length;position++;
                            ...
                            voice->sldLen     = readEndian(stream[position],stream[position+1]) << 1;position+=2;
                            voice->sldDelta   = (signed short)readEndian(stream[position],stream[position+1]) << 1;position+=2;
                            voice->sldActive  = 0;
                            voice->sldCtr = 0;
                            voice->sldSpeed   = stream[position];position++;
                            voice->slide      = 1;
                            voice->sldDone    = 0;
                    ...
                    case -25:
                        ...
                        break;
                    case -24:
                        voice->tick = stream[position];position++;
                        voice->freqsPos += 2;
                        loop = 1;
                        break;
                    case -23:
                        if (m_variant < 2) break;
                        sample = samples[stream[position]];position++;
                        voice->sample = -1;
                        voice->enabled = 1;

                        pos2 = stream[position];position++;
                        pos1 = position;
                        chan->setEnabled(0);

                        position = sampleData + (sample->pointer + 4);
                        c1 = stream[position];position++;
                        c2 = stream[position];position++;
                        value = (c1 * 24) + (c2 << 2);
                        position += (pos2 * 24);

                        voice->loopPtr = readEndian(stream[position],stream[position+1],stream[position+2],stream[position+3]) & 0xfffffffe;position+=4;
                        chan->length   = (readEndian(stream[position],stream[position+1],stream[position+2],stream[position+3]) & 0xfffffffe) - voice->loopPtr;position+=4;
        ...
        do {
            ...
            if (voice->volSustain) {
                ...
                do {
                    ...
                    switch (value) {
                    case -24:
                        voice->volSustain = stream[position];position++;
                        voice->volsPos += 2;
                        loop = 1;

                        break;
                    case -32:
                        voice->volsPos = (stream[position] & 63) - 5;position++;
                        loop = 3;
                        break;
                    default:
                        voice->volume = value;
        ...
        if (coso) {
            ...
            period = readEndian(stream[position],stream[position+1]);position+=2;
        }
