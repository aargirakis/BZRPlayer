...
void CSurroundopl::write(int reg, int val)
{
	...
	if ((iChannel >= 0)) {// && (i == 1)) {
		...
		double dbOriginalFreq = 49716.0 * (double)iFNum * pow((double)2, iBlock - 20); //modified by blazer
		...
		#define calcFNum() ((dbOriginalFreq+(dbOriginalFreq/FREQ_OFFSET)) / (49716.0 * pow((double)2, iNewBlock - 20)))//modified by blazer
