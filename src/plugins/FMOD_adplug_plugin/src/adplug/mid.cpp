...
bool CmidPlayer::load(const std::string &filename, const CFileProvider &fp)
{
    ...
    switch(s[0])
        ...
        case 'M':
            ...
            //added by blazer to not play midi files
            fp.close(f);
            return false;
            break;
        case 'C':
	  ...
	  break;
        default:
            ...
            //added by blazer to have less false hits, not sure if it's correct though
            if (s[2]==0 && s[3]==0 && s[4]=='A' && s[5]=='D') good=FILE_OLDLUCAS;
            break;
        }
