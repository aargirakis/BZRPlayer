...
//        if( abs( j ) > vu[i] ) vu[i] = abs( j );
      ...
      /* If bufmod is 0, assume we don't want to write any data out (for seeking) */
      if (bufmod)
      {
        *(int16 *)buf1 = a;
        *(int16 *)buf2 = b;
...
/* Don't call during playback ! */
uint32 hvl_GetLen(struct hvl_tune *ht, uint32 nr)
{
    uint32 multiplier, maxtime;

    multiplier = ht->ht_SpeedMultiplier * 50;	/* 50 interrupts per second */
    maxtime    = 720 * multiplier;
    //ht->ht_LoopDetector = 0;

    hvl_InitSubsong(ht, nr);

    do
    {
        /* This is a copy of hvl_play_irq() slightly modified */
        uint32 i;
    ...
    while (maxtime && !ht->ht_SongEndReached);

    return (uint32) (((float) ht->ht_PlayingTime / multiplier) * 1000.0f);
}

/* Very crappy seek - not accurate! */
void hvl_Seek(struct hvl_tune *ht, uint32 time)
{
    uint32 multiplier = ht->ht_SpeedMultiplier * 50;	/* 50 interrupts per second */

    hvl_InitSubsong(ht, ht->ht_SongNum);

    while (((uint32)(((float)ht->ht_PlayingTime/multiplier) * 1000.0f) < time) || ht->ht_SongEndReached)
    {
        hvl_DecodeFrame(ht, NULL, NULL, 0);
    }
}
