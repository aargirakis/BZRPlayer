...
FMOD_CODEC_DESCRIPTION tfmxcodec =
{
    ...
    FMOD_TIMEUNIT_MS|FMOD_TIMEUNIT_SUBSONG|FMOD_TIMEUNIT_MUTE_VOICE|FMOD_TIMEUNIT_MODROW|FMOD_TIMEUNIT_MODPATTERN|FMOD_TIMEUNIT_MODORDER|FMOD_TIMEUNIT_MODPATTERN_INFO|FMOD_TIMEUNIT_MODVUMETER,	              // The time format we would like to accept into setposition/getposition.
    &open,                             // Open callback.
...
FMOD_RESULT F_CALLBACK getposition(FMOD_CODEC_STATE *  codec, unsigned int *position, FMOD_TIMEUNIT postype)
{
    ...
    else if(postype==FMOD_TIMEUNIT_MODPATTERN)
    {
        ...
        {
            ahx->info->modTrackPositions.push_back(ahx->m_tune->ht_Positions[Front(ahx->m_tune->patternPosBuffer)].pos_Track[i]);
        }
        *position=0;
        return FMOD_OK;
    }
