...
public:
    ...
    const struct uade_song_info *uadeInfo;
};
...
FMOD_RESULT F_CALLBACK open(FMOD_CODEC_STATE* codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO* userexinfo)
{
    ...
    uade_config_set_option(uadeConfig, UC_FILTER_TYPE, "none");
    uade_config_set_option(uadeConfig, UC_RESAMPLER, "none");
    uade_config_set_option(uadeConfig, UC_PANNING_VALUE, "1");
    uade_config_set_option(uadeConfig, UC_NO_FILTER, nullptr);
    ...
    plugin->uadeInfo = uade_get_song_info(plugin->uadeState);

    //TODO check these:
    plugin->uadeInfo->modulemd5;
    plugin->uadeInfo->duration;

    if (plugin->uadeInfo->formatname[0]) {
        string str(plugin->uadeInfo->formatname);
        plugin->info->fileformat = str.starts_with(TYPE_PREFIX)?
                                       str.substr(TYPE_PREFIX.length()) : plugin->uadeInfo->formatname;
    } else {
        plugin->info->fileformat = plugin->uadeInfo->detectioninfo.ep->playername;
    }

    if (plugin->uadeInfo->modulename[0]) {
        plugin->info->title = plugin->uadeInfo->modulename;
    }
    ...
    plugin->info->md5New = plugin->uadeInfo->modulemd5;
...
FMOD_RESULT F_CALLBACK read(FMOD_CODEC_STATE* codec, void* buffer, unsigned int size, unsigned int* read)
{
    ...
    ssize_t audioData = uade_read(buffer, plugin->waveformat.pcmblocksize, plugin->uadeState);

    //TODO they are not triggered when track end due to uade.md5
    if (audioData < 0) {
    ...
    //TODO (fix I know.ahx & anarchy_in_the_kitchen.ac1d playback start)
    *read = static_cast<unsigned int>(audioData / plugin->waveformat.channels / sizeof(int16_t));
...
FMOD_RESULT F_CALLBACK getlength(FMOD_CODEC_STATE* codec, unsigned int* length, FMOD_TIMEUNIT lengthtype)
{
    ...
    {
        plugin->currentSubsong = plugin->uadeInfo->subsongs.min;
    }
    if (lengthtype == FMOD_TIMEUNIT_SUBSONG)
    {
        //TODO use plugin->uinfo->subsongs.cur?
        *length = 1 + (plugin->uadeInfo->subsongs.max - plugin->uadeInfo->subsongs.min);
        return FMOD_OK;
    }

    int sub = plugin->currentSubsong;
    if (plugin->uadeInfo->subsongs.min == 1)
    {
    ...
    {
        songLength = getLengthFromDatabase(plugin->info->filename.c_str(),  sub, plugin->uadeInfo->modulemd5, plugin->uade_songlengthspath.c_str());
    }
    else
    {
        songLength = 0;
...
FMOD_RESULT F_CALLBACK setposition(FMOD_CODEC_STATE* codec, int subsound, unsigned int position, FMOD_TIMEUNIT postype)
{
    ...
    {
        if (plugin->uadeInfo->subsongs.max - plugin->uadeInfo->subsongs.min == 0)
        {
            return FMOD_OK;
        }
        if (position > plugin->uadeInfo->subsongs.max - plugin->uadeInfo->subsongs.min)
        {
            position = plugin->uadeInfo->subsongs.min;
        }
        else
        {
            position = plugin->uadeInfo->subsongs.min + position;
        }
