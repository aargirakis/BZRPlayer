...
static FMOD_RESULT F_CALL open(FMOD_CODEC_STATE *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo) {
    logDebug("Try", PLUGIN_libopenmpt_NAME);
        ...
        for (int i = 0; i < plugin->mod->get_num_patterns(); i++) {
            ...
            for (int j = 0; j < numberRows; j++) {
                for (int k = 0; k < plugin->info->numChannels; k++) {
                    ...
                    row->note = plugin->mod->
                            get_pattern_row_channel_command(i, j, k, openmpt::module_ext::command_note);
                    ...
                    row->param = plugin->mod->get_pattern_row_channel_command(
                        i, j, k, openmpt::module_ext::command_parameter);
                    row->effect2 = plugin->mod->get_pattern_row_channel_command(
                        i, j, k, openmpt::module_ext::command_volumeffect);
                    row->param2 = 0;
                    row->vol = plugin->mod->get_pattern_row_channel_command(
                        i, j, k, openmpt::module_ext::command_volume);
