...
FMOD_RESULT F_CALLBACK libopenmptsetposition(FMOD_CODEC_STATE *codec, int subsound, unsigned int position, FMOD_TIMEUNIT postype);
...
FMOD_CODEC_DESCRIPTION libopenmptcodec =
{
    ...
    &libopenmptgetlength,                         // Getlength callback.  (If not specified FMOD return the length in FMOD_TIMEUNIT_PCM, FMOD_TIMEUNIT_MS or FMOD_TIMEUNIT_PCMBYTES units based on the lengthpcm member of the FMOD_CODEC structure).
    &libopenmptsetposition,                      // Setposition callback.
    &libopenmptgetposition,                                   // Getposition callback. (only used for timeunit types that are not FMOD_TIMEUNIT_PCM, FMOD_TIMEUNIT_MS and FMOD_TIMEUNIT_PCMBYTES).
...
FMOD_RESULT F_CALLBACK libopenmptopen(FMOD_CODEC_STATE *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo)
{
    ...
    try {
        ...
        libopenmpt->libopenmptwaveformat.lengthpcm    =  0xffffffff;//libopenmpt->mod->get_duration_seconds()*libopenmpt->libopenmptwaveformat.frequency;
...
FMOD_RESULT F_CALLBACK libopenmptgetlength(FMOD_CODEC_STATE *codec, unsigned int *length, FMOD_TIMEUNIT lengthtype)
{
    ...
    if(lengthtype==FMOD_TIMEUNIT_SUBSONG_MS || lengthtype==FMOD_TIMEUNIT_MS || lengthtype==FMOD_TIMEUNIT_MUTE_VOICE)
    {
...
FMOD_RESULT F_CALLBACK libopenmptgetposition(FMOD_CODEC_STATE *  codec, unsigned int *position, FMOD_TIMEUNIT postype)
{
    ...
    else if(postype==FMOD_TIMEUNIT_MODPATTERN_INFO)
    {
        ...
        for (vector< vector<BaseRow*> >::iterator itr = libopenmpt->info->patterns.begin(); itr != libopenmpt->info->patterns.end(); ++itr)
        {
            for (vector<BaseRow*>::iterator itr2 = (*itr).begin(); itr2 != (*itr).end(); ++itr2)
            {
                delete (*itr2);
        ...
        for( int i=0; i<libopenmpt->mod->get_num_patterns(); i++ )
        {
            ...
            for( int j=0; j<numberRows; j++ )
            {
                for( int k=0; k<libopenmpt->info->numChannels; k++ )
                {
                    ...
                    row->note = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_note);
                    row->sample = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_instrument);
                    int effect = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_effect);
                    if(effect>0)
                    {
                        effect--;
                    }
                    ...
                    row->effect = effect;
                    row->param = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_parameter);
                    row->effect2 = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_volumeffect);
                    row->param2 = 0;
                    row->vol = libopenmpt->mod->get_pattern_row_channel_command(i,j,k,libopenmpt->mod->command_volume);
