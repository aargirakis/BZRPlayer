...
//    }
    ...
    unsigned int bytesread;
    fc->myBuffer = new signed short[codec->filesize];

    result = codec->fileseek(codec->filehandle,0,(signed short*)fc->myBuffer);
    result = codec->fileread(codec->filehandle,(signed short*)fc->myBuffer,codec->filesize,&bytesread,0);

    fc->m_player = new kdmeng( 44100, 2, 2 );

    unsigned found = fc->info->filename.find_last_of("/\\");

    long length = fc->m_player->load((signed short*)fc->myBuffer,codec->filesize,fc->info->filename.substr(0,found+1).c_str());
...
FMOD_RESULT F_CALLBACK fcread(FMOD_CODEC_STATE *codec, void *buffer, unsigned int size, unsigned int *read)
{
    ...
    fc->m_player->rendersound(buffer,size);
    *read=size;
