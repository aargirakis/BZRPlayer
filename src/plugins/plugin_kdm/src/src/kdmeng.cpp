...
long kdmeng::loadwaves(const char *refdir) {
    ...
    if (file.is_open()) {
        ...
        dawaversionum = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
        p += 4;
        if (dawaversionum != 0) return 0;
        totsndbytes = 0;
        numwaves = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
        p += 4;
        ...
        for (i = 0; i < numwaves; i++) {
            ...
            wavleng[i] = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
            p += 4;
            repstart[i] = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
            p += 4;
            repleng[i] = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
            p += 4;
            finetune[i] = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
            p += 4;
...
kdmeng::kdmeng(unsigned samplerate, unsigned numspeakers, unsigned bytespersample)
    ...
    for (i = 0; i < 256; i++) {
        ...
        eff[4][i] = 65536 + j;
        switch ((i >> 1) % 3) {
            case 0: eff[5][i] = 65536;
                break;
            case 1: eff[5][i] = 65536 * 330 / 262;
                break;
            case 2: eff[5][i] = 65536 * 392 / 262;
                break;
        }
...
void kdmeng::startwave(long wavnum, long dafreq, long davolume1, long davolume2, long dafrqeff, long davoleff,
                       long dapaneff) {
...
long kdmeng::load(void *data, unsigned long int length, const char *filepath) {
    ...
    kdmversionum = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
    p += 4;
    if (kdmversionum != 0) return 0;
    numnotes = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
    p += 4;
    numtracks = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
    p += 4;
    ...
    for (int i = 0; i < numnotes; i++) {
        nttime[i] = readEndian(d[p + 3], d[p + 2], d[p + 1], d[p]);
        p += 4;
    }
    for (int i = 0; i < numnotes; i++) {
        nttrack[i] = d[p];
    ...
    for (long dacnt = 0; dacnt < numsamplestoprocess; dacnt += bytespertic) {
        ...
        for (i = NUMCHANNELS - 1; i >= 0; i--) {
			...
			*/
            ...
            } else { stereohicomb(voloffs3, kdmsamplerate >> 11, dasinc, splc[i], &stemp[bytespertic << 1]); }
        }
