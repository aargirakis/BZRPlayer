...
gint xs_get_hash(const gchar *filename, xs_md5hash_t hash)
{
    xs_file_t *inFile;
    ...
    /* Size of data buffer used for SID-tune MD5 hash calculation.
     * If this is too small, the computed hash will be incorrect.*/
    #define XS_BUF_SIZE        (1024*1024) //1MB is maybe enough for UADE songs?

    /* Allocate buffer */
    songData = (guint8 *) malloc(XS_BUF_SIZE * sizeof(guint8));
    if (!songData) {
        xs_fclose(inFile);
    ...
    result = xs_fread(songData, sizeof(guint8), XS_BUF_SIZE, inFile);
    xs_fclose(inFile);
