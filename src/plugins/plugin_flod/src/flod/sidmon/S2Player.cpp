...
int S2Player::load(void* data, unsigned long int _length)
{
        ...
        stream[81] == 'R' && stream[82] == 'S' && stream[83] == 'I' && stream[84] == 'O' && stream[85] == 'N')
    {
        ...
        for (int i = 1; i < len; ++i)
        {
            ...
            position++;
            instr->waveSpeed = stream[position];
            position++;
            instr->waveDelay = stream[position];
            position++;
            instr->arpeggio = stream[position] << 4;
            position++;
            instr->arpeggioLen = stream[position];
            position++;
            instr->arpeggioSpeed = stream[position];
            position++;
            instr->arpeggioDelay = stream[position];
            position++;
            instr->vibrato = stream[position] << 4;
            position++;
            instr->vibratoLen = stream[position];
            position++;
            instr->vibratoSpeed = stream[position];
            position++;
            ...
            position++;
            position += 2;
            instr->attackMax = stream[position];
            position++;
            instr->attackSpeed = stream[position];
            position++;
            instr->decayMin = stream[position];
            position++;
            instr->decaySpeed = stream[position];
            position++;
            instr->sustain = stream[position];
            position++;
            instr->releaseMin = stream[position];
            position++;
            instr->releaseSpeed = stream[position];
            position++;
            instruments[i] = instr;
            position += 9;
        }
        ...
        for (int i = 0; i < len; ++i)
        {
            ...
            position += 2;
            sample->loopPtr = (readEndian(stream[position], stream[position + 1]) << 1) + pos;
            position += 2;
            sample->repeat = readEndian(stream[position], stream[position + 1]) << 1;
            position += 2;
            sample->negStart = (readEndian(stream[position], stream[position + 1]) << 1) + pos;
            position += 2;
            sample->negLen = readEndian(stream[position], stream[position + 1]) << 1;
            position += 2;
            sample->negSpeed = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->negDir = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->negOffset = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->negPos = readEndian(stream[position], stream[position + 1], stream[position + 2],
                                        stream[position + 3]);
            position += 4;
            sample->negCtr = readEndian(stream[position], stream[position + 1]);
            position += 2;
            position += 6;
