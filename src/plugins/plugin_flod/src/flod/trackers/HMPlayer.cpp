...
int HMPlayer::load(void *data, unsigned long int _length) {
    ...
    if (!(stream[1080] == 'F' && stream[1081] == 'E' && stream[1082] == 'S' && stream[1083] == 'T') && !(
    ...
    position++;
    restart = stream[position];
    position++;

    for (int i = 0; i < 128; ++i) {
        track[i] = stream[position];
        position++;
    }
    ...
    for (int i = 1; i < 32; ++i) {
        ...
        if (id == "Mupp") {
            ...
            sample->restart = stream[position];
            position++;
            sample->waveLen = stream[position];
            position++;
            position += 17;
            ...
            sample->volume = stream[position];
            position++;
            sample->loop = readEndian(stream[position], stream[position + 1]) << 1;
            position += 2;
...
void HMPlayer::printData() {
    ...
    for (unsigned int i = 0; i < samples.size(); i++) {
        ...
        if (sample) {
            cout << "Sample [" << i << "] length: " << sample->length << " loop: " << sample->loop << " loopPtr: " <<
                    sample->loopPtr << " name: " << sample->name << " pointer: " << sample->pointer << " repeat: " <<
                    sample->repeat << " volume: " << (int) sample->volume << " finetune: " << sample->finetune <<
                    " restart: " << sample->restart << " waveLen: " << sample->waveLen << "\n";
