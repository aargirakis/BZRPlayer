...
int STPlayer::load(void *data, unsigned long int _length) {
    ...
    for (int i = 0; i < higher; ++i) {
        ...
        row->note = readEndian(stream[position], stream[position + 1]);
        position += 2;
...
void STPlayer::printData() {
    for (unsigned int i = 0; i < patterns.size(); i++) {
        ...
        cout << "Pattern [" << i << "] note: " << row->note << " sample: " << row->sample << " param: " << (int) row->
    ...
    for (unsigned int i = 0; i < samples.size(); i++) {
        ...
        if (sample) {
            cout << "Sample [" << i << "] length: " << sample->length << " loop: " << sample->loop << " loopPtr: " <<
                    ...
                    sample->repeat << " volume: " << (int) sample->volume << "\n";
        }
    }
