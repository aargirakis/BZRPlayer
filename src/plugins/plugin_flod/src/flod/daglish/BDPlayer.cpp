                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        else
                        {
                            if (value == 0xfd)
                            {
                                fadeStep = (signed char)stream[position];
                                position++;
                            }
                            else if (value < 0xc8)
                            {
                        ...
                        if (value != 0x7f)
                        {
                            ...
                            if (m_variant)
                            {
                                ...
                                if (voice->s1byte35)
                                {
                                    ...
                                    voice->s1long38 = (signed int)readEndian(
                                        stream[position], stream[position + 1], stream[position + 2],
                                        stream[position + 3]);
                                    position += 4;
                                }
...
int BDPlayer::load(void* _data, unsigned long int _length)
{
    ...
    do
    {
        ...
        case 0xd040: //add.w [d0,d0]
            ...
            {
                //lea [xx,a3]
                periods = position + (signed short)readEndian(stream[position], stream[position + 1]);
                position += 2;
            }
            else if (value == 0xd040)
            {
                //add.w [d0,d0]
                tempVal = readEndian(stream[position], stream[position + 1]);
                position += 2;
                ...
                {
                    //lea [xx,a0]
                    tracks = position + (signed short)readEndian(stream[position], stream[position + 1]);
                    position += 2;
                }
            }
            break;
        case 0x10c2: //move.b [d2,(a0)+]
            ...
            if (value == 0xb43c || value == 0x0c02)
            {
                //cmp.b [xx,d2] || cmpi.b [xx,d2]
                value = readEndian(stream[position], stream[position + 1]);
                position += 2;
        ...
        case 0x294b: //move.l [a3,xx]
            ...
            patterns = position + (signed short)readEndian(stream[position], stream[position + 1]);
            position += 2;
            ...
            {
                commands = position + (signed short)readEndian(stream[position], stream[position + 1]);
                position += 2;
            }
            break;
        case 0x1030: //move.b (a0,d0.w),d0
            ...
            {
                //lea [xx,a0]
                pos = position + (signed short)readEndian(stream[position], stream[position + 1]);
                position += 2;
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        position += 2;
        sample->word14 = (signed short)readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word16 = (signed short)readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word18 = readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word20 = readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word22 = readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word24 = (signed short)readEndian(stream[position], stream[position + 1]);
        position += 2;
        sample->word26 = readEndian(stream[position], stream[position + 1]);
        position += 2;
...
void BDPlayer::setSample(BDVoice* voice, int counter)
{
    ...
    {
        position = value;
        value = sample->word26 * readEndian(stream[position], stream[position + 1]);
        position += 2;
        temp = sample->word26 * readEndian(stream[position], stream[position + 1]);
        position += 2;
...
void BDPlayer::fx(BDVoice* voice)
{
    ...
    case 0xc0:
        voice->s1byte31 = 0xff;
        voice->s1byte42 = 0;
        voice->s1byte32 = stream[position];
        position++;
        voice->s1byte33 = stream[position];
        position++;
        voice->s1byte34 = (signed char)stream[position];
        position++;
        break;
    case 0xc1:
        voice->s1byte31 = 0;
    ...
    case 0xc2:
        voice->s1byte43 = 0xff;
        voice->s1byte44 = stream[position];
        position++;
        voice->s1byte45 = stream[position];
        position++;
        voice->s1word46 = (signed char)stream[position];
        position++;
        break;
    case 0xc3:
        voice->s1byte43 = 0;
