...
void RHPlayer::initialize()
{
    ...
    do
    {
        ...
        voice->patternPos = readEndian(stream[position], stream[position + 1], stream[position + 2],
                                       stream[position + 3]);
        position += 4;
...
int RHPlayer::load(void* _data, unsigned long int _length)
{
    ...
    while (position < 1024)
    {
        ...
        if (value == 0x7e10 || value == 0x7e20)
        {
            ...
            if (value == 0x41fa)
            {
                ...
                {
                    //adda.l
                    samplesData = i + readEndian(stream[position], stream[position + 1], stream[position + 2],
                                                 stream[position + 3]);
                    position += 4;
                    amiga->loopLen = 64;
                    position += 2;
    ...
    for (int i = 0; i < samplesLen; ++i)
    {
        ...
        if (wavesHeader)
        {
            sample->divider = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->vibrato = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->hiPos = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->loPos = readEndian(stream[position], stream[position + 1]);
            position += 2;
            position += 8;
        }
    }
