...
#include <iostream>
#include "MyEndian.h"
const int FEPlayer::PERIODS[72] = {8192,7728,7296,6888,6504,6136,5792,5464,5160,
                ...
                while (loop > 1) {
                    ...
                    if (value < 0) {
                        ...
                        case -127:
                            ...
                            voice->portaSpeed = patterns[position] * speed; position++;
                            voice->portaNote  = patterns[position];position++;
                            voice->portaLimit = (PERIODS[voice->portaNote] * value) >> 10;
                            voice->portamento = 0;
                            voice->portaDelay = patterns[position] * speed; position++;
                            voice->portaFlag  = 1;
                            voice->patternPos = position;
                            break;
                        case -124:
...
int FEPlayer::load(void* _data, unsigned long int _length)
{
    ...
    while (pos > position) {
        ...
        sample->loopPtr  = (signed short)readEndian(stream[position],stream[position+1]);position+=2;
        ...
        sample->vibratoDelay = stream[position];position++;
        position++;
        sample->vibratoSpeed = stream[position];position++;
        sample->vibratoDepth = stream[position];position++;
        sample->envelopeVol  = stream[position];position++;
        sample->attackSpeed  = stream[position];position++;
        sample->attackVol    = stream[position];position++;
        sample->decaySpeed   = stream[position];position++;
        sample->decayVol     = stream[position];position++;
        sample->sustainTime  = stream[position];position++;
        sample->releaseSpeed = stream[position];position++;
        sample->releaseVol   = stream[position];position++;
        ...
        sample->arpeggioSpeed = stream[position];position++;
        sample->type          = stream[position];position++;
        sample->pulseRateNeg  = stream[position];position++;
        sample->pulseRatePos  = stream[position];position++;
        sample->pulseSpeed    = stream[position];position++;
        sample->pulsePosL     = stream[position];position++;
        sample->pulsePosH     = stream[position];position++;
        sample->pulseDelay    = stream[position];position++;
        sample->synchro       = stream[position];position++;
        sample->blendRate     = stream[position];position++;
        sample->blendDelay    = stream[position];position++;
        sample->pulseCounter  = stream[position];position++;
        sample->blendCounter  = stream[position];position++;
        sample->arpeggioLimit = stream[position];position++;
