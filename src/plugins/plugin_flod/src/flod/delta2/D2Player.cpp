...
void D2Player::printData()
{
    ...
    for(unsigned int i = 0; i < samples.size(); i++)
    {
        ...
        std::cout << "Sample [" << i << "] index: " << sample->index << " length: " << sample->length << " loopPtr: " << sample->loopPtr << " loopPtr: " << sample->loopPtr << " name: " << sample->name << " pitchBend: " << sample->pitchBend << " pointer: " << sample->pointer << " repeat: " << sample->repeat<< " synth: " << (int)sample->synth << "\n";
