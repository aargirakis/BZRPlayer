...
int BPPlayer::load(void* data, unsigned long int _length)
{
    ...
    for (int i = 0; ++i < 16;)
    {
        ...
        if (stream[position] == 0xff)
        {
            ...
            sample->adsrControl = stream[position];
            position++;
            sample->adsrTable = stream[position] << 6;
            position++;
            sample->adsrLen = readEndian(stream[position], stream[position + 1]);
            position += 2;
            sample->adsrSpeed = stream[position];
            position++;
            sample->lfoControl = stream[position];
            position++;
            sample->lfoTable = stream[position] << 6;
            position++;
            sample->lfoDepth = stream[position];
            position++;
            ...
            if (m_version < BPSOUNDMON_V3)
            {
                ...
                position++;
                sample->egControl = stream[position];
                position++;
                sample->egTable = stream[position] << 6;
                position++;
                position++;
                sample->egLen = readEndian(stream[position], stream[position + 1]);
                position += 2;
                position++;
                sample->egDelay = stream[position];
                position++;
                sample->egSpeed = stream[position];
                position++;
                sample->fxSpeed = 1;
                sample->modSpeed = 1;
            ...
            else
            {
                ...
                sample->egSpeed = stream[position];
                position++;
                sample->fxControl = stream[position];
                position++;
                sample->fxSpeed = stream[position];
                position++;
                sample->fxDelay = stream[position];
                position++;
                sample->modControl = stream[position];
                position++;
                sample->modTable = stream[position] << 6;
                position++;
                sample->modSpeed = stream[position];
                position++;
                sample->modDelay = stream[position];
                position++;
                sample->volume = stream[position];
                position++;
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        step->pattern = readEndian(stream[position], stream[position + 1]);
        position += 2;
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        row->note = (signed char)stream[position];
        position++;
