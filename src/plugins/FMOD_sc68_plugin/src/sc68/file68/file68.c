...
/*   } */
  ...
  if (res < 0) {
    ...
    switch (res) {
    case -gzip_cc:
/*       debugmsg68("GZIP VERIFY [%s]!\n", fname); */
/*      if (istream_seek_to(is,0) == 0) {
	istream_t * zis;
	zis = istream_z_create(is,ISTREAM_OPEN_READ,istream_z_default_option);
	res = -1;
	if (!istream_open(zis)) {
	  res = SC68file_verify(zis);
	}
	istream_destroy(zis);
      }*/
		//uncommented by blazer
      break;
    case -ice_cc:
...
///*     } */
//    org_os = os;
//    gzopt = istream_z_default_option;
//    gzopt.level = gzip;
//    gzopt.name  = 0;
//    os = istream_z_create(org_os, 2, gzopt);
//    if (istream_open(os)) {
//      errstr = "open error";
//      goto error;
//    }
