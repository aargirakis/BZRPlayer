...
// ---------------------------------------------------------------------------
//	èoóÕÉeÅ[ÉuÉãÇçÏê¨
//	ëfíºÇ…ÉeÅ[ÉuÉãÇ≈éùÇ¡ÇΩÇŸÇ§Ç™è»ÉXÉyÅ[ÉXÅB
//
...
// ---------------------------------------------------------------------------
//	PSG ÇÃÉåÉWÉXÉ^Ç…ílÇÉZÉbÉgÇ∑ÇÈ
//	regnum		ÉåÉWÉXÉ^ÇÃî‘çÜ (0 - 15)
//	data		ÉZÉbÉgÇ∑ÇÈíl
//
...
// ---------------------------------------------------------------------------
//	PCM ÉfÅ[É^ÇìfÇ´èoÇ∑(2ch)
//	dest		PCM ÉfÅ[É^ÇìWäJÇ∑ÇÈÉ|ÉCÉìÉ^
//	nsamples	ìWäJÇ∑ÇÈ PCM ÇÃÉTÉìÉvÉãêî
//
void PSG::Mix(Sample* dest, int nsamples)
{
	...
	if ((r7 & 0x3f) | ((reg[8] | reg[9] | reg[10]) & 0x1f))
	{
		...
		if (p1 != &env && p2 != &env && p3 != &env)
		{
			...
			// ÉGÉìÉxÉçÅ[ÉvÇÃåvéZÇÇ≥Ç⁄Ç¡ÇΩí êKÇ ÇÌÇπ
