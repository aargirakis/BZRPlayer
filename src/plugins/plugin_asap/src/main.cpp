...
FMOD_RESULT F_CALLBACK open(FMOD_CODEC_STATE* codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO* userexinfo)
{
    ...
    song = ASAPInfo_GetDefaultSong(plugin->asap_info);
    plugin->current_song = song;
    duration = ASAPInfo_GetDuration(plugin->asap_info, song);
...
{
    auto* plugin = static_cast<pluginAsap*>(codec->plugindata);
    *read = ASAP_Generate(plugin->asap, static_cast<unsigned char*>(buffer), size << plugin->waveformat.channels, ASAPSampleFormat_S16_L_E);
    return FMOD_OK;
}
