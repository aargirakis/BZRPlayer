...
FMOD_RESULT F_CALLBACK open(FMOD_CODEC_STATE *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo)
{
    ...
    if (psfType == 0x11 || psfType == 0x12)
    {
        ...
        if (extPos == std::string::npos || _stricmp(ahx->info->filename.c_str() + extPos + 1, psfType == 0x11 ? "ssflib" : "dsflib") != 0)
        {
            if (psf_load(ahx->info->filename.c_str(), &ahx->m_psfFileSystem, uint8_t(psfType), ahx->SdsfLoad, ahx, nullptr, nullptr, 0, nullptr, nullptr) >= 0)
            {
