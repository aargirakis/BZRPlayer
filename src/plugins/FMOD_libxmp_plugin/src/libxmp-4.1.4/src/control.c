...
//added by blazer (code from xmp)
void xmp_change_sequence(xmp_context opaque, int i)
{
    struct context_data *ctx = (struct context_data *)opaque;

    struct module_data *mi = &ctx->m;
    ...
    xmp_set_position(opaque, mi->seq_data[seq].entry_point);
}
