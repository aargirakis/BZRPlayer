...
FMOD_RESULT F_CALLBACK open(FMOD_CODEC_STATE* codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO* userexinfo)
{
    ...
    ahx->psfType = psf_load(ahx->info->filename.c_str(), &ahx->m_psfFileSystem, 0, nullptr, nullptr, ahx->InfoMetaPSF,
                            ahx, 1, nullptr, nullptr);
    ...
    if (extPos == std::string::npos || _stricmp(ahx->info->filename.c_str() + extPos + 1,
                                                ahx->psfType == 0x1 ? "psflib" : "psf2lib") != 0)
    {
        ...
        if (psf_load(ahx->info->filename.c_str(), &ahx->m_psfFileSystem, uint8_t(ahx->psfType),
                     ahx->psfType == 1 ? ahx->PsfLoad : psf2fs_load_callback, ahx->psfType == 1 ? ahx : ahx->m_psf2fs,
...
FMOD_RESULT F_CALLBACK setposition(FMOD_CODEC_STATE* codec, int subsound, unsigned int position, FMOD_TIMEUNIT postype)
{
    ...
    psf_load(ahx->info->filename.c_str(), &ahx->m_psfFileSystem, uint8_t(ahx->psfType),
             ahx->psfType == 1 ? ahx->PsfLoad : psf2fs_load_callback, ahx->psfType == 1 ? ahx : ahx->m_psf2fs, nullptr,
