...
#include "src/patternview/ProTracker1PatternView.h"
#include "src/patternview/SoundFXPatternView.h"
#include "src/patternview/ScreamTracker3PatternView.h"
#include "src/patternview/ScreamTracker2PatternView.h"
#include "src/patternview/UltimateSoundTrackerPatternView.h"
#include "src/patternview/Composer669PatternView.h"
#include "src/patternview/HivelyTrackerPatternView.h"
#include "src/patternview/AHXPatternView.h"
#include "src/patternview/MultiTrackerPatternView.h"
#include "src/patternview/DigiBooster17PatternView.h"
#include "src/patternview/StarTrekker13PatternView.h"
#include "src/patternview/UltraTrackerPatternView.h"
#include "src/patternview/GameMusicCreatorPatternView.h"
#include "src/patternview/IceTrackerPatternView.h"
#include "src/patternview/ChipTrackerPatternView.h"
#include "src/patternview/ImpulseTrackerPatternView.h"
#include "src/patternview/OktalyzerPatternView.h"
#include "src/patternview/FastTracker2PatternView.h"
#include "src/patternview/FastTracker1PatternView.h"
#include "src/patternview/MEDPatternView.h"
#include "src/patternview/DigiBoosterProPatternView.h"
#include "src/patternview/OctaMEDPatternView.h"
#include "src/patternview/FastTracker24ChanPatternView.h"
#include "src/patternview/FastTracker26ChanPatternView.h"
#include "src/patternview/NoiseTrackerPatternView.h"
#include "src/patternview/SoundTracker26PatternView.h"
#include "src/patternview/OctaMED44ChanPatternView.h"
#include "src/patternview/OctaMED54ChanPatternView.h"
#include "src/patternview/OctaMED5ChanPatternView.h"
#include "src/patternview/OctaMEDSoundstudioPatternView.h"
#include "src/patternview/ProTracker36PatternView.h"
#include "src/patternview/GenericPatternView.h"
...
void Tracker::init()
{
    ...
    else if(QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (6chn") || QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (8chn") || QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (16ch"))
    {
    ...
    else if((QString(m_info->fileformat.c_str()).toLower().startsWith("protracker") && !QString(m_info->fileformat.c_str()).toLower().startsWith("protracker xm")) || QString(m_info->fileformat.c_str()).toLower().startsWith("probably converted (m.k") || QString(m_info->fileformat.c_str()).toLower().startsWith("unknown/converted (m.k") || QString(m_info->fileformat.c_str()).toLower().startsWith("converted 15 ins")  || QString(m_info->fileformat.c_str()).toLower().startsWith("unknown or converted (M"))
    {
    ...
    else if(QString(m_info->fileformat.c_str()).toLower().startsWith("noise") || QString(m_info->fileformat.c_str()).toLower().startsWith("his master"))
    {
    ...
    else if(QString(m_info->fileformat.c_str()).toLower().startsWith("fasttracker 2") || QString(m_info->fileformat.c_str()).toLower().startsWith("skale tracker xm") || QString(m_info->fileformat.c_str()).toLower().startsWith("madtracker 2.0 xm")|| QString(m_info->fileformat.c_str()).toLower().endsWith("xm 1.04"))
    {
        m_renderTop = true;
...
{
    if(yPixelPosition > m_trackerview->height() || numPixels > painter->window().width()/m_scale || yPixelPosition < 0 || yPixelPosition > painter->window().height()/m_scale)
    {
        return;
    }
    ...
    if((m_trackerview->font().family()!="DejaVu Sans Mono"))
    {
        ...
        for(int i = 0;i<text.length();i++)
        {
            ...
            if(m_font.m_characterWidths.isEmpty())
            {
                ...
                painter->drawPixmap(x,y,m_font.m_fontWidth,m_font.m_fontHeight, m_font.m_characterMap,m_font.m_characterPositions.value(text.at(i)).x(),m_font.m_characterPositions.value(text.at(i)).y(),m_font.m_fontWidth,m_font.m_fontHeight);
            }
            //variable width font
            else
            {
                ...
                int y = yPixelPosition-(m_font.m_fontHeight);
                painter->drawPixmap(x,y,m_font.m_characterWidths.value(text.at(i)),m_font.m_fontHeight, m_font.m_characterMap,m_font.m_characterPositions.value(text.at(i)).x(),m_font.m_characterPositions.value(text.at(i)).y(),m_font.m_characterWidths.value(text.at(i)),m_font.m_fontHeight);
...
void Tracker::paint(QPainter *painter, QPaintEvent *event)
{
    ...
    //if(currentRow!=m_currentRow || m_renderVUMeter || updateHively) //only redraw if it's a new row
    {
        ...
        while(i<outerLoopBreakValue)
        {
            ...
            {
                yPixelPosition=((m_trackerview->fontHeight()+m_trackerview->yOffsetRowAfter())*j)-(currentRow*(m_trackerview->font().pixelSize()+m_trackerview->yOffsetRowAfter()));
            ...
            if(!m_trackerview->rowStart().isEmpty())
            {
                ...
                drawText(m_trackerview->rowStart(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
            ...
            if(j==currentRow && m_trackerview->bitmapFont().m_bitmapFontPath==m_trackerview->bitmapFontRownumber().m_bitmapFontPath)
            {
                setFont(m_trackerview->currentRowBitmapFont());
            ...
            for(int chan = 0; chan < m_info->numChannels ; chan++)
            {
                ...
                drawText(channelSeparator,painter,(m_trackerview->xOffsetRow()+numPixels),height/2+yPixelPosition);
                numPixels+=channelSeparator.length()*fontWidth;
                drawText(strRowNumberMultiple,painter,(m_trackerview->xOffsetRow()+numPixels),height/2+yPixelPosition);
                ...
                drawText(m_trackerview->separatorNote(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);

                numPixels+=m_trackerview->separatorNote().length()*fontWidth;
                if(instrument==m_trackerview->emptyInstrument() && !m_trackerview->noEmptyInstrumentColor() && j!=currentRow)
                {
                    if(!alternateChannelColors)
                    {
                        painter->setPen(m_trackerview->colorEmpty());
                ...
                if(j==currentRow && m_trackerview->bitmapFont().m_bitmapFontPath==m_trackerview->bitmapFontInstrument().m_bitmapFontPath)
                {
                    setFont(m_trackerview->currentRowBitmapFont());
                    fontWidth = m_trackerview->fontWidth();
                }
                ...
                drawText(m_trackerview->separatorInstrument(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                ...
                drawText(m_trackerview->separatorVolume(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                numPixels+=m_trackerview->separatorVolume().length()*fontWidth;

                if(!m_trackerview->effectsThenParametersEnabled())
                {
                    if(effect==m_trackerview->emptyEffect() && !m_trackerview->noEmptyEffectColor() &&j!=currentRow)
                    {
                        if(!alternateChannelColors)
                        {
                            painter->setPen(m_trackerview->colorEmpty());
                    ...
                    else
                    {
                        if(!alternateChannelColors)
                        {
                            if(m_trackerview->useInstrumentColorOnEffectAndParameterFrequency() && j % m_trackerview->useInstrumentColorOnEffectAndParameterFrequency()==0)
                            {
                                painter->setPen(colorInstrument);
                            }
                            else
                    ...
                    if(j==currentRow && m_trackerview->bitmapFont().m_bitmapFontPath==m_trackerview->bitmapFontEffects().m_bitmapFontPath)
                    {
                        setFont(m_trackerview->currentRowBitmapFont());
                        fontWidth = m_trackerview->fontWidth();
                    }
                    ...
                    drawText(m_trackerview->separatorEffect(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                    numPixels+=m_trackerview->separatorEffect().length()*fontWidth;

                    if(parameter==m_trackerview->emptyParameter() && !m_trackerview->noEmptyParameterColor() &&j!=currentRow)
                    {
                        if(!alternateChannelColors)
                        {
                            painter->setPen(m_trackerview->colorEmpty());
                    ...
                    if(j==currentRow && m_trackerview->bitmapFont().m_bitmapFontPath==m_trackerview->bitmapFontParameters().m_bitmapFontPath)
                    {
                        setFont(m_trackerview->currentRowBitmapFont());
                        fontWidth = m_trackerview->fontWidth();
                    }
                    ...
                    drawText(m_trackerview->separatorParameter(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                    ...
                    drawText(m_trackerview->separatorEffect2(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                    numPixels+=m_trackerview->separatorEffect2().length()*fontWidth;

                    if(parameter2==m_trackerview->emptyParameter() && !m_trackerview->noEmptyParameter2Color() &&j!=currentRow)
                    {
                        if(!alternateChannelColors)
                        {
                            painter->setPen(m_trackerview->colorEmpty());
                    ...
                    drawText(m_trackerview->separatorParameter2(),painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
                ...
                else
                {
                    ...
                    if(effect2==m_trackerview->emptyEffect() && !m_trackerview->noEmptyEffect2Color() && j!=currentRow)
                    {
                        if(!alternateChannelColors)
                        {
                            painter->setPen(m_trackerview->colorEmpty());
                    ...
                    if(parameter2==m_trackerview->emptyParameter() && !m_trackerview->noEmptyParameterColor() && j!=currentRow)
                    {
                        if(!alternateChannelColors)
                        {
                            painter->setPen(m_trackerview->colorEmpty());
            ...
            if(m_trackerview->rowNumbersLastChannelEnabled())
            {
                ...
                painter->setPen(colorRowNumber);
                drawText(m_trackerview->separatorRowNumberLast()+row,painter,m_trackerview->xOffsetRow()+numPixels,height/2+yPixelPosition);
...
//            }
        ...
        if(m_renderTop)
        {
            ...
            setFont(m_trackerview->bitmapFont());
            if(QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (6chn") || QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (8chn") || QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (16ch"))
            {
                m_height=31;
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("protracker mod (flt4)") )
            {
                ...
                drawText(QString("%1").arg(m_info->title.c_str(),-20,QChar('_')).toUpper(),painter,left+(72),top+(22));
                ...
                drawText(QString("%1").arg(m_trackerview->getCurrentSample()+1, 2, 10, QChar('0')),painter,left+(24),top+(33));
                drawText(QString("%1").arg(m_info->samples[m_trackerview->getCurrentSample()].c_str(),-22,QChar('_')).toUpper(),painter,left+(141),top+(33));
            ...
            else if((QString(m_info->fileformat.c_str()).toLower().startsWith("protracker") && !QString(m_info->fileformat.c_str()).toLower().startsWith("protracker xm"))  || QString(m_info->fileformat.c_str()).toLower().startsWith("probably converted (m.k") || QString(m_info->fileformat.c_str()).toLower().startsWith("unknown/converted (m.k") || QString(m_info->fileformat.c_str()).toLower().startsWith("converted 15 ins") || QString(m_info->fileformat.c_str()).toLower().startsWith("unknown or converted (M"))
            {
                ...
                drawText(QString("%1").arg(m_info->title.c_str(),-20,QChar('_')).toUpper(),painter,left+(141),top+(11));
                ...
                drawText(QString("%1").arg(m_trackerview->getCurrentSample()+1, 2, 10, QChar('0')),painter,left+(24),top+(22));
                drawText(QString("%1").arg(m_info->samples[m_trackerview->getCurrentSample()].c_str(),-22,QChar('_')).toUpper(),painter,left+141,top+22);
                m_pen.setWidth(1);
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("chiptracker"))
            {
                ...
                drawText(QString("%1").arg(m_currentPosition, 4, 16, QChar('0')).toUpper(),painter,left+71,top+0);
                drawText(QString("%1").arg(m_info->restart, 4, 16, QChar('0')).toUpper(),painter,left+(178),top+0);
                drawText(QString("%1").arg(m_info->numOrders, 4, 16, QChar('0')).toUpper(),painter,left+(285),top+0);
                drawText(QString("%1").arg(m_info->title.c_str(),-22,QChar('_')).toUpper(),painter,left+(137),top+(11));
                ...
                painter->setPen(QColor(0,0,0));
                drawText(QString("%1").arg(m_trackerview->getCurrentSample()+1, 4, 16, QChar('0')).toUpper(),painter,left+(24),top+(22));
                drawText(QString("%1").arg(m_info->samples[m_trackerview->getCurrentSample()].c_str(),-22,QChar('_')).toUpper(),painter,left+(137),top+(22));
            }
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("fasttracker 2") || QString(m_info->fileformat.c_str()).toLower().startsWith("skale tracker xm") || QString(m_info->fileformat.c_str()).toLower().startsWith("madtracker 2.0 xm")|| QString(m_info->fileformat.c_str()).toLower().endsWith("xm 1.04"))
            {
                ...
                {
                    painter->fillRect(left,0,55+(m_info->numChannels*96*m_trackerview->fontWidth()/8),m_height,colorBase);
                }
                else
                {
                    painter->fillRect(left,0,55+(m_info->numChannels*72*m_trackerview->fontWidth()/8),m_height,colorBase);
                }
                ...
                if(m_info->numChannels>4 && m_info->numChannels<7)
                {
                    painter->fillRect(left+55+(m_info->numChannels*96*m_trackerview->fontWidth()/8),0,1,m_height,colorShadow);
                }
                else
                {
                    painter->fillRect(left+55+(m_info->numChannels*72*m_trackerview->fontWidth()/8),0,1,m_height,colorShadow);
                }

                //top
                if(m_info->numChannels>4 && m_info->numChannels<7)
                {
                    painter->fillRect(left,0,55+(m_info->numChannels*96*m_trackerview->fontWidth()/8),1,colorHilite);
                }
                else
                {
                    painter->fillRect(left,0,55+(m_info->numChannels*72*m_trackerview->fontWidth()/8),1,colorHilite);
                }

                //bottom
                if(m_info->numChannels>4 && m_info->numChannels<7)
                {
                    painter->fillRect(left,m_height-1,55+(m_info->numChannels*96*m_trackerview->fontWidth()/8),1,colorShadow);
                }
                else
                {
                    painter->fillRect(left,m_height-1,55+(m_info->numChannels*72*m_trackerview->fontWidth()/8),1,colorShadow);
                }
                ...
                setFont(m_trackerview->infoFont());
                drawText(QString("%1").arg(m_info->modPatternRows, 3, 16, QChar('0')).toUpper(),painter,left+156,26);
                //title frame
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd3"))
            {
                ...
                setFont(m_trackerview->infoFont());
                painter->setPen(QColor(0,0,0));
                drawText(QString("Block ") + QString::number(m_currentPattern)+"/"+QString::number(m_info->numPatterns-1),painter,left+30,30);
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd2"))
            {
                ...
                drawText(QString("%1").arg(m_currentPosition+1, 3, 10, QChar('0'))+"/"+QString("%1").arg(m_info->numOrders, 3, 10, QChar('0')),painter,left+(191),44);
                drawText("B",painter,left+(255),44);
                drawText(QString("%1").arg(m_currentPattern, 3, 10, QChar('0'))+"/"+QString("%1").arg(m_info->numPatterns-1, 3, 10, QChar('0'))+":",painter,left+(272),44);
            ...
            else if(QString(m_info->fileformat.c_str())=="AHX")
            {
                ...
                {
                    drawText(QString("%1").arg(m_info->modTrackPositions[i], 3, 10, QChar('0')),painter,left+(32)+(i*72),top+(40));
                }
                ...
                drawText(QString("%1").arg(m_info->modPatternRestart, 3, 10, QChar('0')),painter,left+126,top+(11));
            ...
            else if(QString(m_info->fileformat.c_str())=="HivelyTracker")
            {
                ...
                for(int i = 0 ; i<m_info->modTrackPositions.size();i++)
                {
                    ...
                    painter->setFont(font);
                    painter->drawText(left+(108)+(i*120),m_height-8,QString("%1").arg(m_info->modTrackPositions[i], 3, 10, QChar('0')));
                    m_pen.setColor(colorShadow);
                    painter->setPen(m_pen);
                ...
                painter->drawText(left+(96),top+(64),QString("%1").arg(m_info->modPatternRestart, 3, 10, QChar('0')));
                painter->drawText(left+(217),top+(24),QString("%1").arg(m_info->modPatternRows, 3, 10, QChar('0')));
            ...
            else if(QString(m_info->fileformat.c_str()).toLower()== "soundtracker")
            {
                ...
                drawText(QString("%1").arg(m_currentPattern, 4, 10, QChar('0')),painter,left+(178),top+0);
                drawText(QString("%1").arg(m_info->numOrders, 4, 10, QChar('0')),painter,left+(285),top+0);
                drawText(QString("%1").arg(m_info->title.c_str(),-20,QChar('_')).toUpper(),painter,left+(143),top+(11));
                drawText(QString("%1").arg(m_trackerview->getCurrentSample()+1, 2, 16, QChar('0')).toUpper(),painter,left+(26),top+(22));
                drawText(QString("%1").arg(m_info->samples[m_trackerview->getCurrentSample()].c_str(),-22,QChar('_')).toUpper(),painter,left+(143),top+(22));
                m_pen.setWidth(1);
    ...
    if(m_render && m_renderVUMeter)
        {
            ...
            else if(QString(m_info->fileformat.c_str()).toLower().startsWith("ultimate") || QString(m_info->fileformat.c_str()).toLower().startsWith("soundtracker"))
            {
                HEIGHT = 48;
                WIDTH = 8;
                LEFT_OFFSET = 56;
            ...
            if(QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd0") || QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd1") || QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd2"))
            {
            ...
            {
                unsigned char volume = static_cast<unsigned char>(((m_info->modVUMeters[i]/64.0)+0.005)*100); //volume is between 0-100. 0.005 for rounding
                ...
                QRect rectLDark(xPos+vuWidth-HILIGHT_WIDTH,maxHeight-vumeterCurrentHeight,HILIGHT_WIDTH,vumeterCurrentHeight);
                ...
                else if(QString(m_info->fileformat.c_str()).toLower().startsWith("noise") || QString(m_info->fileformat.c_str()).toLower().startsWith("chiptracker") || QString(m_info->fileformat.c_str()).toLower().startsWith("ice tracker")|| QString(m_info->fileformat.c_str()).toLower().startsWith("mnemotron")|| QString(m_info->fileformat.c_str()).toLower().startsWith("his master"))
                {
                    //main color
                ...
                else if(QString(m_info->fileformat.c_str()).toLower().startsWith("pt3.6") || QString(m_info->fileformat.c_str()).toLower().startsWith("pt4"))
                {
                    //main color
                ...
                else if(QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd0") || QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd1")|| QString(m_info->fileformat.c_str()).toLower().startsWith("octamed (mmd2"))
                {
                    //main color
                ...
                else if(QString(m_info->fileformat.c_str()).toLower().startsWith("ultimate") || QString(m_info->fileformat.c_str()).toLower().startsWith("soundtracker")|| QString(m_info->fileformat.c_str()).toLower().startsWith("converted st2"))
                {
                    //main color
        ...
        for(int i = 0; i < m_info->numChannels; i++)
        {
            if(SoundManager::getInstance().isChannelMuted(i))
            {
                ...
                int x = m_trackerview->channelStart()+(i*m_trackerview->channelWidth())+(i*m_trackerview->channelxSpace());
...
void Tracker::drawVerticalEmboss(int xPos,int yPos, int height, QColor hilite, QColor shadow, QColor base, QPainter* painter, bool left, bool right)
{
