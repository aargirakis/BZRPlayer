...
//added by blazer
void CSoundFile::GetSampleData(UINT nInstr,signed char* d) const
//-----------------------------------------------------------
{
        char sztmp[40] = "";  // changed from CHAR
	if ((nInstr >= MAX_SAMPLES))
	{
		//do nothing
	}
	else
	{
		d = Ins[nInstr].pSample;
	}
}
//added by blazer
unsigned char CSoundFile::GetChannelVUMeter(unsigned char* channels) const
//-----------------------------------------------------------
{

	unsigned char numChannels = GetNumChannels();
	for(int i = 0; i<numChannels;i++)
	{
		channels[i] = Chn[i].nVUMeter;
	}
	return numChannels;
}
#ifndef NO_PACKING
