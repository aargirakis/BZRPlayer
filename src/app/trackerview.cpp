...
void TrackerView::paintEvent(QPaintEvent *event)
{
    ...
    if(tracker==nullptr || !inited || !tracker->m_render || !SoundManager::getInstance().IsPlaying() || (SoundManager::getInstance().m_Info1==nullptr))
    {
    ...
    if (obj == this)
    {
        ...
        if (event->type() == QEvent::MouseButtonRelease && (mouseEvent->button() & Qt::LeftButton) )
        {
            if(tracker->m_render)
            {
                ...
                if(x>=tracker->m_trackerview->getbuttonNextSampleX() && x<=(tracker->m_trackerview->getbuttonNextSampleX()+tracker->m_trackerview->getbuttonNextSampleWidth()) && y>=tracker->m_trackerview->getbuttonNextSampleY() && y<=(tracker->m_trackerview->getbuttonNextSampleY()+tracker->m_trackerview->getbuttonNextSampleHeight()))
                {
                ...
                else if(x>=tracker->m_trackerview->getbuttonPrevSampleX() && x<=(tracker->m_trackerview->getbuttonPrevSampleX()+tracker->m_trackerview->getbuttonPrevSampleWidth()) && y>=tracker->m_trackerview->getbuttonPrevSampleY() && y<=(tracker->m_trackerview->getbuttonPrevSampleY()+tracker->m_trackerview->getbuttonPrevSampleHeight()))
                {
