...
#else
#ifdef OUTPUT_DIR
    dataPath = QString::fromStdString(OUTPUT_DIR) + DATA_DIR;
    userPath = QString::fromStdString(OUTPUT_DIR) + "/" + USER_DIR;
#else
    dataPath = QString::fromStdString(DATA_DIR);
...
void MainWindow::updateScrollText()
{
    ...
    if (ui->visualizer->getEffect()->getCustomScrolltextEnabled()) {
        ...
        visualizerText = QString::fromStdString(SoundManager::getInstance().m_Info1->artist);
        if (!visualizerText.isEmpty()) {
            visualizerText += " ";
        }

        visualizerText += QString::fromStdString(SoundManager::getInstance().m_Info1->game);
        if (!visualizerText.isEmpty()) {
            visualizerText += " ";
        }

        visualizerText += QString::fromStdString(SoundManager::getInstance().m_Info1->comments);
        if (!visualizerText.isEmpty()) {
            visualizerText += " ";
        }

        visualizerText += QString::fromStdString(SoundManager::getInstance().m_Info1->copyright);
        if (!visualizerText.isEmpty()) {
            visualizerText += " ";
        }

        visualizerText += QString::fromStdString(SoundManager::getInstance().m_Info1->date);
        if (!visualizerText.isEmpty()) {
            visualizerText += " ";
        }
        ...
        if (SoundManager::getInstance().m_Info1->instruments != nullptr) {
            for (int i = 0; i < SoundManager::getInstance().m_Info1->numInstruments; i++) {
                visualizerText += QString::fromStdString(
        ...
        if (SoundManager::getInstance().m_Info1->samples != nullptr) {
            for (int i = 0; i < SoundManager::getInstance().m_Info1->numSamples; i++) {
                visualizerText += QString::fromStdString(SoundManager::getInstance().m_Info1->samples[i]) +
                        QString(" ");
            }
        }
...
void MainWindow::PlaySong(int currentRow)
{
    ...
    if (loadSound(fullPath))
    {
        ...
        if (pi.info->title != "")
        {
            ...
            {
                filename = QString::fromStdString(pi.info->title);
            }
        }
        ...
        if (pi.info->artist != "")
        {
            ...
            {
                artist = QString::fromStdString(pi.info->artist);
            }
        }
        else if (pi.info->author != "")
        {
            ...
            {
                artist = QString::fromStdString(pi.info->author);
            }
        }
        else if (pi.info->composer != "")
        {
            ...
            {
                artist = QString::fromStdString(pi.info->composer);
            }
        }
        if (artist != "")
        {
