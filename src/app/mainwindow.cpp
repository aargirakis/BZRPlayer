    ...
    ui(new Ui::MainWindow)
{
    ...
    QSettings settings(QApplication::applicationDirPath() + QDir::separator() + "user/settings.ini",QSettings::IniFormat);
    ...
    if(!instanceExists)
    {
        ...
        getEffect()->setScrollerReflectionColor(QColor(settings.value("scroller/reflectionColor","#00032e").toString()));
        ...
        connect(ui->tableInfo->horizontalHeader(),&QHeaderView::sectionResized,[=]( int logicalIndex, int oldSize, int newSize) {//with lambda
        ...
        discordActivityTimer->setInterval(2000);
        discordActivityTimer->start(2000);
        connect( m_Timer, SIGNAL(timeout()), this, SLOT(discordUpdateActivity()) );
        ...
        if(!QFileInfo::exists(QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + "Default.m3u"))
        {
        ...
        addDebugText("Loading " + QString::number(playlists.count()) + " playlists from " + QApplication::applicationDirPath() + QDir::separator() + "user/playlists");
        ...
        foreach(QString filename, playlists)
        {
            ...
            tableWidgetPlaylists[f.fileName()]->setStyleSheet(ui->dockWidgetContents_4->styleSheet() + "QHeaderView::section{font-family:Roboto;padding:0;} QTableView{padding:9px;}");
            ...
            connect(tableWidgetPlaylists[f.fileName()], SIGNAL(doubleClicked(const QModelIndex &)), SLOT(on_playlist_itemDoubleClicked(const QModelIndex &)));
            ...
            if(!QFileInfo::exists(QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + "Default.m3u") && filename=="Default.m3u")
            {
                swapColumns(tableWidgetPlaylists[f.fileName()]);
            }
            tableWidgetPlaylists[f.fileName()]->horizontalHeader()->restoreState(settings.value(QString("playlist_") + f.fileName(),"").toByteArray());
            ...
            QUrl u = QUrl::fromLocalFile( QDir::separator() + QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + "/" + f.fileName());
            QList<QUrl> ql;
            ql.append(u);
        ...
        if(!QFileInfo::exists(QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + currentPlaylist))
        {
        ...
        connect(ui->samples->horizontalHeader(),&QHeaderView::sectionResized,[=]( int logicalIndex, int oldSize, int newSize) {//with lambda
        ...
        connect(ui->instruments->horizontalHeader(),&QHeaderView::sectionResized,[=]( int logicalIndex, int oldSize, int newSize) {//with lambda
...
void MainWindow::dockWindowClosed(bool b)
{
    foreach(ads::CDockWidget* dock, dockWidgets)
    {
        ...
        {
            if(menuAction->text()==dock->windowTitle() || ((dock->windowTitle().toLower().endsWith(".m3u") || dock->windowTitle().toLower().endsWith(".m3u8")) && menuAction->text()=="Playlist"))
            {
...
//            }
        ...
        {
            QKeyEvent *keyEvent = static_cast<QKeyEvent*>(event);
            if(keyEvent->key() == Qt::Key_Space || keyEvent->key() == Qt::Key_Escape || keyEvent->key() == Qt::Key_Enter || keyEvent->key() == Qt::Key_Return)
            {
    ...
    else if (event->type() == QEvent::Wheel && obj == ui->labelFilename && QApplication::keyboardModifiers()==(Qt::ControlModifier))
    {
    ...
    else if (event->type() == QEvent::Wheel && obj == ui->visualizer && QApplication::keyboardModifiers()==(Qt::ControlModifier))
    {
        QWheelEvent *wheelEvent = static_cast<QWheelEvent*>(event);
        QPoint numPixels = wheelEvent->pixelDelta();
        QPoint numDegrees = wheelEvent->angleDelta() / 8;
        ...
        scale=scale/40;
        scale=1-scale;
        if(scale*getEffect()->getResolutionWidth()>=80 && scale*getEffect()->getResolutionHeight()>=80 && scale*getEffect()->getResolutionWidth()<=4096 && scale*getEffect()->getResolutionHeight()<=4096)
        {
...
unsigned MainWindow::getFontSize(QRect rect, QFont font, QString text)
{
    ...
    while( i<30 )
    {
        ...
        addDebugText("r.height(): " + QString::number(r.height()) + " rect.height(): " + QString::number(rect.height()) + " r.width(): " + QString::number(r.width()) + " rect.width():" + QString::number(rect.width()));
...
void MainWindow::timerProgress()
{
    ...
    if(playStarted)
    {
        ...
        if(currentMs>=song_length_ms || (!SoundManager::getInstance().IsPlaying() && !SoundManager::getInstance().GetPaused()) )
        {
            playNextSong(false);
        }
    }
...
void MainWindow::dragMoveEvent(QDragMoveEvent *event)
{
    if (event->mimeData()->hasFormat("text/uri-list"))
    {
        ...
        if(DockWidgetPlaylists->isCurrentTab())
        {
            ...
            dockWidgetPlaylistsRect.setBottom(pointPlaylists.y()+dockWidgetPlaylistsRect.height()-UNKNOWN_OFFSET+16);
            ...
            else if(DockWidgetPlaylist->isCurrentTab())
            {
                ...
                dockWidgetPlaylistRect.setBottom(pointPlaylist.y()+dockWidgetPlaylistRect.height()-(UNKNOWN_OFFSET/2)+8);
    ...
    while (it.hasNext())
    {
        ...
        if(filenameFullPath.endsWith(".m3u",Qt::CaseInsensitive) || filenameFullPath.endsWith(".m3u8"))
        {
            ...
            for(unsigned int e = 0; e < playlistEntries.size() ; e++)
            {
                ...
                addPlaylistEntry(tableWidgetPlaylists[playlistName],tableWidgetPlaylists[playlistName]->model()->rowCount(),playlistEntries.at(e)->title,playlistEntries.at(e)->fileFormat,length,playlistEntries.at(e)->startSubsongPlayList,playlistEntries.at(e)->fullPath,playlistEntries.at(e)->length,playlistEntries.at(e)->artist);
        ...
        {
            addPlaylistEntry(tableWidgetPlaylists[playlistName],tableWidgetPlaylists[playlistName]->model()->rowCount(),filename,"",0,0,filenameFullPath,0,"");
...
void MainWindow::addPlaylistEntry(QTableView* table, int rowPosition, QString filename, QString fileFormat, QString length, int subsong, QString fullPath, int lengthInt, QString artist)
{
...
//        addDebugText("startSubsongPlayList: " + QString::number(playlists[currentPlaylist].at(playlistNumber)->startSubsongPlayList));
        ...
        tableWidgetPlaylists[currentPlaylist]->model()->setData(index, SoundManager::getInstance().m_Info1->fileformat.c_str(), Qt::EditRole);
        index = tableWidgetPlaylists[currentPlaylist]->model()->index(currentRow, 2, QModelIndex());
        tableWidgetPlaylists[currentPlaylist]->model()->setData(index, msToNiceStringExact(song_length_ms,m_displayMilliseconds), Qt::EditRole);
...
//        //OK, now to add all subsongs, one for each row
        ...
        if(tableWidgetPlaylists[currentPlaylist]->model()->index(currentRow,3).data().toInt()<1 && SoundManager::getInstance().m_Info1->numSubsongs>1)
        {
            ...
            {
                addPlaylistEntry(tableWidgetPlaylists[currentPlaylist],currentRow+i,title,fileformat,"",i+1,fullpath,0,artist);
            }
...
//        }
        ...
        else
        {
            ...
            if(SoundManager::getInstance().m_Info1->instruments!=nullptr)
            {
                ...
                {
                    visualizerText+=QString::fromLocal8Bit(SoundManager::getInstance().m_Info1->instruments[i].c_str()) + QString(" ");
                }
            }
            if(SoundManager::getInstance().m_Info1->samples!=nullptr)
            {
                ...
                {
                    visualizerText+=QString::fromLocal8Bit(SoundManager::getInstance().m_Info1->samples[i].c_str()) + QString(" ");
                }
            }
    ...
    if(SoundManager::getInstance().m_Info1!=nullptr)
    {
        if(SoundManager::getInstance().m_Info1->plugin == PLUGIN_libopenmpt)
        {
            if(SoundManager::getInstance().m_Info1->numSamples>0)
            {
                ...
                columnLabelsSamples << tr("#") << tr("Name") << tr("Size") << tr("Loopstart") << tr("Loopend") << tr("Volume") << tr("Finetune") /*<< tr("Resolution") << tr("Channels")*/;
                ...
                for(int j = 0; j < SoundManager::getInstance().m_Info1->numSamples; j++)
                {
                    ...
                    if( size > 0 )
                    {
                        ...
                        ui->samples->setItem(j,2,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesSize[j]) ));
                        ui->samples->setItem(j,3,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesLoopStart[j]) ));
                        ui->samples->setItem(j,4,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesLoopEnd[j])  ));
                        ui->samples->setItem(j,5,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesVolume[j])  ));
                        ui->samples->setItem(j,6,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesFineTune[j])  ));
                    }
            ...
            if(SoundManager::getInstance().m_Info1->numInstruments>0)
            {
                ...
                columnLabelsInstruments << tr("#") << tr("Name") /*<< tr("Volume") << tr("Wavelen") << tr("Attack") << tr("Attack Volume") << tr("Decay") << tr("Decay Volume") << tr("Sustain") << tr("Sustain Volume") << tr("Release") << tr("Release Volume") << tr("Delay") << tr("Delay Depth") << tr("Delay Speed") << tr("Filter Lower Limit") << tr("Filter Upper Limit") << tr("Filterspeed") << tr("Filter Lower Limit") << tr("Filter Upper Limit") << tr("Filterspeed")*/;
            ...
            if(SoundManager::getInstance().m_Info1->numSamples>0)
            {
                ...
                for(int j = 0; j < SoundManager::getInstance().m_Info1->numSamples; j++)
                {
                    ...
                    if( size > 0 )
                    {
                        ...
                        ui->samples->setItem(j,2,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesSize[j]) ));
                        ui->samples->setItem(j,3,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesVolume[j])  ));
                    }
            ...
            if(SoundManager::getInstance().m_Info1->numSamples>0)
            {
                ...
                for(int j = 0; j < SoundManager::getInstance().m_Info1->numSamples; j++)
                {
                    ...
                    if( size > 0 )
                    {
                        ...
                        ui->samples->setItem(j,5,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesVolume[j]/4)  ));
                        ui->samples->setItem(j,6,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesFineTune[j]/16)  ));
                    }
        ...
        else if(SoundManager::getInstance().m_Info1->plugin == PLUGIN_hivelytracker)
        {
            if(SoundManager::getInstance().m_Info1->numInstruments>0)
            {
                ...
                for(int j = 0; j < SoundManager::getInstance().m_Info1->numInstruments; j++)
                {
                    ui->instruments->setItem(j,0,new QTableWidgetItem(QString::number(j+1)));
                    ui->instruments->setItem(j,1,new QTableWidgetItem(SoundManager::getInstance().m_Info1->instruments[j].c_str()));
                    ui->instruments->setItem(j,2,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->instrumentsVolume[j])));
                    ui->instruments->setItem(j,3,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->instrumentsWavelen[j])));
                }
            }
        }
        ...
        else if(SoundManager::getInstance().m_Info1->plugin == PLUGIN_libfc14audiodecoder)
        {
            if(SoundManager::getInstance().m_Info1->numSamples>0)
            {
                ...
                for(int j = 0; j < SoundManager::getInstance().m_Info1->numSamples; j++)
                {
                    ...
                    if( size > 0 )
                    {
                        ui->samples->setItem(j,1,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesSize[j]) ));
                        ui->samples->setItem(j,2,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesLoopStart[j]) ));
                        ui->samples->setItem(j,3,new QTableWidgetItem(QString::number(SoundManager::getInstance().m_Info1->samplesLoopLength[j]) ));
                    }
        ...
        else if(SoundManager::getInstance().m_Info1->plugin == PLUGIN_libpac)
        {
            ...
            {
                QStringList columnLabelsInstruments;
                columnLabelsInstruments << tr("#") << tr("Name") << tr("Size") << tr("Loopstart") << tr("Loopend") << tr("Volume") << tr("Finetune") << tr("Resolution");
...
void MainWindow::exportInstrument(QString format)
{
    ...
    if(SoundManager::getInstance().m_Info1->plugin==PLUGIN_libxmp &&  ui->samples->rowCount()>0 && SoundManager::getInstance().m_Info1->samplesSize[row]>0)
    {
        ...
        unsigned int loopLength = SoundManager::getInstance().m_Info1->samplesLoopEnd[row]-SoundManager::getInstance().m_Info1->samplesLoopStart[row];

        if(format=="WAV")
        {
            QString fileName = QFileDialog::getSaveFileName(this, "Export sample", "/" + defaultFileName, "Wave (*.wav)" );


            addDebugText("Saving sample to WAV, no. " + QString::number(row) + ", filesize: " + QString::number(SoundManager::getInstance().m_Info1->samplesSize[row]));
        ...
        {
            addDebugText("Saving sample to IFF, no. " + QString::number(row) + ", filesize: " + QString::number(SoundManager::getInstance().m_Info1->samplesSize[row]));

            QString fileName = QFileDialog::getSaveFileName(this, "Export sample", "/" + defaultFileName, "IFF (*.iff)" );
...
void MainWindow::renamePlaylist()
{
    if(ui->listWidget->currentItem()->text()!="Default.m3u")
    {
        ...
        if ((oldName!=newName) && ok && !newName.isEmpty())
        {
            ...
            QString playlistNewName = QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + newName;
            QString playlistOldName = QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + oldName;
...
void MainWindow::savePlaylistAs()
{
    ...
    if ((oldName!=newName) && ok && !newName.isEmpty())
    {
        ...
        tableWidgetPlaylists[newName]->setStyleSheet(ui->dockWidgetContents_4->styleSheet() + "QHeaderView::section{font-family:Roboto;padding:0;} QTableView{padding:9px;}");
        ...
        connect(tableWidgetPlaylists[newName], SIGNAL(doubleClicked(const QModelIndex &)), SLOT(on_playlist_itemDoubleClicked(const QModelIndex &)));

        QUrl u = QUrl::fromLocalFile( QDir::separator() + QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + "/" + newName);
        QList<QUrl> ql;
        ql.append(u);
...
void MainWindow::savePlayList(QString path, QString newPath)
{
    ...
    for(int i = 0; i <tableWidgetPlaylists[fileInfoOld.fileName()]->model()->rowCount() ; i++)
    {
        ...
        QString extInfo = "#EXTINF:" + QString::number(tableWidgetPlaylists[playlistKey]->model()->index(i,5).data().toInt()) + "," + QString(tableWidgetPlaylists[playlistKey]->model()->index(i,0).data().toString());
        out << extInfo << "\n" << path << NEZPLAYLISTSPLITTER << tableWidgetPlaylists[playlistKey]->model()->index(i,1).data().toString() << PLAYLISTFIELDSPLITTER << tableWidgetPlaylists[playlistKey]->model()->index(i,3).data().toString() << PLAYLISTFIELDSPLITTER << tableWidgetPlaylists[playlistKey]->model()->index(i,0).data().toString() << PLAYLISTFIELDSPLITTER << "[time(h:m:s)] " << PLAYLISTFIELDSPLITTER << "[loop(h:m:s)][-]" << PLAYLISTFIELDSPLITTER << "[fade(h:m:s)]" << PLAYLISTFIELDSPLITTER << "[loopcount]" << PLAYLISTFIELDSPLITTER << tableWidgetPlaylists[playlistKey]->model()->index(i,8).data().toString() << "\n";
    }
    file.close();
...
{
    savePlayList(QApplication::applicationDirPath() + QDir::separator() + "user/playlists/" + ui->listWidget->currentItem()->text(),QApplication::applicationDirPath() + QDir::separator() + "user/playlists/" + ui->listWidget->currentItem()->text());
}
void MainWindow::deleteAllPlaylists()
{
    for(int rowNumber = ui->listWidget->count()-1;rowNumber>=0;rowNumber--)
    {
        ...
        if(ui->listWidget->currentItem()->text()!="Default.m3u")
        {
            QString playlistToDelete = QApplication::applicationDirPath() + QDir::separator() + "user/playlists" + QDir::separator() + ui->listWidget->currentItem()->text();
...
void MainWindow::clearPlaylist()
{
    ...
    removeHighlight();
    currentRow=0;
    tableWidgetPlaylists[selectedPlaylist]->model()->removeRows(0, tableWidgetPlaylists[selectedPlaylist]->model()->rowCount());
    ...
    QObject::connect(list, &QListWidget::customContextMenuRequested, [list, this](const QPoint& pos) {
            ...
            if (list->itemAt(pos) != nullptr)
            {
                if (index.row()==0)
                {
                    ...
                    rightClickMenu.addActions({clearPlaylistAction, deleteAllPlaylistsAction, savePlaylistAction, savePlaylistAsAction  });
                    rightClickMenu.exec(list->viewport()->mapToGlobal(pos));
                }
                else
                {
                    QMenu rightClickMenu(list);
                    rightClickMenu.addActions({ renamePlaylistAction, clearPlaylistAction, deletePlaylistAction, deleteAllPlaylistsAction, savePlaylistAction, savePlaylistAsAction  });
                    rightClickMenu.exec(list->viewport()->mapToGlobal(pos));
                }
            }
            else
...
void MainWindow::playPrevSong()
{
    ...
    if((currentRow!=0 && ui->checkBoxShuffle->checkState()==Qt::Unchecked) || (ui->checkBoxShuffle->checkState()==Qt::Checked && m_iCurrentShufflePosition[currentPlaylist]>0) || (ui->checkBoxLoop->checkState()==Qt::PartiallyChecked && ui->checkBoxShuffle->checkState()==Qt::Unchecked))
    {
        if(ui->checkBoxShuffle->checkState()==Qt::Checked)
        {
            m_iCurrentShufflePosition[currentPlaylist]--;
...
vector<PlaylistItem*> MainWindow::getPlayListEntriesM3U(QString filename)
{
    ...
    if (file.open(QIODevice::ReadOnly ))
    {
        ...
        while (!line.isNull())
        {
            ...
            else if(!line.startsWith("#EXTM3U",Qt::CaseInsensitive) && !line.startsWith("#EXTINF",Qt::CaseInsensitive))
            {
                ...
                else //it's a relative path
                {
                    ...
                    else if(!line.startsWith("http://",Qt::CaseInsensitive) && !line.startsWith("https://",Qt::CaseInsensitive))
                    {
...
QString MainWindow::createPlaylist(QString name)
{
    ...
    tableWidgetPlaylists[newItem->text()]->setStyleSheet(ui->dockWidgetContents_4->styleSheet() + "QHeaderView::section{font-family:Roboto;} QTableView{padding:9px;}");
    ...
    connect(tableWidgetPlaylists[newItem->text()], SIGNAL(doubleClicked(const QModelIndex &)), SLOT(on_playlist_itemDoubleClicked(const QModelIndex &)));
...
void MainWindow::quit()
{
    ...
    {
        savePlayList(QApplication::applicationDirPath() + QDir::separator() + "user/playlists/" + e,QApplication::applicationDirPath() + QDir::separator() + "user/playlists/" + e);
    }
...
void MainWindow::on_actionAdd_network_stream_triggered()
{
    ...
    QString url = QInputDialog::getText(this, tr("Add URL"),
                                        tr("Enter URL to open:\nFor example: http://www.myfavradio.com:80/stream/1234"), QLineEdit::Normal,
                                        0, &ok);
    if (ok && !url.isEmpty())
    {
        QList<QUrl> list;
...
void MainWindow::SetAttrRecur(QDomElement elem, QString strtagname, QString strtagname2, QString strattr, QString strattrval)
{
...
void MainWindow::CreateNewWorkspace(QString filename)
{
    ...
    QSettings settings(QApplication::applicationDirPath() + "/user/layouts/" + fileNameAndExtension,QSettings::IniFormat);
    settings.setValue("dockingState", m_DockManager->saveState());
    QAction* action = new QAction(filename);
    ui->menuRestore_Layout->insertAction(workspaceSeparator,action);
    connect(action,  &QAction::triggered, this, [&, this, fileNameAndExtension]{ slot_LoadWorkspace(fileNameAndExtension); });
}
void MainWindow::downloadComplete()
{
    if(filedownloader->downloadedData().size()>0)
    {
        ...
        if ( file.open(QIODevice::ReadWrite) )
        {
            ...
            settings.setValue("libsidplayfp/sidsongpath", QApplication::applicationDirPath() + QDir::separator() +  "user/plugin/sid/Songlengths.md5");
